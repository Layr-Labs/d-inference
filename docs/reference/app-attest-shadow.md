...
| `key_history.generations_last_24h` | int 0–100 | `ready` | `generateKey` attempts in the trailing 24 h, recorded when the hourly budget is consumed (before the Apple call). Rolling list of the last 20 attempts shared across account scopes per coordinator + environment, so it saturates at 20 | Repeated fresh-key churn (e.g. every new key failing with `invalidKey`). |
...
| `push_history.last_reply_sent_age_seconds` | int 0–315360000 | `ready` | now − last `code_attestation_response` sent for an APNs-delivered challenge (recorded right after the send; WebSocket resume-challenge replies are not counted); omitted as above | A push received after the last reply: it arrived but was not answered; compare with `code_attest.push_reply`. |
...
The failure diagnostics page at `/app-attest/diagnostics` (linked from `/app-attest`, same window) groups macOS 27+ connections into the open failure cohorts: `is_supported_false`, fresh-key `apple_invalid_key`, dead-key assertion `apple_error`, stalled `busy` and `identity_required`. It breaks each cohort down by the [provider diagnostics](#provider-diagnostics), the derived reboot/restart flags, OS build, provider version and chip family. It also classifies dead keys per day as OS change, reboot, process restart, no restart or unknown, reports rotation outcomes from `app_attest_key_rotations`, and summarises `push_history` for all OS versions. Rows from providers that predate these fields show as no data rather than zero. Coordinator-side push results are metrics only; see [APNs code identity](../architecture/security/attestation.md#flag--apns-code-identity). Code: `admin-ui/src/lib/queries/app-attest-diagnostics.ts`, `admin-ui/src/lib/app-attest-diagnostics.ts` and `admin-ui/src/app/app-attest/diagnostics/page.tsx`.
