...
func TestMemoryGetMDAChainBySerial_NotShadowedByNewerEmptyRow(t *testing.T) {
	...
	if err := st.UpsertProvider(ctx, contracts.ProviderRecord{
...
func TestMemoryListProvidersByAccount(t *testing.T) {
	...
	records := []contracts.ProviderRecord{
...
func TestMemoryDeleteProvidersBySerial(t *testing.T) {
	...
	if err := st.UpsertReputation(ctx, "a", contracts.ReputationRecord{TotalJobs: 5}); err != nil {
...
func TestMemoryDeleteProvidersBySerial_ByID(t *testing.T) {
	...
	if err := st.UpsertProvider(ctx, contracts.ProviderRecord{ID: "noserial", AccountID: "acct-1", LastSeen: time.Now()}); err != nil {
...
func TestMemoryDeleteProvidersBySerial_WrongOwnerNoOp(t *testing.T) {
	...
	if err := st.UpsertProvider(ctx, contracts.ProviderRecord{ID: "a", SerialNumber: "SER", AccountID: "acct-1", LastSeen: time.Now()}); err != nil {
		t.Fatalf("UpsertProvider: %v", err)
	}
