...
func (s *Server) startMachineInventory(ctx context.Context, p *registry.Provider, r *protocol.RegisterMessage, account string) *machineInventorySession {
	...
	x := &machineInventorySession{s: s, p: p, store: st, ready: make(chan struct{}), observation: store.MachineObservation{
		...
		Chip: boundedShadowLabel(r.Hardware.ChipName), MemoryGB: float64(r.Hardware.MemoryGB), Protocol: r.AppAttestProtocol, ShadowEnabled: s.appAttestShadow.Enabled,
	}}
...
func (x *machineInventorySession) capture(disconnected bool) {
	...
	defer cancel()
	if x.s.machineInventorySlots != nil {
		select {
		case x.s.machineInventorySlots <- struct{}{}:
			defer func() { <-x.s.machineInventorySlots }()
		case <-ctx.Done():
