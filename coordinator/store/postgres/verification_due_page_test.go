...
func seedDueVerificationJobs(tb testing.TB, s verificationJobStore, prefix string, n int, now time.Time) {
	tb.Helper()
	for i := range n {
		if _, err := s.UpsertVerificationJob(context.Background(), contracts.VerificationJob{
			SEPubKey:      fmt.Sprintf("%s-%04d", prefix, i),
			Serial:        fmt.Sprintf("serial-%s-%04d", prefix, i),
			Kind:          contracts.VerificationTaskSecurityInfo,
			State:         contracts.VerificationStatePending,
			Priority:      contracts.VerificationPriorityRefresh,
