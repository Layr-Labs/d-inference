...
| W5 | Attestation-churn unlock (retry/backoff, freshness, cache) | coordinator attestation/provider | — | **Fix 6 (observability) + Fixes 1/3/4/5 DONE** (W5b: round-trip verified in read-loop delivery path → late replies/reconnects attest, fail-closed preserved; mode-aware backoff+jitter; freshness 6m→16m; expiry/timeout 60/90→300s). Fix 2 (provider token in heartbeat + reuse persistence) deferred. **Fix 0 (`APNS_MODE=alert`) = human config flip.** See routing-v2-attestation-churn.md |
