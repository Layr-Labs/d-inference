...
var routeCSVHeader = []string{
	...
	"cancel_phase", "cancel_source", "partial_settlement_status",
	"settled_partial_tokens", "settled_partial_micro_usd",
	"cancel_signal_sent_at_ms", "provider_terminal_received", "provider_terminal_at_ms",
	"first_chunk_at_ms", "last_chunk_at_ms", "chunks_sent", "bytes_sent",
	"estimated_delivered_tokens", "max_idle_gap_ms",
...
func routeCSVRow(rec store.InferenceRouteRecord) []string {
	return []string{
		...
		rec.CancelPhase,
		rec.CancelSource,
		rec.PartialSettlementStatus,
		csvInt(rec.SettledPartialTokens),
		csvI64(rec.SettledPartialMicroUSD),
		csvI64(rec.CancelSignalSentAtMs),
		csvBool(rec.ProviderTerminalReceived),
		csvI64(rec.ProviderTerminalAtMs),
		csvI64(rec.FirstChunkAtMs),
		csvI64(rec.LastChunkAtMs),
		csvInt(rec.ChunksSent),
		csvI64(rec.BytesSent),
		csvInt(rec.EstimatedDeliveredTokens),
		csvFloat(rec.MaxIdleGapMs),
