...
func newWithdrawableMigrationStore(t *testing.T, databaseURL string) *Store {
	t.Helper()
...
func prepareWithdrawableMigrationSchema(t *testing.T, s *Store, withWithdrawableColumn bool) {
	t.Helper()
	balanceColumns := `
...
func seedMigrationBalance(t *testing.T, s *Store, accountID string, balance int64, withdrawable *int64) {
	t.Helper()
	var err error
	if withdrawable == nil {
...
func migrationWithdrawableBalance(t *testing.T, s *Store, accountID string) int64 {
	t.Helper()
	var amount int64
...
func migrationMarker(t *testing.T, s *Store) (count int, appliedAt time.Time) {
	t.Helper()
	if err := s.pool.QueryRow(context.Background(),
