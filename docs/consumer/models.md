...
| `quantization` | string | Quantization of a concrete build; empty on alias entries because an alias spans quants. Decorated labels use the earliest recognized format, preferring the longest spelling at that position: `bfloat16-gs64` maps to `bf16`, while `q4-bfloat16` maps to `int4`. Unrecognized labels are omitted. | `mapQuantizationToOpenRouter` (`coordinator/api/openrouter_models.go`) |
