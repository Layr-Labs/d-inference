...
struct ToolChoiceViolationErrorTests {
    ...
    @Test func mapsTo422() {
        for message in violations {
            let status = WorkerInferenceSupport.mapInferenceErrorToStatus(
                MultiModelBatchSchedulerEngineError.toolChoiceViolation(message))
            #expect(status == 422, "\(message)")
        }
        #expect(
            WorkerInferenceSupport.mapInferenceErrorToStatus(
    ...
    @Test func serviceOutputFailuresUseRetryableWireVocabulary() {
        let failures = [
            WorkerInferenceSupport.sanitizedInferenceFailure(
