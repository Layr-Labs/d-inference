...
func TestLegacyCacheAffinityScrubMigration(t *testing.T) {
	for _, required := range []string{
		...
		if !strings.Contains(startupschema.LegacyCacheAffinityScrubMigration, required) {
			t.Fatalf("legacy cache-affinity scrub migration missing %q", required)
		}
	}
	if !strings.Contains(startupschema.LegacyCacheAffinityGuardFunction, "NEW.cache_affinity_key := ''") ||
		!strings.Contains(startupschema.LegacyCacheAffinityGuardTrigger, "BEFORE INSERT OR UPDATE OF cache_affinity_key") {
	...
	for _, required := range []string{
		...
		if !strings.Contains(startupschema.LegacyCacheAffinityGuardTrigger, required) {
...
func TestLegacyCacheAffinityMigrationScrubsAndInstallsScopedTrigger(t *testing.T) {
	...
	for _, migration := range []string{
		startupschema.LegacyCacheAffinityGuardFunction,
		startupschema.LegacyCacheAffinityGuardTrigger,
		startupschema.LegacyCacheAffinityScrubMigration,
	} {
...
func testInferenceRouteStore(t *testing.T, s contracts.Store) {
	t.Helper()

	beforeRecord := time.Now().Add(-time.Minute)

	rec := &contracts.InferenceRouteRecord{
		RequestID:               "req-1",
		Attempt:                 1,
		ProviderID:              "prov-1",
	...
	outcome := &contracts.InferenceRouteOutcome{
		FinalStatus:            "success",
		ErrorCode:              0,
		ErrorClass:             "",
		ErrorReason:            "",
	...
	if err := s.UpdateInferenceRouteOutcome("req-1", 1, &contracts.InferenceRouteOutcome{FinalStatus: "error", ErrorClass: "provider_error", ErrorCode: 500, ErrorReason: "jinja_template"}); err != nil {
	...
	// token/cost fields.
	if err := s.UpdateInferenceRouteOutcome("req-1", 1, &contracts.InferenceRouteOutcome{ActualTTFTMs: 175}); err != nil {
	...
	rec2 := &contracts.InferenceRouteRecord{
		RequestID:  "req-1",
		Attempt:    2,
		ProviderID: "prov-2",
		Model:      rec.Model,
	...
	queued := &contracts.InferenceRouteRecord{
	...
	all = s.InferenceRouteRecordsSince(time.Time{})
	var queuedGot *contracts.InferenceRouteRecord
	queuedCount := 0
	for i := range all {
...
func testRejectionStore(t *testing.T, s contracts.Store) {
	t.Helper()

	beforeRecord := time.Now().Add(-time.Minute)

	rec := &contracts.RejectionRecord{
		RequestID:               "req-rej-1",
	...
	rec2 := &contracts.RejectionRecord{
		RequestID:      "req-rej-2",
		Endpoint:       "/v1/chat/completions",
	...
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			for i, state := range states {
				if err := st.RecordRejection(&contracts.RejectionRecord{RequestID: fmt.Sprintf("state-%d", i), CouldHaveServed: state, CreatedAt: time.Now()}); err != nil {
					t.Fatal(err)
				}
			}
