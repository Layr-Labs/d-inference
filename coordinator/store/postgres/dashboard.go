...
func (s *Store) AccountEarningsWindows(accountID string, now time.Time) (contracts.AccountEarningsWindows, error) {
	...
	if err != nil {
		return contracts.AccountEarningsWindows{}, fmt.Errorf("store: account earnings windows: %w", err)
	}
	return w, nil
}

func (s *Store) GetReputations(ctx context.Context, providerIDs []string) (map[string]*contracts.ReputationRecord, error) {
