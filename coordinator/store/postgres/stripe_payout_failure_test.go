...
func TestStripePayoutFailureChecksCurrentOwnership(t *testing.T) {
	for backend, s := range storeBackends(t) {
		t.Run(backend, func(t *testing.T) {
			if err := s.CreateUser(&contracts.User{AccountID: "acct-sweep-guard", PrivyUserID: "did:privy:sweep-guard"}); err != nil {
				t.Fatal(err)
			}
			for _, tc := range []struct {
				...
				t.Run(tc.id, func(t *testing.T) {
					wd := &contracts.StripeWithdrawal{ID: tc.id, AccountID: "acct-sweep-guard", StripeAccountID: "acct_stripe_guard",
