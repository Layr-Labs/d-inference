...
| **Heartbeat** / **eviction** | Provider state report over the WebSocket (provider default every 5s; coordinator liveness window 30s); two consecutive missed sweeps (`evictStrikeThreshold` = 2, ~90s) evict | [`architecture/scheduling.md`](architecture/scheduling.md) |
