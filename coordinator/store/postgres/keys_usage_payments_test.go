...
func TestCreateKey(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if !strings.HasPrefix(key, contracts.KeyPrefix) {
				t.Errorf("key %q does not have %q prefix", key, contracts.KeyPrefix)
			}
...
func TestProviderPayouts_RecordListAndSettle(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			p1 := &contracts.ProviderPayout{
				ProviderAddress: "0xProvider1",
				AmountMicroUSD:  900_000,
				Model:           "qwen3.5-9b",
				JobID:           "job-1",
			}
			p2 := &contracts.ProviderPayout{
				ProviderAddress: "0xProvider2",
				AmountMicroUSD:  450_000,
				Model:           "llama-3",
				JobID:           "job-2",
			}
			for _, payout := range []*contracts.ProviderPayout{p1, p2} {
...
func TestCreditProviderAccountAtomic(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if history[0].Type != contracts.LedgerPayout {
...
func TestCreditProviderWalletAtomic(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			payout := &contracts.ProviderPayout{
