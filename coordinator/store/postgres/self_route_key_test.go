...
func exerciseSelfRouteOnlyRoundTrip(t *testing.T, s contracts.Store) {
	t.Helper()

	raw, rec, err := s.CreateAPIKey("acct-self", contracts.APIKeyCreate{Name: "my-machine", SelfRouteOnly: true})
	if err != nil {
	...
	_, plain, err := s.CreateAPIKey("acct-self", contracts.APIKeyCreate{Name: "normal"})
	if err != nil {
...
func TestMemorySelfRouteOnlyRoundTrip(t *testing.T) {
	exerciseSelfRouteOnlyRoundTrip(t, memory.New(contracts.Config{}))
}
