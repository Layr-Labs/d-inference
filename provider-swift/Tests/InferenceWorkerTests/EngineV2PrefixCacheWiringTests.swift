...
struct EngineV2PrefixCacheUsageTests {
    ...
    func injectCachedTokens() throws {
        let usageFrame = """
            ...
            """
        let out = WorkerInferenceSupport.injectCachedTokens(into: usageFrame, cachedTokens: 256)
    ...
    func injectMergesExistingDetails() throws {
        let frame = """
            ...
            """
        let out = WorkerInferenceSupport.injectCachedTokens(into: frame, cachedTokens: 8)
        let payload = try #require(out.split(separator: "\n").first?
            .replacingOccurrences(of: "data: ", with: ""))
        let obj = try #require(
            try JSONSerialization.jsonObject(with: Data(payload.utf8)) as? [String: Any])
    ...
    func injectPassThrough() {
        ...
        #expect(WorkerInferenceSupport.injectCachedTokens(into: contentFrame, cachedTokens: 5)
            == contentFrame)
        let done = "data: [DONE]\n\n"
        #expect(WorkerInferenceSupport.injectCachedTokens(into: done, cachedTokens: 5) == done)
        let usageFrame = "data: {\"usage\":{\"prompt_tokens\":10}}\n\n"
        #expect(WorkerInferenceSupport.injectCachedTokens(into: usageFrame, cachedTokens: 0) == usageFrame)
    }
}
