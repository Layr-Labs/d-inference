...
// works.
func minimalMediaServer(cfg mediafetch.Config) *Controller {
...
func sealedReq() *http.Request {
	...
	return req.WithContext(context.WithValue(req.Context(), fixtureSealedKey{}, struct{}{}))
}
...
func TestResolveRemoteMediaInlinesOnSuccess(t *testing.T) {
	...
	raw, parsed := inferencefixture.ChatBody(t, media.URL+"/cat.png")
...
func TestResolveRemoteMediaNoRemoteNoOp(t *testing.T) {
	...
	s.deps.FirstContentDeadlineBase = func() time.Duration { return 5 * time.Second }
	raw, parsed := inferencefixture.ChatBody(t, "data:image/png;base64,iVBORw0KGgo=")
	w := httptest.NewRecorder()
...
func TestMediaFetchBudgetUsesPinnedQwenDeadline(t *testing.T) {
	srv, _ := testControllerWithConfig(t, testConfig{
...
func TestResolveRemoteMediaExpiredFirstContentClockDoesNotFetch(t *testing.T) {
	...
	s := minimalMediaServer(cfg)
	s.deps.FirstContentDeadlineBase = func() time.Duration { return 9 * time.Second }
	raw, parsed := inferencefixture.ChatBody(t, media.URL+"/cat.png")
	w := httptest.NewRecorder()
	timing := &registry.RequestTiming{ReceivedAt: time.Now().Add(-15 * time.Second)}
...
func TestResolveRemoteMediaNilResolverPassthrough(t *testing.T) {
	s := newTestController(testServices{logger: quietLogger()}) // e.g. a bare test Server
	raw, parsed := inferencefixture.ChatBody(t, "https://example.com/cat.png")
	w := httptest.NewRecorder()
...
func TestResolveRemoteMediaClientCancellationWritesNoRejection(t *testing.T) {
	s := minimalMediaServer(mediafetch.DefaultConfig())
	raw, parsed := inferencefixture.ChatBody(t, "https://example.com/cancelled.png")
...
func TestResolveRemoteMediaFailureWrites400(t *testing.T) {
	...
	raw, parsed := inferencefixture.ChatBody(t, media.URL+"/fake.png")
	w := httptest.NewRecorder()
	...
	if code := inferencefixture.ErrorType(t, w.Body.Bytes()); code != "media_invalid_type" {
...
func TestResolveRemoteMediaNeverLogsPresignedURLSecrets(t *testing.T) {
	...
	s := newTestController(testServices{logger: logger, mediaResolver: mediafetch.NewResolver(cfg, logger)})
	...
	raw, parsed := inferencefixture.ChatBody(t, media.URL+"/private.png?"+secret)
	w := httptest.NewRecorder()
...
func TestGateSealedRejectsRemoteWithoutFetching(t *testing.T) {
	...
	_, parsed := inferencefixture.ChatBody(t, media.URL+"/cat.png")
	w := httptest.NewRecorder()
...
func TestGateSealedAllowsInlineData(t *testing.T) {
	s := minimalMediaServer(mediafetch.DefaultConfig())
	_, parsed := inferencefixture.ChatBody(t, "data:image/png;base64,iVBORw0KGgo=")
	w := httptest.NewRecorder()
	if s.gateRemoteMediaPreDispatch(w, sealedReq(), parsed, "test", "test", true, false) {
...
func TestGateRemoteFetchableDefersToResolver(t *testing.T) {
	s := minimalMediaServer(mediafetch.DefaultConfig())
	_, parsed := inferencefixture.ChatBody(t, "https://example.com/cat.png")
	w := httptest.NewRecorder()
...
func TestResolveRemoteMediaSelfRouteUnavailableSkipsFetch(t *testing.T) {
	srv, _ := testController(t) // real registry + store, no linked providers
	cfg := mediafetch.DefaultConfig()
	cfg.AllowPrivateIPs = true
	cfg.AllowNonStandardPorts = true
...
func TestPreludeDefersRemoteMediaResolution(t *testing.T) {
	...
	body, _ := inferencefixture.ChatBody(t, media.URL+"/x.png")
...
func TestResolveRemoteMediaSelfRouteUsesFullTraits(t *testing.T) {
	...
	makeVisionRoutableProvider(t, srv.deps.Registry(), "self-route-traits", "test")
	for _, id := range srv.deps.Registry().ProviderIDs() {
		if p := srv.deps.Registry().GetProvider(id); p != nil {
			p.Mu().Lock()
			p.AccountID = owner
	...
	if !srv.deps.Registry().HasToolCapableProviderForModel("test") {
