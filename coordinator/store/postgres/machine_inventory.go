...
func (s *Store) ObserveMachine(ctx context.Context, o contracts.MachineObservation) (contracts.MachineIdentity, error) {
	var result contracts.MachineIdentity
	...
	returnExisting := func() (contracts.MachineIdentity, error) {
		result.ID = existing
	...
	if existing != "" && inventoryrecord.ObservationSuperseded(lastSeen, disconnectedAt != nil, disconnectReason, o) {
		return returnExisting()
	}
	...
	result.Assurance = inventoryrecord.Assurance(o)
	...
	result.Assurance = inventoryrecord.StrongerAssurance(oldAssurance, result.Assurance)
