...
extension EngineV2Bridge {
    ...
    nonisolated func prefixCacheAdvertisement() -> (
        capability: PrefixCacheV2Capability?,
        status: PrefixCacheModelStatus
    ) {
        guard let ssdPrefixCache else {
            return (nil, prefixCacheBaseStatus)
        }
        return ssdPrefixCache.prefixCacheAdvertisement(base: prefixCacheBaseStatus)
    }

    nonisolated func takeNextPrefixCacheV2Sequence(
        expectedEpoch: String
    ) -> UInt64? {
        ssdPrefixCache?.takeNextPrefixCacheV2Sequence(
