...
> Last updated: 2026-09-14 · commit `beadb4338`
...
| `coordinator/registry/dispatchplan` | Private bounded alternates, quote ranking, attempted IDs and one-refresh claim (`Plan`); quote correlation, expiry and settlement-before-outcome publication (`Probes`). The registry retains exact provider identity, admission, heartbeat and transport bindings. |
