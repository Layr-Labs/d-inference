...
func seedAccountEarnings(t *testing.T, st contracts.Store, accountID string, n int, now time.Time) (want contracts.AccountEarningsWindows) {
	t.Helper()
	span := 7 * 24 * time.Hour
	rows := make([]contracts.ProviderEarning, 0, n)
	for i := range n {
		...
		rows = append(rows, contracts.ProviderEarning{
...
func TestAccountEarningsWindowsExactBeyondPageLimit(t *testing.T) {
	...
	results := map[string]contracts.AccountEarningsWindows{}
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if err != nil || other != (contracts.AccountEarningsWindows{}) {
	...
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			for i := range 20 {
				...
				if err := st.UpsertReputation(ctx, id, contracts.ReputationRecord{
