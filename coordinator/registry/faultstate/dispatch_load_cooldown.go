...
// on the provider's stable-identity gate (state.go) so the cool-down
// survives a reconnect within its TTL; takes only gate.mu.
func (r *Manager[C]) RecordDispatchLoadFailure(providerID, modelID string) bool {
...
func (r *Manager[C]) ClearDispatchLoadCooldown(providerID, modelID string) {
...
func (r *Manager[C]) DispatchLoadCooled(providerID, modelID string, now time.Time) bool {
