...
struct ReasoningParserSelectionTests {
    ...
    func gemmaSelectsGemma4() {
        #expect(WorkerInferenceSupport.inferReasoningParser(for: "gemma") == .gemma4)
        #expect(WorkerInferenceSupport.inferReasoningParser(for: "gemma3") == .gemma4)
        #expect(WorkerInferenceSupport.inferReasoningParser(for: "Gemma") == .gemma4)
    }
    ...
    func nilFallbackDiffersFromGemma() {
        ...
        #expect(WorkerInferenceSupport.inferReasoningParser(for: nil) == .qwen3)
        ...
        #expect(WorkerInferenceSupport.inferReasoningParser(for: "gemma") != WorkerInferenceSupport.inferReasoningParser(for: nil))
    }

    @Test("other known families map to their parsers")
    func knownFamilies() {
        #expect(WorkerInferenceSupport.inferReasoningParser(for: "gpt_oss") == .harmony)
        #expect(WorkerInferenceSupport.inferReasoningParser(for: "qwen") == .qwen3)
        #expect(WorkerInferenceSupport.inferReasoningParser(for: "deepseek") == .deepseekR1)
    }
}
