...
use crate::support::pg::flows::{self, CONSUMER, PROVIDER_BENEFICIARY, REFERRER};
...
async fn reserve_idempotent_replay_debits_once() {
    ...
    pg::seed_consumer(&db.pool, CONSUMER, 10_000_000, 4_000_000).await;
    ...
    assert_eq!(
        pg::balance_of(&db.pool, CONSUMER).await,
        (3_000_000, 3_000_000)
    );
    ...
    assert_eq!(
        pg::count(&db.pool, "SELECT COUNT(*) FROM rust_coord.inference_jobs").await,
        1
    );
    assert_eq!(
        pg::count(
...
async fn reserve_insufficient_funds_moves_nothing() {
    ...
    let db = pg::boot().await;
    pg::set_epoch(&db.pool, 1).await;
    pg::seed_consumer(&db.pool, CONSUMER, 1_000_000, 0).await;
    let ledger = pg::ledger_at_epoch(&db.pool, 1);
    ...
    assert_eq!(pg::balance_of(&db.pool, CONSUMER).await, (1_000_000, 0));
    assert_eq!(
        pg::count(&db.pool, "SELECT COUNT(*) FROM rust_coord.inference_jobs").await,
        0
    );
...
async fn reserve_enforces_spend_cap_against_active_reservations() {
    ...
    let db = pg::boot().await;
    pg::set_epoch(&db.pool, 1).await;
    pg::seed_consumer(&db.pool, CONSUMER, 20_000_000, 0).await;
    let ledger = pg::ledger_at_epoch(&db.pool, 1);
...
async fn resize_freeze_adjusts_reservation_and_freezes_terms() {
    ...
    assert_eq!(pg::job_state(&db.pool, job.get()).await, "start_authorized");
...
async fn settle_happy_path_with_fees_and_projections() {
    ...
    assert_eq!(
        pg::balance_of(&db.pool, PROVIDER_BENEFICIARY).await,
        (3_400_000, 3_400_000)
    );
    // Platform/referrer balances are NOT updated synchronously (plan §12.6).
    assert_eq!(pg::balance_of(&db.pool, "platform").await, (0, 0));
    assert_eq!(pg::balance_of(&db.pool, REFERRER).await, (0, 0));
    ...
    assert_eq!(pg::count(&db.pool, "SELECT COUNT(*) FROM usage").await, 1);
    assert_eq!(
        pg::count(&db.pool, "SELECT COUNT(*) FROM provider_earnings").await,
        1
    );
    assert_eq!(
        pg::count(
    ...
    assert_eq!(pg::job_state(&db.pool, job.get()).await, "settled");
...
async fn different_digest_conflict_moves_no_money() {
    ...
    ledger
        ...
        .await
        .expect("settle");
    let before_consumer = pg::balance_of(&db.pool, CONSUMER).await;
    let before_provider = pg::balance_of(&db.pool, PROVIDER_BENEFICIARY).await;
    ...
    assert_eq!(pg::balance_of(&db.pool, CONSUMER).await, before_consumer);
    assert_eq!(
        pg::balance_of(&db.pool, PROVIDER_BENEFICIARY).await,
        before_provider
    );
...
async fn release_restores_exact_provenance() {
    ...
    let db = pg::boot().await;
    pg::set_epoch(&db.pool, 1).await;
    pg::seed_consumer(&db.pool, CONSUMER, 6_000_000, 4_000_000).await;
    let ledger = pg::ledger_at_epoch(&db.pool, 1);
    ...
    assert_eq!(pg::job_state(&db.pool, job.get()).await, "released");
...
async fn terminal_after_release_is_recorded_late() {
    ...
    let db = pg::boot().await;
    pg::set_epoch(&db.pool, 1).await;
    pg::seed_consumer(&db.pool, CONSUMER, 6_000_000, 0).await;
    let ledger = pg::ledger_at_epoch(&db.pool, 1);
    ...
    ledger
        ...
        .await
        .expect("release");
    let restored = pg::balance_of(&db.pool, CONSUMER).await;
    ...
    assert_eq!(pg::balance_of(&db.pool, CONSUMER).await, restored);
...
async fn capped_completion_flags_review_and_retains_reservation() {
    ...
    let after_resize = pg::balance_of(&db.pool, CONSUMER).await;
    ...
    assert_eq!(pg::job_state(&db.pool, job.get()).await, "review_pending");
    // Reservation retained: balances unchanged from the post-resize state.
    assert_eq!(pg::balance_of(&db.pool, CONSUMER).await, after_resize);
    assert_eq!(pg::balance_of(&db.pool, PROVIDER_BENEFICIARY).await, (0, 0));
    assert_eq!(
        pg::count(&db.pool, "SELECT COUNT(*) FROM rust_coord.fee_allocations").await,
        0
    );
}
...
async fn unverified_v2_terminal_parks_review_but_v1_settles() {
    ...
    let db = pg::boot().await;
    pg::set_epoch(&db.pool, 1).await;
    pg::seed_consumer(&db.pool, CONSUMER, 20_000_000, 4_000_000).await;
    let ledger = pg::ledger_at_epoch(&db.pool, 1);
    ...
    assert!(!outcome.flagged_for_review);
    assert_eq!(outcome.charged.get(), 4_000_000);
    assert_eq!(pg::job_state(&db.pool, v1_job.get()).await, "settled");
}
...
async fn epoch_fence_rejects_stale_coordinator() {
    ...
    let before = pg::balance_of(&db.pool, CONSUMER).await;

    // Another coordinator bumps the epoch behind this ledger's back.
    pg::set_epoch(&db.pool, 2).await;
    ...
    assert_eq!(pg::balance_of(&db.pool, CONSUMER).await, before);
    assert_eq!(pg::job_state(&db.pool, job.get()).await, "running");
}
