...
func waitForDrops(t *testing.T, sink *Sink, n int64) {
	t.Helper()
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
...
func TestTelemetrySinkClampsWorkersToOne(t *testing.T) {
	sink := NewBatching(quietLogger(), 8, 4, 256, time.Second)
	...
	if got := New(quietLogger(), 8, 0); got.workerCount != 1 {
...
func TestTelemetrySinkCloseIsBoundedByStuckStore(t *testing.T) {
	...
	sink := NewBatching(quietLogger(), 8, 1, 1, time.Millisecond)
	sink.Bind(st)
