...
func TestCapacityRateIsIndependentOfOutcomeOrder(t *testing.T) {
	...
	for _, tc := range []struct {
		name   string
		record func(r *testManager, provider string)
	}{
		{
			name: "accepts first",
			record: func(r *testManager, provider string) {
...
func TestFaultTimestampHistoriesStayOrderedAcrossIdentityRebind(t *testing.T) {
	...
	p := attachTestSession(r, provider)
	bindTestIdentity(r, p, &attestation.VerificationResult{Valid: true, PublicKey: publicKey})
	...
	bindTestIdentity(r, p, &attestation.VerificationResult{
