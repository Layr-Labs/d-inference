...
func TestShouldStopFailover_ClientError400(t *testing.T) {
	d := &execution{s: newTestController(t), model: "m", lastErrCode: 400, lastErr: "assistant message contains multiple tool_calls; Harmony supports one tool call per assistant message"}
...
func TestShouldStopFailover_413And415(t *testing.T) {
	for _, code := range []int{413, 415} {
		d := &execution{s: newTestController(t), model: "m", lastErrCode: code}
		if !d.shouldStopFailover() {
...
func TestShouldStopFailover_422FailsOver(t *testing.T) {
	d := &execution{s: newTestController(t), model: "m", lastErrCode: 422, lastErr: "model output was not valid JSON"}
	if d.shouldStopFailover() {
...
func TestShouldStopFailover_ColdMiss404StillFailsOver(t *testing.T) {
	d := &execution{s: newTestController(t), model: "m", lastErrCode: 404, lastErr: "Model 'm' is not loaded on this provider"}
	if d.shouldStopFailover() {
...
func TestShouldStopFailover_QueueFull429StillFailsOver(t *testing.T) {
	d := &execution{s: newTestController(t), model: "m", lastErrCode: 429, lastErr: "request rejected: queue full"}
	if d.shouldStopFailover() {
...
func TestClientErrorStop_KillSwitch(t *testing.T) {
	...
	s.deps.DisableClientErrorStop = func() bool { return true }
	d := &execution{s: s, model: "m", lastErrCode: 400, lastErr: "invalid tool payload"}
	if d.shouldStopFailover() {
...
func TestClientErrorRouteOutcome_NotAdmittedButFailed(t *testing.T) {
	...
	out := attempt.PreCommitProviderErrorOutcome(pr, protocol.InferenceErrorMessage{StatusCode: 400, Error: "invalid tool payload"})
	if out.ErrorClass != attempt.ErrorClassClientError {
		t.Fatalf("400 outcome class = %q, want %q", out.ErrorClass, attempt.ErrorClassClientError)
	}
	...
	d := &execution{s: newTestController(t), model: "m", lastErrCode: 400, lastErr: "invalid tool payload"}
	...
	fout := attempt.PreCommitProviderErrorOutcome(pr, protocol.InferenceErrorMessage{StatusCode: 500, Error: "boom"})
