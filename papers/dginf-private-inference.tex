...
The protocol relies on two facts about Apple's push infrastructure:
\begin{enumerate}[leftmargin=*,itemsep=1pt]
    \item Only a process signed with our Developer ID, carrying our globally unique App ID \texttt{io.darkbloom.provider}, and authorized by an Apple-signed provisioning profile with the \texttt{aps-environment} entitlement can receive a push for our topic. Apple's Mobile File Integrity (AMFI) enforces this at launch.
    \item The provider holds an in-memory X25519 key $K$ that is bound to its SE identity and used to decrypt every inference request.
\end{enumerate}

At registration the provider sends its APNs device token $T$ alongside its X25519 public key $K$ and SE public key. The coordinator:
\begin{enumerate}[leftmargin=*,itemsep=1pt]
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    ...
    \item \textbf{Tier~2 (\texttt{hardware}):} Combines four independent verification mechanisms:
    \begin{enumerate}[leftmargin=*,itemsep=1pt]
    ...
    \end{enumerate}
    \item \textbf{Code-attested gate (\texttt{code\_attested}):} The effective requirement for private text traffic once code-attestation enforcement is enabled. In addition to Tier~2, the provider must pass the APNs code-identity round-trip (Section~\ref{sec:apns-attest}), proving it runs the genuine Darkbloom binary. The provider signs each inference response with the bound Secure Enclave key: $\sigma = \text{ECDSA-P256}(\text{SHA-256}(\text{request\_id} \| \text{token\_count} \| \text{content}))$, providing per-response integrity that any party can verify.
\end{itemize}
...
\begin{enumerate}[leftmargin=*,itemsep=1pt]
    ...
    \item Coordinator computes nonce $n = \text{SHA-256}(pk_k)$
    \item Coordinator sends MDM \texttt{DeviceInformation} command with \texttt{DeviceAttestationNonce} = $n$
    \item Device contacts Apple's attestation servers; Apple generates a fresh MDA certificate chain with $\text{FreshnessCode} = n$
    \item Coordinator verifies the certificate chain against Apple Enterprise Attestation Root CA
    \item Coordinator verifies $\text{FreshnessCode} = n$, confirming the binding
\end{enumerate}
