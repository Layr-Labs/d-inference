...
func TestPostgresSetUserStripeAccount(t *testing.T) {
	...
	u := &contracts.User{AccountID: "acct-pg-1", PrivyUserID: "did:privy:pg1", Email: "a@b"}
...
func TestPostgresCreateUserPersistsRoleAndFee(t *testing.T) {
	...
	if got.Role != contracts.RoleService {
		t.Errorf("role = %q, want %q (dropped on insert)", got.Role, contracts.RoleService)
	}
	...
	if err := s.CreateUser(&contracts.User{AccountID: "acct-pg-plain", PrivyUserID: "did:privy:pgplain", Email: "p@b"}); err != nil {
...
func TestPostgresStripeWithdrawalCRUD(t *testing.T) {
	...
	u := &contracts.User{AccountID: "acct-pg-wd", PrivyUserID: "did:privy:pgwd"}
	_ = s.CreateUser(u)
	_ = s.SetUserStripeAccount("acct-pg-wd", "acct_wd", "ready", "", "bank", "6789", false)

	wd := &contracts.StripeWithdrawal{
		ID:              "wd-pg-1",
	...
	if _, err := s.GetStripeWithdrawalByPayoutID("po_pg_missing"); !errors.Is(err, contracts.ErrNotFound) {
		t.Errorf("missing payout lookup err = %v, want ErrNotFound", err)
	}
	if _, err := s.GetStripeWithdrawalByTransferID("tr_pg_missing"); !errors.Is(err, contracts.ErrNotFound) {
	...
	applied, err := s.CreditWithdrawableOnce("acct-pg-wd", 500_000, contracts.LedgerRefund, "stripe_withdraw_fee:wd-pg-1")
	if err != nil || !applied {
...
func TestPostgresStripeWithdrawalRefundFlag(t *testing.T) {
	s := testPostgresStore(t)
	u := &contracts.User{AccountID: "acct-pg-rf", PrivyUserID: "did:privy:pgrf"}
	_ = s.CreateUser(u)
...
func TestPostgresStripeWithdrawalDuplicateIDRejected(t *testing.T) {
	s := testPostgresStore(t)
	u := &contracts.User{AccountID: "acct-pg-dup", PrivyUserID: "did:privy:pgdup"}
	_ = s.CreateUser(u)
...
func newPostgresWithMaxConns(t *testing.T, maxConns int32) *Store {
	t.Helper()
...
func TestPostgresWalletPriceCleanupPreservesPlatform(t *testing.T) {
	...
	if err := s.CreateUser(&contracts.User{AccountID: "acct-real", PrivyUserID: "did:privy:real"}); err != nil {
		t.Fatalf("create user: %v", err)
	}
...
func TestPostgresDeleteProvidersBySerial(t *testing.T) {
	...
	for _, rec := range []contracts.ProviderRecord{
	...
	// provider would fail.
	if err := s.UpsertReputation(ctx, "a", contracts.ReputationRecord{TotalJobs: 7}); err != nil {
...
func TestPostgresDeleteProvidersBySerial_WrongOwner(t *testing.T) {
	...
	if err := s.UpsertProvider(ctx, contracts.ProviderRecord{ID: "a", Hardware: json.RawMessage(`{}`), Models: json.RawMessage(`[]`), Backend: "vllm_mlx", SerialNumber: "SER", AccountID: "acct-1", RegisteredAt: time.Now(), LastSeen: time.Now()}); err != nil {
		t.Fatalf("UpsertProvider: %v", err)
	}
...
func TestPostgresCreateStripeWithdrawalWithDebit(t *testing.T) {
	s := testPostgresStore(t)
	u := &contracts.User{AccountID: "acct-pg-wdb", PrivyUserID: "did:privy:pgwdb"}
	_ = s.CreateUser(u)
	if err := s.CreditWithdrawable("acct-pg-wdb", 10_000_000, contracts.LedgerPayout, "earnings"); err != nil {
		t.Fatal(err)
	}
	...
	if err := s.CreateStripeWithdrawalWithDebit(wd, contracts.LedgerStripePayout, "stripe_withdraw:wd-pg-atomic-1"); err != nil {
	...
	wd2 := &contracts.StripeWithdrawal{
		ID: "wd-pg-atomic-2", AccountID: "acct-pg-wdb", StripeAccountID: "acct_pgwdb",
		AmountMicroUSD: 60_000_000, NetMicroUSD: 60_000_000,
		Method: "standard", Status: "pending",
	}
	err = s.CreateStripeWithdrawalWithDebit(wd2, contracts.LedgerStripePayout, "stripe_withdraw:wd-pg-atomic-2")
	if !errors.Is(err, contracts.ErrInsufficientBalance) {
	...
	dup := &contracts.StripeWithdrawal{
		ID: "wd-pg-atomic-1", AccountID: "acct-pg-wdb", StripeAccountID: "acct_pgwdb",
		AmountMicroUSD: 1_000_000, NetMicroUSD: 1_000_000,
		Method: "standard", Status: "pending",
	}
	if err := s.CreateStripeWithdrawalWithDebit(dup, contracts.LedgerStripePayout, "stripe_withdraw:pg-dup"); err == nil {
