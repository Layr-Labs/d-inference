...
func seedLeaderboardFixture(t *testing.T) *memory.Store {
	t.Helper()
	s := memory.New(contracts.Config{})
	now := time.Now()

	// alice: inference work only.
	if err := s.RecordProviderEarning(&contracts.ProviderEarning{
	...
	if err := s.CreditWithdrawable("acct-bob", 2000, contracts.LedgerReferralReward, "ref-bob"); err != nil {
	...
	if err := s.CreditWithdrawable("acct-carol", 700, contracts.LedgerAdminReward, "ref-carol"); err != nil {
	...
	if err := s.Credit("acct-frank", 8000, contracts.LedgerAdminCredit, "ref-frank"); err != nil {
		t.Fatalf("credit frank admin_credit: %v", err)
	}

	// eve: admin_credit only — non-withdrawable consumer credit, not an earning.
	if err := s.Credit("acct-eve", 5000, contracts.LedgerAdminCredit, "ref-eve"); err != nil {
...
func findRow(rows []contracts.LeaderboardRow, accountID string) (contracts.LeaderboardRow, bool) {
	for _, r := range rows {
		if r.AccountID == accountID {
			return r, true
		}
	}
	return contracts.LeaderboardRow{}, false
}

func TestLeaderboardWorkRewardDifferentiation(t *testing.T) {
	s := seedLeaderboardFixture(t)
	rows := s.Leaderboard(contracts.LeaderboardEarnings, time.Time{}, 50)
...
func TestLeaderboardTokensAndJobsMetrics(t *testing.T) {
	...
	tokens := s.Leaderboard(contracts.LeaderboardTokens, time.Time{}, 50)
	...
	jobs := s.Leaderboard(contracts.LeaderboardJobs, time.Time{}, 50)
...
func rowID(rows []contracts.LeaderboardRow, i int) string {
	if i < 0 || i >= len(rows) {
		return "<none>"
	}
...
func TestLeaderboardLimitClamp(t *testing.T) {
	...
	if got := s.Leaderboard(contracts.LeaderboardEarnings, time.Time{}, 0); len(got) != 4 {
		t.Errorf("limit 0 -> %d rows, want 4", len(got))
	}
	if got := s.Leaderboard(contracts.LeaderboardEarnings, time.Time{}, 1000); len(got) != 4 {
		t.Errorf("limit 1000 -> %d rows, want 4", len(got))
	}
	// A real positive limit truncates after sorting.
	if got := s.Leaderboard(contracts.LeaderboardEarnings, time.Time{}, 2); len(got) != 2 {
...
func TestLeaderboardExcludesNonProviderRewards(t *testing.T) {
	...
	if err := s.RecordProviderEarning(&contracts.ProviderEarning{AccountID: "prov", AmountMicroUSD: 100, PromptTokens: 4, CompletionTokens: 6, CreatedAt: time.Now()}); err != nil {
		t.Fatalf("record prov earning: %v", err)
	}
	if err := s.CreditWithdrawable("prov", 50, contracts.LedgerAdminReward, "r1"); err != nil {
		t.Fatalf("credit prov reward: %v", err)
	}
	// A non-provider who only earned referral + admin rewards (never served).
	if err := s.CreditWithdrawable("consumer", 900, contracts.LedgerReferralReward, "r2"); err != nil {
		t.Fatalf("credit consumer referral: %v", err)
	}
	if err := s.CreditWithdrawable("consumer", 98, contracts.LedgerAdminReward, "r3"); err != nil {
...
func TestNetworkTotalsExcludesNonRewardLedgerTypes(t *testing.T) {
	s := memory.New(contracts.Config{})
	if err := s.Credit("acct-x", 1234, contracts.LedgerAdminCredit, "x"); err != nil {
		t.Fatalf("credit admin_credit: %v", err)
	}
	if err := s.Credit("acct-y", 5678, contracts.LedgerInviteCredit, "y"); err != nil {
	...
	if rows := s.Leaderboard(contracts.LeaderboardEarnings, time.Time{}, 50); len(rows) != 0 {
...
func TestIsRewardLedgerType(t *testing.T) {
	reward := []contracts.LedgerEntryType{contracts.LedgerReferralReward, contracts.LedgerAdminReward}
	for _, lt := range reward {
		if !contracts.IsRewardLedgerType(lt) {
			t.Errorf("IsRewardLedgerType(%q) = false, want true", lt)
		}
	}
	notReward := []contracts.LedgerEntryType{
		contracts.LedgerDeposit, contracts.LedgerCharge, contracts.LedgerPayout, contracts.LedgerPlatformFee, contracts.LedgerWithdrawal,
		contracts.LedgerStripeDeposit, contracts.LedgerStripePayout, contracts.LedgerInviteCredit, contracts.LedgerRefund,
		contracts.LedgerAdminCredit, contracts.LedgerMigration,
	}
	...
	if len(contracts.RewardLedgerTypes) != 2 {
		t.Errorf("RewardLedgerTypes has %d entries, want 2", len(contracts.RewardLedgerTypes))
	}
}
