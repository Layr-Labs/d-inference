...
func stripeReversalFixture(t *testing.T, s contracts.Store, id string) *contracts.StripeWithdrawal {
	t.Helper()
	wd := &contracts.StripeWithdrawal{ID: id, AccountID: "acct-" + id, StripeAccountID: "acct_stripe_" + id,
	...
	if err := s.CreateUser(&contracts.User{AccountID: wd.AccountID, PrivyUserID: "did:privy:" + wd.AccountID}); err != nil {
		t.Fatal(err)
	}
	...
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			for i := 0; i < 12; i++ {
				...
				go func() {
					defer wg.Done()
					if _, err := s.CreditWithdrawableOnce(wd.AccountID, wd.FeeMicroUSD, contracts.LedgerRefund, "stripe_withdraw_fee:"+wd.ID); err != nil {
						t.Error(err)
					}
				}()
			}
