...
type faultAttributionFixture struct {
	dispatch   *execution
	observer   *fixtureOutcomeObserver
...
func (f faultAttributionFixture) assertFinalFault(
	...
	outcomes := f.observer.requestOutcomes
	if len(outcomes) != 1 {
		t.Fatalf("request outcomes = %v, want one", outcomes)
	}
	if outcomes[0].class != orClassProvider5xx || outcomes[0].attr.Backend != wantBackend || outcomes[0].model != f.model {
		t.Fatalf("request outcome = %+v, want provider_5xx attributed to %s", outcomes[0], wantBackend)
	}
}
...
func TestSpeculativeFaultLoserAttributionOrdering(t *testing.T) {
	t.Run("on-time winner owns success", func(t *testing.T) {
		...
		outcomes := f.observer.requestOutcomes
		if len(outcomes) != 1 || outcomes[0].class != OrClassSuccess || outcomes[0].attr.Backend != registry.KVBackendContiguous || outcomes[0].model != f.model {
			t.Fatalf("winning-content outcome = %v, want contiguous success", outcomes)
		}
	})
