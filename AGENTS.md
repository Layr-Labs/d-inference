...
- An operator can cap the provider's total footprint with `[provider] memory_limit_gb` (`darkbloom memory limit <GB>`, `provider-swift/Sources/ProviderCore/Inference/MemoryLimit.swift`): the limit is folded into one effective reserve — `max(memory_reserve_gb, physical − memory_limit_gb)` — consumed by every `configReserveBytes` gate, so the effective cap is `min(0.90 × physical, physical − memory_reserve_gb, memory_limit_gb)`. The heartbeat reports `total_memory_gb = min(physical, limit)` so coordinator fit/admission respects the cap with no protocol change; registration `memory_gb` stays raw hardware.
