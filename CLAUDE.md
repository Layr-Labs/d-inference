...
tools/systemmap/      Deterministic Darkbloom system-map generator (own Go module, so go/packages
                      ...
                      add clusters, not schema. cmd/history re-runs the same extractor at every
                      commit that touched the service and writes one delta-encoded timeline, which
                      `-history` embeds as a slider over the service's lifespan (opt-in: minutes)
...
- Coordinator routes, `Server` state, SQL, and outbound hosts are published in the generated Darkbloom system map. The map itself is **never committed** — it is git-ignored, built by the CI `System Map` job for every PR (downloadable artifact) and published to Pages from master. What you must update when you change any of them is the curated half: node labels, clusters, namespaces, auth classes and prose in `docs/reference/api-map/overlay.json`. The **prose** part of that — a route's description and details, a node's label, its `depDocs` — is generated in CI instead (`.github/workflows/systemmap-enrich.yml` bot-commits `docs/reference/api-map/prose.json` to the PR branch), so leave it to the job unless you want to say something better by hand: hand-written prose in `overlay.json` always wins and is never regenerated. Generated entries are hashed against the derived facts they describe, so `-check` reports prose whose route has since changed auth class or state — do not hand-edit `prose.json` to silence that, run `make -C tools/systemmap enrich` (needs `ANTHROPIC_API_KEY`) or write the entry into `overlay.json`. Everything the graph *draws* — clusters, namespaces, auth classes, `deps.sqlDriver.assembled` — is still yours: a new route with no cluster or no auth class fails the gate no matter what the enrichment job does. Verify with `make -C tools/systemmap check` (renders the map, writes nothing, fails when source contains something the overlay does not explain) or `make -C tools/systemmap` to also write it locally for reading. `tools/systemmap` is a separate Go module, so `go test ./...` at the repo root does not cover it — use `make -C tools/systemmap test`. That target also *executes* the generated page: `render/webtest` loads `system-map.html` into jsdom and drives the explorer (filters, focus, fullscreen, table drawers, provenance views) and its geometry — every label is scored against where the page's own `sx`/`sy` put its subject, against every other label, and against the frame, at every zoom and after every pan, drag, filter and refit. `go test` points it at the five-route fixture, whose only crowded moment is the one test that shrinks the viewport to 420×320; two claims are out of its reach entirely and rest on CI's real-map run — that the label budget ever binds, and that a cluster name ever climbs its rung ladder. So the real 108-route map is what exercises the label pass properly, and CI runs every `*.test.mjs` over both. It needs `make -C tools/systemmap webdeps` once for jsdom, skips when node is absent, and CI (`SYSTEMMAP_WEBTEST=1`) turns that skip into a failure. The page's script is three files — `render/page.js` (data, filters, layout, drawing), `render/page.timeline.js` (the `TL` replay and the slider), `render/page.boot.js` (`draw`, the control wiring, the bootstrap) — concatenated in that order into one `<script>` by `render.go`'s `pageParts`; the order is a correctness contract, not a preference, because `TL` is a `const` the bootstrap's first `draw()` reads. A change to any of them or to `page.html` must keep the suite green — `make -C tools/systemmap webtest` runs it alone against a freshly built map. The **history slider** is a second, opt-in artifact: `make -C tools/systemmap history` walks every commit that touched the service (one full type-check each — minutes), writes `docs/reference/api-map/history.json`, and renders the map with `-history` so the page can be stepped through the coordinator's whole lifespan; `make -C tools/systemmap timeline` writes the walk alone, and `HISTORY_ARGS="-every 25"` samples it. Embedding is the flag and never the file's presence — a map built without `-history` is the map of one commit whatever is lying in the output directory. The slider is inert until touched (the drawn-topology fingerprint of a `-history` page equals a plain one's, and returning to Now restores it), which `render/webtest/timeline.test.mjs` asserts; the Go side of the walk — delta encoding replayed both ways, the three module eras, overlay retargeting, and the wire-mode rule that must match the page's `epMode` — is `history/timeline_test.go`.
- Store queries must be readable by the system map, because statement text is the only place the map's table edges come from. Two rules, both enforced by `make -C tools/systemmap check` ("Statement text the extractor cannot read"): a function calling pgx `Query`/`QueryRow`/`Exec`/`Queue` must contain the statement it runs (not receive it as a parameter or build it in a `strings.Builder`), and wherever text names a table after an upper-case `FROM`/`JOIN`/`INTO`/`UPDATE` the name must be literal and the text around it must be a whole statement. So `` `SELECT ` + userColumns + ` FROM users` `` is fine (`go/types` folds it into one statement), `q += " LIMIT $2"`, `q += " FOR UPDATE"`, `FOR NO KEY UPDATE`, `FOR UPDATE SKIP LOCKED` and `ON CONFLICT ... DO UPDATE ` + `"SET x = $1"` are fine (those clauses are blanked before the scan, because `UPDATE` there is a lock or a conflict action and never precedes a table, and the other fragments name none), and `q += " UNION SELECT id FROM usage"`, `q += " JOIN " + other` (ends *after* a keyword, so the table never reaches the extractor) or `fmt.Sprintf("... FROM %s", t)` is drift — those hide a table while every count still balances. Every keyword in a literal is checked and every table it names is remembered, so both `fmt.Sprintf("... FROM models m JOIN %s u ...")` and a `JOIN` added beside an already-declared table are drift, and a literal that names a table *and* trails off at a keyword is held to both. A `WITH` clause shadows a table of the same name, scoped to the bare string variable the query is assembled into or, failing that, to the expression or statement the text appears in (so `base, tail := q1, q2` and `return base, tail` are two queries — unless the walk sees them concatenated, `Exec(ctx, base+tail)`, which puts them back together, because a CTE that really does cover a read must not draw an edge the statement does not have); a variable reused for several queries holds one set of names per query, bounded by the binding *statement* (`q :=`/`q =` starts a query whatever text it carries, `q +=` continues it, and `q = "WITH ..." + q` continues it too because it reads the local back — until that query itself goes to the database as the *statement* argument of a driver call, or a copy of it does (`r := q`, and every source of `r := q + tail`), or it is copied somewhere no name can be followed at all (`r, n := q, 1`, `h.q = q`, `qs[0] = q` — settled where the copy is made, since the dispatch may happen out of sight), or a loop that dispatches repeats the prepend on a local bound outside it, whether the prepend is in the body or the `for` clause's post statement) rather than by the literal — so neither a middle literal that parses on its own, the shape every long query in `postgres.go` has, nor a mid-query `q += "SELECT ... FROM models"` orphans the CTE names above it — and a fragment is settled against the set in force where it was read. So a CTE in one query does not silence a same-named table in another; the shapes where it still can, none of them in the store, are two statements in one literal (split by `;`), control flow jumping over the binding a `WITH` clause is in, and one local bound twice by the same multi-value statement, which has no generation to tell the two values apart. In the other direction, two halves of one statement joined where the walk cannot see it — `strings.Join(parts, " ")`, or a helper that returns a base and a tail for its caller to concatenate — leave the CTE drawn as the table of the same name, with nothing reported; keep such a statement in one expression. A fragment whose `WITH` clause is out of reach — assigned to a different variable, or assembled through `qs[0] +=`, `s.q +=` or `*p +=`, where one variable stands for several queries — is reported even though the SQL is legal; inline the `WITH` literal into the same assignment, or declare the tables. The scan reads SQL rather than parsing it, so it says nothing about lower-case keywords. A query that genuinely cannot be one expression (the earnings CTE chains in `postgres.go`, where a conditional `WHERE` is spliced mid-statement) declares its tables in `deps.sqlDriver.assembled` in `docs/reference/api-map/overlay.json`; the declaration is checked against the schema, every table the text names but the entry omits is still reported, and the entry is itself reported once the function stops needing it or is no longer reachable from a route.
