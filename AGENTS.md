...
- The activation reserve inside that cap is **flat**: 3 GiB for every model, every attention posture, every batch. `coordinator/registry/servability.go` hard-codes the same figure as `servabilityActivationFloorGB` so `coldTokenBudgetEstimate` can predict a not-yet-loaded slot's post-load token budget — a cold slot sends no heartbeat, so there is nothing to read instead. The two numbers are equal only because someone moves both; retuning the reserve on one side alone silently desyncs admission. That has happened once: the coordinator briefly charged a per-token score-tensor surcharge the provider never held back, making it strictly tighter than the gate it mirrors and 429ing prompts the fleet could serve. Do not re-introduce a per-model or per-shape reserve on either side without the other.
