...
- The provider's memory model is `UnifiedMemoryCap` (`provider-swift/Sources/ProviderCore/Inference/UnifiedMemoryCap.swift`): hard cap = 0.90 × physical RAM (always leaving ≥ 2 GiB for the OS; `DARKBLOOM_MEM_CAP_FRACTION` override). The model-load gate requires resident weights + incoming weights + the resolved activation reserve + 1 GiB minimum KV ≤ the cap, and a post-load guard unloads a freshly-loaded model whose measured live KV headroom is below that minimum. `DARKBLOOM_ACTIVATION_RESERVE_GB` is **raise-only** relative to the selected model profile. `BackendCapacity.freeForLoadGb` uses the largest advertised profile so old coordinators remain conservative; current coordinators add back only the reported delta for a lower-reserve candidate.
- `ModelActivationPolicy` selects activation memory from measured execution profiles, not a formula or model name. The exact measured GPT-OSS 20B config/architecture uses 3 GiB; unknown, incomplete, modified, and vision profiles use the conservative 5.5 GiB reserve. The process holds the maximum reserve across resident and loading models because MLX evaluations serialize on the process-global evaluation lock. Providers report the resolved byte count as `ModelInfo.activation_reserve_bytes`; `coordinator/registry/servability.go` consumes that exact value for cold-budget prediction and uses version-gated 3/5.5 GiB fallbacks only when old providers omit it. Never derive a second coordinator-side model estimate: profile selection, provider gates, wire reporting, and coordinator consumption must move together.
