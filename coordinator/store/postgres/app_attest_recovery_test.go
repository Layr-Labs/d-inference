...
func TestAppAttestRecoveryPreservesFailuresAndDoesNotAuthorizePostgres(t *testing.T) {
	...
	now := time.Now().UTC()
	evidence := contracts.AppAttestEvidence{ID: "enrollment", SessionID: "session", KeyID: "key", ReceivedAt: now.Add(-10 * time.Minute), Action: "attestation", Proof: []byte{1}, Context: json.RawMessage(`{}`)}
	if err := s.BeginAppAttestEvidence(ctx, evidence); err != nil {
		t.Fatal(err)
	}
	key := contracts.AppAttestShadowKey{KeyID: "key", Owner: "owner", AccountID: "account", PublicKey: []byte{2}}
	receipt := contracts.AppAttestReceipt{ID: "old", KeyID: "key", EvidenceID: evidence.ID, ReceivedAt: now, Outcome: "receipt_creation_time", Body: []byte{3}, Context: json.RawMessage(`{}`), Details: json.RawMessage(`{}`), NextAt: now}
	if _, err := s.CompleteAppAttestEvidence(ctx, evidence.ID, contracts.AppAttestDecision{Outcome: "verified", Key: &key, Receipt: &receipt}); err != nil {
		t.Fatal(err)
	}
	...
	counter := uint32(99)
	if outcome, err := s.CompleteAppAttestEvidence(ctx, pending.ID, contracts.AppAttestDecision{Outcome: "verified", Counter: &counter, KeyID: "key", Owner: "owner"}); err != nil || outcome != "interrupted" {
...
func TestAppAttestRevocationIsAccountScopedAndDurable(t *testing.T) {
	for name, backend := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			keys, ok := contracts.As[contracts.AppAttestShadowStore](backend)
			if !ok {
				t.Fatal("key store hidden")
			}
			readiness, ok := contracts.As[contracts.AppAttestReadinessStore](backend)
			if !ok {
				t.Fatal("revocation store hidden")
			}
			ctx := context.Background()
			_, err := keys.InsertAppAttestShadowKey(ctx, contracts.AppAttestShadowKey{KeyID: "revoked", Owner: "owner", AccountID: "account"})
			if err != nil {
				t.Fatal(err)
			}
...
func TestAppAttestMachineIdentitySurvivesWithoutLegacyProof(t *testing.T) {
	for name, backend := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			s, ok := contracts.As[contracts.MachineInventoryStore](backend)
			if !ok {
				t.Fatal("inventory missing")
			}
			ctx := context.Background()
			now := time.Now()
			initial := contracts.MachineObservation{SessionID: "first", AccountID: "account", At: now, SEKey: "legacy", VerifiedSerial: "hardware", VerifiedAppAttestKey: "verified-app-key"}
			first, err := s.ObserveMachine(ctx, initial)
			if err != nil {
				t.Fatal(err)
			}
			next := contracts.MachineObservation{SessionID: "next", AccountID: "account", At: now.Add(time.Second)}
