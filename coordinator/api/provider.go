...
//   - hardware: live device trust granted through verified MDM evidence;
//     Apple MDA proof is tracked separately
...
func (s *Server) providerReadLoop(ctx context.Context, conn *websocket.Conn, providerID string, r *http.Request) {
	...
	challenges := s.newProviderChallengeVerifier().NewSession()
	...
	for {
		...
		case protocol.TypeRegister:
			...
			if err := s.newProviderVerifier().VerifyRegistration(loopCtx, providerID, provider, regMsg); err != nil {
			...
			saferun.Go(s.logger, "challengeLoop", func() {
				challenges.Run(loopCtx, providerID, provider)
			})
		...
		case protocol.TypeAttestationResponse:
			...
			challenges.Deliver(providerID, provider, respMsg)
