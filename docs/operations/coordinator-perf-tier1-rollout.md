...
| Capacity accept off the first-byte path | `coordinator/api/dispatch.go` (`commitFirstContent`); `coordinator/registry/fault_capacity.go` (`RecordCapacityAcceptObserved` → `CapacityAccept.Apply`) |
