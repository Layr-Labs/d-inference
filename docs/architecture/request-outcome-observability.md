...
| `d_inference.inference.cancel_sent` | `cause`, `model` | `api/cancel_lifecycle.go` `sendRecordedCancel` / `noteStrayChunk` | One count per Cancel frame handed to the provider writer for an ABANDONED attempt — `cause` ∈ {`first_chunk_timeout`, `hedge_loser`, `client_gone_pre`, `client_gone_post`, `stream_timeout`, `overflow`, `late_content`, `stray_chunk`}. A clean `inference_complete` or a provider `inference_error` terminal never produces one (the terminal handler already removed the pending record); before this rule the post-commit defer cancelled after every served request. The request profile's `cancel_sent` stamp is taken at the same site, so it too marks only real sends. When the control lane is full the frame is handed to the bounded blocking fallback instead; the count and stamp are taken at that hand-off, and a fallback that later drops the frame shows up in `provider.cancel_dropped{reason:fallback_timeout\|fallback_writer_stopped}` and is fed back to the zombie tracker as a failed send. |
| `d_inference.inference.cancel_send_failed` | `reason` ∈ {`queue_full`, `writer_stopped`, `ctx`, `other`} | `api/consumer.go` `sendProviderCancel` | The non-blocking control-lane enqueue of a cancel failed. `queue_full` then takes the bounded blocking fallback (`provider.ws.control_fallbacks` counts deliveries, `provider.cancel_dropped{reason:fallback_cap\|fallback_timeout\|fallback_writer_stopped\|oversized_id}` the terminal drops — only `fallback_timeout` is a control-lane capacity signal; `fallback_writer_stopped` is connection teardown and may overlap with `provider.ws.dropped_on_close` when the frame had already landed on the lane); `writer_stopped` is the disconnect race. |
| `d_inference.inference.cancel_to_terminal_ms` | `terminal` ∈ {`complete`, `error`, `stray_chunk`}, `model`, `cause` | `api/cancel_lifecycle.go` `resolveCancelledTerminal` / `emitExpiredCancelEntries` | Latency from the FIRST cancel for an abandoned request to the provider's terminal for it (`inference_complete` / `inference_error` matched against the zombie tracker instead of being logged as unknown), or to its last stray chunk when no terminal arrived within 5 min. |
| `d_inference.inference.cancelled_terminal` | `outcome` ∈ {`complete_partial`, `error_cancelled`, `error_other`}, `cause` | same | How the provider answered a cancel: partial completion, a 499/`cancelled` error, or another error. |
| `d_inference.inference.cancel_unresolved` | `cause`, `model` | `api/cancel_lifecycle.go` `emitExpiredCancelEntries` | A cancel that produced neither a terminal nor a stray chunk within 5 min: the provider honored it silently, disconnected, or never saw it. |
| `d_inference.inference.zombie_stream_cancel` | `resend_index` ∈ {`0`..`4`} | `api/cancel_lifecycle.go` `noteStrayChunk` | A cancel (re-)sent because a chunk arrived for an abandoned request: `0` = first cancel triggered by a stray chunk (no abandon path recorded one), `1`..`3` = the escalating +1 s / +3 s / +10 s re-sends after the first cancel, `4` = the steady 30 s regime. Replaces the flat 10 s throttle; the per-chunk "chunk for unknown request" Warn is rate-limited to one line per provider per 10 s with the suppressed count. |
| `d_inference.inference.chunk_overflow_grace` | `outcome` ∈ {`delivered`, `expired`, `would_skip`} | `api/provider.go` `handleChunk` overflow fallback | One count per 250 ms overflow grace window the relay spent on a consumer that was a full chunk buffer (256) behind: `delivered` = the first window on a request freed a slot in time, `expired` = the window ran out and the request was failed (499 + cancel, `inference.chunk_overflow_abort`), `would_skip` = a REPEAT window on the same request that delivered — the requests a one-window-per-request policy would fail that survive today. Each window stalls the provider's single read goroutine, so sibling streams' next chunks are stamped after the dwell and checked late against their first-content deadlines; the policy is enforced only once this series shows `would_skip` is rare or confined to stuck consumers. |
