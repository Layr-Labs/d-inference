...
func (s *PostgresStore) migrate(ctx context.Context) error {
	migrations := []string{
		...
		// Durable inference request, attempt, terminal, delivery, and financial
		// effect journal. These tables are correctness state, not best-effort
		// routing telemetry.
		`CREATE TABLE IF NOT EXISTS request_settlements (
			...
			openrouter_exact BOOLEAN NOT NULL DEFAULT FALSE,
			state TEXT NOT NULL,
			funding_kind TEXT NOT NULL,
			base_reserved_micro_usd BIGINT NOT NULL,
			reserved_micro_usd BIGINT NOT NULL,
			request_budget_ms BIGINT NOT NULL DEFAULT 0,
			winner_attempt_id TEXT NOT NULL DEFAULT '',
			winner_epoch BIGINT NOT NULL DEFAULT 0,
			winner_ingress_sequence BIGINT NOT NULL DEFAULT 0,
			fence_cause TEXT NOT NULL DEFAULT '',
			fence_version INTEGER NOT NULL DEFAULT 0,
			fence_sequence BIGINT NOT NULL DEFAULT 0,
			last_ingress_sequence BIGINT NOT NULL DEFAULT 0,
			transport_state TEXT NOT NULL DEFAULT 'T0',
			protocol_state TEXT NOT NULL DEFAULT 'V0',
			semantic_state TEXT NOT NULL DEFAULT 'C0',
			written_ingress_sequence BIGINT NOT NULL DEFAULT 0,
			written_chunk_sequence BIGINT NOT NULL DEFAULT 0,
			written_completion_tokens INTEGER NOT NULL DEFAULT 0,
			written_commitment TEXT NOT NULL DEFAULT '',
			delivery_state TEXT NOT NULL DEFAULT 'none',
			client_snapshot_hash TEXT NOT NULL DEFAULT '',
			client_snapshot BYTEA,
			settlement_snapshot_hash TEXT NOT NULL DEFAULT '',
			settlement_snapshot BYTEA,
			effects_sealed BOOLEAN NOT NULL DEFAULT FALSE,
			started_at TIMESTAMPTZ NOT NULL,
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			CHECK (base_reserved_micro_usd >= 0),
			CHECK (reserved_micro_usd >= base_reserved_micro_usd),
			CHECK (winner_ingress_sequence >= 0 AND fence_sequence >= 0 AND last_ingress_sequence >= 0),
			CHECK (written_ingress_sequence >= 0 AND written_chunk_sequence >= 0 AND written_completion_tokens >= 0)
		)`,
		`CREATE INDEX IF NOT EXISTS idx_request_settlements_recovery ON request_settlements(state, updated_at) WHERE state <> 'settled'`,
		`CREATE INDEX IF NOT EXISTS idx_request_settlements_key_holds ON request_settlements(key_id, started_at) WHERE key_id <> '' AND state <> 'settled'`,
		`CREATE INDEX IF NOT EXISTS idx_request_settlements_service_holds ON request_settlements(consumer_account_id) WHERE funding_kind = 'service_hold' AND state <> 'settled'`,
		`CREATE TABLE IF NOT EXISTS request_attempts (
			client_request_id TEXT NOT NULL REFERENCES request_settlements(client_request_id) ON DELETE CASCADE,
			...
			admission_state TEXT NOT NULL DEFAULT 'pre_accept',
			top_up_micro_usd BIGINT NOT NULL DEFAULT 0,
			...
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			PRIMARY KEY (client_request_id, provider_request_id),
			UNIQUE (provider_request_id),
			UNIQUE (client_request_id, attempt, role),
			CHECK (top_up_micro_usd >= 0)
		)`,
		`CREATE INDEX IF NOT EXISTS idx_request_attempts_provider ON request_attempts(provider_id, updated_at)`,
		`CREATE TABLE IF NOT EXISTS attempt_terminals (
			...
			last_chunk_sequence BIGINT NOT NULL DEFAULT 0,
			last_completion_tokens INTEGER NOT NULL DEFAULT 0,
			termination_reason TEXT NOT NULL DEFAULT '',
			request_fence_version INTEGER NOT NULL DEFAULT 0,
			request_fence_sequence BIGINT NOT NULL DEFAULT 0,
			cancel_reason TEXT NOT NULL DEFAULT '',
			...
			health_outcome TEXT NOT NULL DEFAULT '',
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			PRIMARY KEY (client_request_id, provider_request_id),
			FOREIGN KEY (client_request_id, provider_request_id)
				REFERENCES request_attempts(client_request_id, provider_request_id) ON DELETE CASCADE,
			CHECK (ingress_sequence > 0),
			CHECK (prompt_tokens >= 0 AND completion_tokens >= 0 AND reasoning_tokens >= 0),
			CHECK (request_fence_sequence >= 0 AND last_chunk_sequence >= 0)
		)`,
		`CREATE TABLE IF NOT EXISTS settlement_effects (
			...
			payload JSONB,
			sealed_plan BOOLEAN NOT NULL DEFAULT FALSE,
			state TEXT NOT NULL,
			...
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			CHECK (amount_micro_usd >= 0 AND apply_attempts >= 0)
		)`,
		`CREATE INDEX IF NOT EXISTS idx_settlement_effects_request ON settlement_effects(client_request_id, created_at)`,
		`CREATE INDEX IF NOT EXISTS idx_settlement_effects_pending ON settlement_effects(state, updated_at) WHERE state IN ('pending', 'applying', 'indeterminate')`,
		`CREATE INDEX IF NOT EXISTS idx_settlement_effects_claim ON settlement_effects(state, claim_expires_at, created_at) WHERE state IN ('pending', 'applying', 'indeterminate')`,
