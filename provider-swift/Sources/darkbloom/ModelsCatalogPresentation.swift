...
enum ModelsCatalogPresentation {
    static func otherLocalLines(localModels: [ModelInfo], catalogIDs: Set<String>) -> [String] {
        let localOnly = localModels.filter { !catalogIDs.contains($0.id) }
        guard !localOnly.isEmpty else { return [] }

        var lines = ["", "Other local models (not in displayed catalog)", ""]
        lines += localOnly.map {
            "  \($0.id)  \(String(format: "%.1f", $0.estimatedMemoryGb)) GB"
        }
        lines += [
            "",
            "  These IDs are absent from the catalog results shown above.",
            "  With --type, other catalog model types can also appear here.",
