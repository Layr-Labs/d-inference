...
func reserveForCalibration(t *testing.T, srv *Controller, model, requestID string) *registry.PendingRequest {
	t.Helper()
	...
	p, decision := srv.deps.Registry().ReserveProviderEx(model, pr)
	if p == nil {
		t.Fatalf("reserve failed for %s: %+v", requestID, decision)
	}
	p.RemovePending(requestID)
	srv.deps.Registry().SetProviderIdle(p.ID)
	return pr
}
