...
const SUGGESTED_PROMPT_KEYS = ["quantum", "python", "frameworks", "zk"] as const;
...
export default function ChatPage() {
    ...
    async (content: string) => {
      ...
      try {
        ...
        if ((err as Error).name !== "AbortError") {
          ...
          updateMessage(chatId!, assistantId, {
            content: t("errors.connection", { error: msg }),
            streaming: false,
            error: true,
          });
          addToast(t("errors.connection", { error: msg }));
        }
        setIsStreaming(false);
      }
    },
  ...
  const handleRetry = useCallback(
      ...
      streamChat(
        ...
        selectedModel,
        {
          ...
          onError: (error) => {
            ...
            updateMessage(activeChat.id, errorMsgId, {
              content: t("errors.stream", { error }), streaming: false, error: true,
            });
            addToast(error);
            setIsStreaming(false);
          },
        ...
        if ((err as Error).name !== "AbortError") {
          ...
          updateMessage(activeChat.id, errorMsgId, {
            content: t("errors.connection", { error: (err as Error).message }),
    ...
    [activeChat, isStreaming, authenticated, apiKeyReady, selectedModel, updateMessage, appendToMessage, appendToThinking, addToast, t]
  );
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center max-w-lg px-6">
            ...
            <p className="text-base text-text-secondary mb-8 leading-relaxed">
              {t("guest.title")}
              <br />
              <span className="text-text-tertiary">{t("guest.subtitle")}</span>
            </p>
              ...
              {SUGGESTED_PROMPT_KEYS.map((key) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    handleSend(t(`suggestions.${key}.prompt`));
                  }}
