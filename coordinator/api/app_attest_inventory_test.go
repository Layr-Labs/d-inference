...
func TestMachineInventoryRequiresSEBoundMDAForSerialAlias(t *testing.T) {
	...
	p.MDAVerified = true
	st := &captureInventory{}
	x := &machineInventorySession{s: &Server{}, p: p, store: st, observation: store.MachineObservation{SessionID: p.ID, AccountID: "authenticated"}}
	x.capture(false)
...
func TestAppAttestEnrollmentRecoveryUsesOriginalTranscriptAndOwner(t *testing.T) {
	...
	now := time.Now()
	x := &appAttestShadowSession{s: &Server{store: st, appAttestShadow: AppAttestShadowConfig{AppID: "TEST.app", Environment: "production"}}, owner: "owner", account: "account", id: "new", challenge: "new challenge", publicKey: "new endpoint", protocolVersion: 2, key: &store.AppAttestShadowKey{KeyID: "key"}}
...
func TestAppAttestArchiveFailureDoesNotAdvanceCounter(t *testing.T) {
	...
	p := newCodeAttestProvider("endpoint", "se")
	x := &appAttestShadowSession{s: &Server{}, provider: p, store: st, archive: failedEvidenceArchive{}, key: key, expected: "assertion"}
