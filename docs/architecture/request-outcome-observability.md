...
| 4. Provider aggregate counters | Add provider counters for completed, provider_error, timeout, disconnect, client_cancel_pre_commit, client_cancel_after_commit, no_terminal_after_cancel, and dropped_unknown_terminal. | Do not fold client cancellations into provider failure reputation. Export counters through admin/provider stats and Datadog tags for fleet health dashboards. **Partially implemented (DAR-332):** the completed-after-disconnect split (`inference.partial_success`, `inference.client_cancellations{phase=after_commit}`, `inference.no_terminal_after_cancel`) is emitted — see "Implemented (DAR-332)" below. |
...
| `d_inference.inference.client_cancellations` | `model`, `phase` | `phase=after_commit`: `api/settlement.go` `holdForSettlement` (the single post-commit-disconnect chokepoint) | Broad client-cancellation counter. `after_commit` counts every parked post-commit disconnect regardless of how it later settles. The `before_first_token` phase value is reserved for the pre-commit client-gone paths in `api/dispatch.go` / `api/consumer.go` (owned by the DAR-330 `routing.client_gone{phase}` work) so the two tickets do not edit the same files; together the two phases make the metric splittable. |
