...
func (d *execution) noteProviderDispatched() {
...
func (d *execution) exhaustionAttemptCount() int {
...
// execution. timing is caller-supplied because the speculative backup
...
func (d *execution) dispatchProviderWith(
...
func (d *execution) dispatchFromPlanMachinery(
	timing *registry.RequestTiming,
	exclude map[string]struct{},
	backupOf string,
	recordRoute routeDecisionRecorder,
	...
	provider, pr, decision, _, lastErr, lastErrCode = d.dispatchProviderWith(
		func(pending *registry.PendingRequest, excludeIDs []string) (*registry.Provider, registry.RoutingDecision, *registry.DispatchPlan) {
			p, dec, _ := d.s.deps.Registry().ReserveNextFromPlan(pending, plan, excludeIDs...)
			reserved = p != nil
			return p, dec, nil
		},
	...
	// 2) The single full re-scan refresh. Latched on execution (not just
	...
	provider, pr, decision, fresh, lastErr, lastErrCode = d.dispatchProviderWith(
		func(pending *registry.PendingRequest, excludeIDs []string) (*registry.Provider, registry.RoutingDecision, *registry.DispatchPlan) {
			p, dec, freshPlan, performed := d.s.deps.Registry().RefreshDispatchPlan(pending, plan, excludeIDs...)
			if !performed {
				return nil, dec, nil
			}
			reserved = p != nil
...
func (d *execution) maybeProbePlanCandidates() {
	...
	outcomes := d.s.deps.Registry().ProbePlanCandidates(d.plan, registry.CapacityProbeShape{
		Model:             d.model,
	...
	saferun.Go(d.s.deps.Logger(), "api.collectCapacityQuotes", func() {
...
func (d *execution) tryAcquireBackupHedge(primaryID string) (hedgeVerdict, bool) {
	g := d.s.hedgeGov
	if g == nil {
		return hedgeAllow, false
	}
	exclude := append(d.excludedProviderIDs(), primaryID)
	idleAlt, queueDepth, fleetIdle, capacitySignals := d.s.deps.Registry().HedgeGovernorSnapshot(d.model, d.pr, exclude...)
	if !capacitySignals {
