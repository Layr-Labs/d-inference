...
The lowest-cost candidate wins. Candidates within `nearTieCostWindowMs` (`3_000` ms) of the best are considered tied (`coordinator/registry/scheduler.go`, `selectRoutingCandidate`). Ties are broken by lowest `effectiveQueue`, then lowest `totalPending`, then exact-cache benefit. Remaining equals use structural best-fit — smallest sufficient token-budget ceiling, or smallest RAM class when every ceiling is unknown — before uniform random choice (`coordinator/registry/capacity_fit.go`). This keeps small requests on constrained machines only inside the existing latency/load equivalence window and preserves large-request headroom.
