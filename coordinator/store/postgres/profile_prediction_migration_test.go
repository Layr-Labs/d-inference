...
func TestPostgresPredictionColumnsUpgradeKeepsUnknownHistory(t *testing.T) {
	...
	old := testfixture.FullProfile("before-prediction-telemetry", 0, time.Now())
	old.AdmissionMode = ""
	...
	fresh := testfixture.FullProfile("after-prediction-telemetry", 0, time.Now())
	if err := s.RecordRequestProfiles([]*contracts.RequestProfileRecord{fresh}); err != nil {
		t.Fatal(err)
	}
	rows = s.RequestProfilesSince(time.Time{})
