...
// Per-pass dominance skip for the queue drain
// (drainQueuedRequestsForModelsWithReason).
//
...
// identical "no candidate" verdict from an identical full fleet scan (one
// ReserveProviderEx over every advertiser of the model). Those requests are
// requeued without a scan. Anything not provably dominated — a different
// constraint shape, a strictly smaller request, a looser TTFT ceiling, a
// cache plan that could shorten prefill — is scanned exactly as before, so a
// small request behind a large rejected one is still admitted in the same
// pass.
//
// Concurrency note: no lock spans the pass, so "the fleet state does not
// change" is a statement about THIS pass's own effects, not about other
// goroutines. A capacity-freeing trigger (SetProviderIdle, challenge, load)
// that runs while this pass holds waiters in `skipped` drains what is left in
// the queue itself; if it finds the queue momentarily empty, that capacity is
// picked up by the next trigger or, for heartbeats, the trailing pass the
// suppressor arms (queue_drain_suppress.go). Master recovered that window at
// the next waiter's scan inside the same pass; the ported design recovers it
// at the next trigger (heartbeats arrive every few ms fleet-wide).
