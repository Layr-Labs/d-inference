    ...
    static func prefixReuse(
        ...
        if let weakest = exercised.min() {
            ...
            detail += ". Adoption was token- and terminal-exact against each arm's OWN "
                ...
                + "PrefixCacheV2, so a PASS says nothing about the SSD tier, which shares "
                + "the restore and frozen-replay machinery but stages its bytes "
                + "differently. And do not invert that into an attribution: the tier the "
                + "DIVERGING adoption used was never pinned either (the e2e aborted on the "
                + "equality assertion one line before its SSD-hit assertion), so neither "
                + "tier is exonerated by a green run here"
        } else {
