...
export function ModelDemandPanel({ refreshToken, catalogData }: { refreshToken: string | null; catalogData: CatalogDataSummary | null }) {
  ...
  if (loading) content = <p role="status" className="py-12 text-sm text-text-secondary">Loading model demand…</p>;
          ...
          <p className="mt-2 text-xs leading-5 text-text-tertiary">Recording is best-effort. Counts and percentages describe published hourly cohorts only, not all requests in the window or guaranteed network-wide coverage.</p>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3 text-xs text-text-tertiary">
            ...
            <label className="flex items-center gap-2 sm:ml-auto">Sort by<select aria-label="Sort model demand" value={sort} onChange={event => setSort(event.target.value)} className="min-h-10 rounded-lg border border-border-dim bg-bg-white px-3 text-text-secondary"><option value="requests">Published requests</option><option value="capacity">Published capacity rejections</option></select></label>
          </div>
          {models.length ? <>
            <div className={`mt-6 hidden items-end gap-5 text-xs text-text-tertiary lg:grid ${DEMAND_COLUMNS}`} aria-hidden="true"><span>Model</span><span>Published requests · shared scale</span><span className="text-right">Completed</span><span className="text-right">Capacity rejected</span><span className="text-right">Other</span><span /></div>
          ...
          <details className="mt-5 text-xs leading-5 text-text-tertiary">
            ...
            <p className="mt-2 max-w-4xl">Counts and percentages cover only published hourly cohorts of recorded public requests that reached routing admission after account and request checks. Each incoming HTTP request counts once; internal retries do not add requests. Client retries count separately. Private, owner-preferred, and machine-restricted requests are excluded, along with validation and account-limit rejections. Admin-key traffic is excluded. Authenticated load tests using ordinary public keys cannot be distinguished from other traffic.</p>
            <p className="mt-2 max-w-4xl">For every range, a model&apos;s UTC clock hour needs at least 20 recorded requests from 3 distinct consumer accounts to contribute to any count. Wider display intervals sum only eligible hours, so they may cover only some constituent hours. Model totals equal the published interval counts; models without eligible hours are omitted. Recording is best-effort: missing observations are not estimated, and percentages are not full-window or network-wide success rates. A gateway can represent many users under one account. No suppressed totals or account identifiers are published. These thresholds are not a differential-privacy guarantee.</p>
  ...
  return (
    <section aria-labelledby="model-demand-title" className="min-w-0 border-t border-border-dim pt-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><h2 id="model-demand-title" className="text-xl font-semibold tracking-tight text-text-primary">Model demand &amp; fulfillment</h2><p className="mt-2 text-sm leading-6 text-text-tertiary">Published request observations and outcomes from privacy-eligible hourly cohorts.</p></div>
