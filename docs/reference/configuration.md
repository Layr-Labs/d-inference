...
> Last updated: 2026-09-18 · commit `8488a9e2e`
...
| `EIGENINFERENCE_NEAR_TIE_WINDOW_FRACTION` | float 0–1 (out of range is rejected, not clamped) | `0` (band off; absolute window only) | `coordinator/cmd/coordinator/main.go`; `coordinator/registry/near_tie_window.go` (`ValidateNearTieWindowFraction`, `SetNearTieCostWindowFraction`) | Opens a near-tie band proportional to the cheapest candidate's own work term (`thisReqMs`), floored at `nearTieCostWindowMs`. Because the absolute window is fixed while the work term grows with `max_tokens`, load spreading narrows to the fastest hardware on long requests; this restores it proportionally. Candidates carrying a health, capacity-rate or cold-state penalty are never admitted by the band. `0` keeps selection byte-for-byte unchanged. See [`../architecture/routing.md`](../architecture/routing.md). |
