...
JSON metadata refuses duplicate keys, nonstandard constants, floating-point literals that overflow to infinity, and integer conversions exceeding the interpreter's digit limit. These metadata refusals are separate from native tensor NaN/Inf bit patterns, which remain available for the diagnostic counts described above.
