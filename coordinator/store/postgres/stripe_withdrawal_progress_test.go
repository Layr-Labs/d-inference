...
func TestStripeWithdrawalProgressCompareAndSwap(t *testing.T) {
	for backend, s := range storeBackends(t) {
		t.Run(backend, func(t *testing.T) {
			...
			if err := s.CreateUser(&contracts.User{AccountID: account, PrivyUserID: "did:privy:progress-cas"}); err != nil {
				t.Fatal(err)
			}
			mutations := []func(*contracts.StripeWithdrawal){
				func(w *contracts.StripeWithdrawal) { w.TransferID = "tr-new" }, func(w *contracts.StripeWithdrawal) { w.PayoutID = "po-new" },
				func(w *contracts.StripeWithdrawal) { w.SweepPayoutID = "sweep-new" }, func(w *contracts.StripeWithdrawal) { w.Status = "paid" },
				func(w *contracts.StripeWithdrawal) { w.FailureReason = "new reason" }, func(w *contracts.StripeWithdrawal) { w.Refunded = true }, func(w *contracts.StripeWithdrawal) { w.FeeRefunded = true },
			}
			for i, change := range mutations {
				t.Run(fmt.Sprint(i), func(t *testing.T) {
					row := &contracts.StripeWithdrawal{ID: fmt.Sprintf("progress-cas-%d", i), AccountID: account, StripeAccountID: "acct_progress", AmountMicroUSD: 5_000_000, NetMicroUSD: 4_500_000, FeeMicroUSD: 500_000, Method: "instant", Status: "transferred", TransferID: fmt.Sprintf("tr_old_%d", i), PayoutID: fmt.Sprintf("po_old_%d", i)}
			...
			if ok, err := s.CompareAndSwapStripeWithdrawal(&contracts.StripeWithdrawal{ID: "missing"}, &contracts.StripeWithdrawal{ID: "missing"}); err != nil || ok {
				t.Fatalf("missing row=%t error=%v", ok, err)
			}
			if ok, err := s.CompareAndSwapStripeWithdrawal(nil, &contracts.StripeWithdrawal{ID: "missing"}); err == nil || ok {
...
func TestStripeWithdrawalProgressConcurrentWriters(t *testing.T) {
	for backend, s := range storeBackends(t) {
		t.Run(backend, func(t *testing.T) {
			...
			if err := s.CreateUser(&contracts.User{AccountID: account, PrivyUserID: "did:privy:progress-race"}); err != nil {
				t.Fatal(err)
			}
			row := &contracts.StripeWithdrawal{ID: "progress-race", AccountID: account, AmountMicroUSD: 5_000_000, NetMicroUSD: 5_000_000, Status: "pending", Method: "standard"}
			if err := s.CreateStripeWithdrawal(row); err != nil {
				t.Fatal(err)
			}
			before, err := s.GetStripeWithdrawal(row.ID)
