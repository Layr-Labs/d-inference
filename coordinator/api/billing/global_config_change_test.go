...
func newConfigChangeQuote(t *testing.T, failFirst bool) (*controllerFixture, *store.MemoryStore, *store.User, *fakeGlobalStripe, string) {
	t.Helper()
...
func TestGlobalPayoutFundingChangeReconcilesKnownReturn(t *testing.T) {
	...
	globalAPIRequest(t, s, u, "/withdraw", `{"amount_usd":"10","quote_id":"`+id+`"}`, s.StripeWithdraw)
...
func TestGlobalPayoutFundingChangeRejectsUndebitedQuote(t *testing.T) {
	s, st, u, f, id := newConfigChangeQuote(t, false)
	s.billing().GlobalPayouts().FinancialAccount = "fa_new"
	w := globalAPIRequest(t, s, u, "/withdraw", `{"amount_usd":"10","quote_id":"`+id+`"}`, s.StripeWithdraw)
	p, _ := st.GetGlobalPayout(id)
...
func TestGlobalPayoutPauseExpiresUnsubmittedConfirmation(t *testing.T) {
	...
	s.SetBilling(billingservice.NewService(st, s.billing().Ledger(), s.logger, billingservice.Config{MockMode: true, StripeGlobalPayoutsFinancialAccount: "fa_gp", StripeGlobalPayoutsSecretKey: "rk_test_gp"}))
