...
func TestAppAttestStorageBoundsBusyAndStoppedSessionsThroughCompletion(t *testing.T) {
	...
	s := &Server{store: st}
	newSession := func(reason string) *appAttestShadowSession {
		p := newCodeAttestProvider("endpoint", "se")
		return &appAttestShadowSession{s: s, provider: p, store: st, archive: st, id: "session", expected: "assertion", rejectReason: reason, in: make(chan protocol.AppAttestShadowPayload, 2)}
	}
