...
func requestOutcomeStoreContract(t *testing.T, s contracts.RequestOutcomeStore) {
	t.Helper()
	ctx := context.Background()
	now := time.Now()
	r := contracts.RequestOutcomeRecord{CoordRequestID: "coord-1", SchemaVersion: 1, Revision: 1, ReceivedAt: now, UpdatedAt: now, Endpoint: "/v1/messages", Termination: "in_progress", Attempts: []contracts.RequestAttemptOutcome{}}
	write := func(rows ...contracts.RequestOutcomeRecord) {
		t.Helper()
		if err := s.RecordRequestOutcomes(ctx, rows); err != nil {
			t.Fatal(err)
		}
	}
	read := func() contracts.RequestOutcomeRecord {
		t.Helper()
	...
	r.Attempts = []contracts.RequestAttemptOutcome{{RequestID: "attempt-1", Attempt: 0, WriteCompleted: true, ProviderOutcome: "completed", Finalized: true}}
	write(r)
	stale := r
	stale.Revision = 1
	...
	receiptOnly := contracts.RequestOutcomeRecord{CoordRequestID: "receipt-only", SchemaVersion: 1, Revision: 1, ReceivedAt: now.Add(-3 * time.Hour), UpdatedAt: now, Termination: "in_progress", Attempts: []contracts.RequestAttemptOutcome{}}
	...
	r.CoordRequestID = ""
	if err := s.RecordRequestOutcomes(ctx, []contracts.RequestOutcomeRecord{r}); err == nil {
		t.Fatal("empty identity accepted")
	}
	r.CoordRequestID = "oversized"
	r.Attempts = make([]contracts.RequestAttemptOutcome, contracts.MaxRequestOutcomeAttempts+1)
	if err := s.RecordRequestOutcomes(ctx, []contracts.RequestOutcomeRecord{r}); err == nil {
...
func TestMemoryRequestOutcomeContract(t *testing.T) {
	requestOutcomeStoreContract(t, memory.New(contracts.Config{}))
}
...
func TestRequestOutcomeRetention(t *testing.T) {
	for _, backend := range []string{"memory", "postgres"} {
		t.Run(backend, func(t *testing.T) {
			...
			ctx := context.Background()
			now := time.Now()
			r := contracts.RequestOutcomeRecord{CoordRequestID: "old", SchemaVersion: 1, Revision: 1, ReceivedAt: now.Add(-48 * time.Hour), UpdatedAt: now, Attempts: []contracts.RequestAttemptOutcome{}}
			if err := s.RecordRequestOutcomes(ctx, []contracts.RequestOutcomeRecord{r}); err != nil {
				t.Fatal(err)
			}
...
func TestRequestOutcomeTerminalStoreContract(t *testing.T) {
	for _, backend := range []string{"memory", "postgres"} {
		t.Run(backend, func(t *testing.T) {
			var s contracts.RequestOutcomeStore
			if backend == "memory" {
				s = memory.New(contracts.Config{})
			} else {
				s = testPostgresStore(t)
			...
			ctx := context.Background()
			now := time.Now()
			r := contracts.RequestOutcomeRecord{CoordRequestID: "terminal-contract", SchemaVersion: 1, Revision: 1, ReceivedAt: now, UpdatedAt: now, Endpoint: "/v1/responses", Attempts: []contracts.RequestAttemptOutcome{}}
			write := func() {
				t.Helper()
				if err := s.RecordRequestOutcomes(ctx, []contracts.RequestOutcomeRecord{r}); err != nil {
					t.Fatal(err)
