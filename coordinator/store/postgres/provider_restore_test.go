...
func TestProviderRestoreSelectsLatestPriorIdentity(t *testing.T) {
	for name, backend := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			st := storecache.New(backend, storecache.DefaultCacheConfig()) // exercise decorator forwarding
...
func TestListProviderRecordsRejectsPartialScan(t *testing.T) {
	...
	s, err := New(context.Background(), contracts.Config{DatabaseURL: databaseURL})
	if err != nil {
		t.Fatal(err)
	}
	defer s.Close()
	...
	for i, id := range []string{"good", "bad"} {
		if err := s.UpsertProvider(context.Background(), contracts.ProviderRecord{ID: id, Hardware: json.RawMessage(`{}`), Models: json.RawMessage(`[]`), RegisteredAt: now, LastSeen: now.Add(-time.Duration(i) * time.Minute)}); err != nil {
			t.Fatal(err)
		}
	}
...
func TestProviderAndReputationPublicationIsAtomic(t *testing.T) {
	...
	s, err := New(ctx, contracts.Config{DatabaseURL: newWithdrawableTestDatabase(t)})
	if err != nil {
		t.Fatal(err)
	}
	defer s.Close()
	...
	rec := contracts.ProviderRecord{ID: "completed", SerialNumber: "serial", Hardware: json.RawMessage(`{}`), Models: json.RawMessage(`[]`), RegisteredAt: time.Now(), LastSeen: time.Now(), LifetimeTokensGenerated: 700}
	if err := s.UpsertProviderWithReputation(ctx, rec, contracts.ReputationRecord{TotalJobs: -1}); err == nil {
	...
	cached := storecache.New(s, storecache.DefaultCacheConfig())
	if err := cached.UpsertProviderWithReputation(ctx, rec, contracts.ReputationRecord{TotalJobs: 12}); err != nil {
		t.Fatal(err)
	}
