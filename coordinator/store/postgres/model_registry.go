...
func (s *Store) ListActiveModelRegistryWithError() ([]contracts.ModelRegistryRecord, error) {
	...
	var records []contracts.ModelRegistryRecord
	...
	if len(versionIDs) > 0 {
		...
		for i := range records {
			if records[i].ActiveVersion != nil {
				...
				if files == nil {
					files = []contracts.ModelVersionFile{}
				}
...
func (s *Store) FindPublishingAPIKeysWithError() ([]contracts.PublishingAPIKey, error) {
	...
	var keys []contracts.PublishingAPIKey
	for rows.Next() {
		var key contracts.PublishingAPIKey
...
func scanModelRegistryRecord(row rowScanner) (*contracts.ModelRegistryRecord, error) {
	var rec contracts.ModelRegistryRecord
	var version contracts.ModelVersion
...
func (s *Store) loadModelRegistryFiles(ctx context.Context, rec *contracts.ModelRegistryRecord) error {
	...
	for rows.Next() {
		var file contracts.ModelVersionFile
	...
	if rec.Files == nil {
		rec.Files = []contracts.ModelVersionFile{}
	}
	return nil
}
...
func (s *Store) loadModelRegistryFilesBatch(ctx context.Context, versionIDs []int64) (map[int64][]contracts.ModelVersionFile, error) {
	...
	result := make(map[int64][]contracts.ModelVersionFile, len(versionIDs))
	for rows.Next() {
		var file contracts.ModelVersionFile
		if err := rows.Scan(&file.ID, &file.ModelVersionID, &file.Path, &file.SizeBytes, &file.SHA256, &file.Role); err != nil {
...
func (s *Store) UpsertModelAlias(alias *contracts.ModelAlias) error {
	...
	if sourceKind == "" {
		sourceKind = contracts.ModelAliasSourceAlias
	}
