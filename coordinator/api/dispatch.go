...
func (d *dispatchState) dispatchPrimary() dispatchOutcome {
	...
	if d.provider == nil {
		...
		if dispatchErr == errClientGoneBeforeScan || dispatchErr == errClientGoneDuringSend {
			// The caller's context fired while parked for a scan slot, or
			// while the frame was in the provider writer. Mirror the
			// queue-wait cancellation arm exactly: cancelled route outcome,
			// refund, no response body — NEVER the routing_saturated /
			// provider_unreachable 429 or a rejection-ledger row (the client
			// is not retrying; the ledger must not count a shed that never
			// happened).
		...
		if writeErr != nil {
			...
			if r.Context().Err() != nil {
				// The consumer hung up while the frame was in the writer
				// (writeCtx derives from r.Context()): client_gone, exactly
				// as the errClientGoneBeforeScan / queue-wait arms — never
				// the send-failed class and its provider_unreachable verdict.
				d.emitClientGone(phaseBeforeFirstToken)
				d.updateRoutingOutcome(d.errorRoutingOutcome("cancelled", "client_gone", 0))
				d.refundReservation()
				return outcomeClientGone
...
func (d *dispatchState) noteDispatchRetry(provider *registry.Provider, pr *registry.PendingRequest, errMsg protocol.InferenceErrorMessage, held *[]string) {
	d.noteDeadlineWedgeRefusal(provider, pr, errMsg.ErrorReason, errMsg.CoordinatorCause)
	if !d.noteProviderError(provider, pr, errMsg.StatusCode, errMsg.Error, errMsg.ErrorReason, errMsg.TerminalCause, held) {
		...
		case chunk, ok := <-pr.ChunkCh:
			...
			if ok {
				...
				case errMsg := <-pr.ErrorCh:
					...
					d.noteDispatchRetry(provider, pr, errMsg, &d.heldChunks)
					d.provider = nil
					d.pr = nil
					return outcomeRetry
				default:
		...
		case chunk, ok := <-pr.ChunkCh:
			...
			if ok {
				...
				case errMsg2 := <-pr.ErrorCh:
					...
					d.noteDispatchRetry(provider, pr, errMsg2, &d.heldChunks)
					d.provider = nil
					d.pr = nil
					d.requestID = ""
					return outcomeRetry
		...
		case chunk, ok := <-backupPR.ChunkCh:
			...
			if ok {
				...
				case errMsg2 := <-backupPR.ErrorCh:
					...
					d.noteDispatchRetry(backupProvider, backupPR, errMsg2, &backupHeld)
					d.provider = nil
					d.pr = nil
					return outcomeRetry
				default:
