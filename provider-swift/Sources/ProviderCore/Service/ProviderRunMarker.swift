...
public struct ProviderRunMarker: Sendable {
    ...
    public struct Record: Codable, Sendable, Equatable {
        ...
        public init(state: State, processStartedAt: Int64?, version: String?, exitCause: ExitCause? = nil,
    ...
    public func markRunning(processStartedAt: Int64?, version: String, previousExit: AppAttestPreviousExit?) throws {
        try write(Record(state: .running, processStartedAt: processStartedAt, version: version, previousExit: previousExit))
    }
    ...
    public func markClean(processStartedAt: Int64?, cause: ExitCause, now: Date = Date()) throws {
        guard var record = read(), record.processStartedAt == processStartedAt else { return }
        record.state = .clean
    ...
    public static func previousExit(_ previous: Record?, processStartedAt: Int64?) -> AppAttestPreviousExit {
        if let previous, let processStartedAt, previous.processStartedAt == processStartedAt {
