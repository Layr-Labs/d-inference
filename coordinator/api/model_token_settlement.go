...
func (s *Server) settleModelTokenPromotion(pr *registry.PendingRequest, provider *registry.Provider, usage protocol.UsageInfo, in, out int64, custom bool, payout int64, freeSelf bool) (bool, int64, error) {
	...
	if err != nil {
		...
		if errors.Is(err, store.ErrPromotionInvalidSettlement) {
			_, refundErr := s.releaseModelTokenReservation(pr.ModelTokenReservationID)
			return false, 0, errors.Join(err, refundErr)
		}
		s.modelTokenSettlements.Store(pr.ModelTokenReservationID, func() error {
			_, retryErr := backend.SettleModelTokenReservation(pr.ModelTokenReservationID, actual, quote, earning)
			return retryErr
		})
		return false, 0, err
	}
