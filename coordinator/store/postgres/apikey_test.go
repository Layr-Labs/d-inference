...
func TestCreateAPIKeyAndAuthenticate(t *testing.T) {
	...
	raw, rec, err := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{
		Name:          "prod",
		LimitMicroUSD: &limit,
		LimitReset:    contracts.KeyResetMonthly,
	})
	if err != nil {
		t.Fatalf("CreateAPIKey: %v", err)
	}
	if !strings.HasPrefix(raw, contracts.KeyPrefix) {
		t.Errorf("raw key %q missing prefix %q", raw, contracts.KeyPrefix)
	}
...
func TestAuthenticateKeyDisabledAndExpired(t *testing.T) {
	...
	raw, rec, err := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "k"})
	if err != nil {
		t.Fatalf("CreateAPIKey: %v", err)
	}
...
func TestListAndScopingByOwner(t *testing.T) {
	s := memory.New(contracts.Config{})
	_, k1, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "a"})
	s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "b"})
	s.CreateAPIKey("acct-2", contracts.APIKeyCreate{Name: "c"})
...
func TestRevokeAPIKeyByID(t *testing.T) {
	s := memory.New(contracts.Config{})
	raw, rec, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "a"})
...
func TestUpdateAPIKeyClearsLimits(t *testing.T) {
	...
	rpm := int64(60)
	_, rec, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{
		Name: "a", LimitMicroUSD: &limit, RPMLimit: &rpm, LimitReset: contracts.KeyResetDaily,
	})
	...
	mut.RPMLimit = nil
	mut.LimitReset = contracts.KeyResetNone
	...
	if updated.LimitReset != contracts.KeyResetNone {
...
func TestKeySpendSinceWindows(t *testing.T) {
	s := memory.New(contracts.Config{})
	_, rec, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "a"})
	...
	since := contracts.KeySpendWindowStart(contracts.KeyResetDaily, time.Now())
...
func TestKeySpendWindowStart(t *testing.T) {
	...
	if got := contracts.KeySpendWindowStart(contracts.KeyResetNone, now); !got.IsZero() {
		t.Errorf("none window = %v, want zero", got)
	}
	if got := contracts.KeySpendWindowStart(contracts.KeyResetDaily, now); !got.Equal(time.Date(2026, 5, 29, 0, 0, 0, 0, time.UTC)) {
		t.Errorf("daily window = %v", got)
	}
	// Monday of that week is 2026-05-25.
	if got := contracts.KeySpendWindowStart(contracts.KeyResetWeekly, now); !got.Equal(time.Date(2026, 5, 25, 0, 0, 0, 0, time.UTC)) {
		t.Errorf("weekly window = %v, want 2026-05-25", got)
	}
	if got := contracts.KeySpendWindowStart(contracts.KeyResetMonthly, now); !got.Equal(time.Date(2026, 5, 1, 0, 0, 0, 0, time.UTC)) {
...
func TestRotateAPIKeyAtomic(t *testing.T) {
	...
	oldRaw, old, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{
		Name: "prod", LimitMicroUSD: &limit, LimitReset: contracts.KeyResetWeekly, AllowedModels: []string{"m1"},
	})
	...
	// Limits carried over.
	if rec.LimitMicroUSD == nil || *rec.LimitMicroUSD != limit || rec.LimitReset != contracts.KeyResetWeekly || len(rec.AllowedModels) != 1 {
...
func TestRotateCarriesDisabledState(t *testing.T) {
	s := memory.New(contracts.Config{})
	_, old, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "k"})
	mut := *old
	mut.Disabled = true
...
func TestRevokeKeySoftDisables(t *testing.T) {
	s := memory.New(contracts.Config{})
	raw, rec, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "k"})
...
func TestValidateKeyEnforcesExpiry(t *testing.T) {
	s := memory.New(contracts.Config{})
	past := time.Now().Add(-time.Hour)
	raw, _, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "k", ExpiresAt: &past})
	if s.ValidateKey(raw) {
		t.Error("expired key must not validate")
	}
	future := time.Now().Add(time.Hour)
	raw2, _, _ := s.CreateAPIKey("acct-1", contracts.APIKeyCreate{Name: "k2", ExpiresAt: &future})
...
func TestLegacyAccountID(t *testing.T) {
	...
	id := contracts.LegacyAccountID(raw)
...
func TestMigrateAccountBalance(t *testing.T) {
	s := memory.New(contracts.Config{})
	from := "sk-db-rawtoken"
	to := contracts.LegacyAccountID(from)

	// Seed the old raw-token identity with a balance (mix of withdrawable).
	if err := s.Credit(from, 5_000_000, contracts.LedgerDeposit, "seed"); err != nil {
		t.Fatalf("Credit: %v", err)
	}
	if err := s.CreditWithdrawable(from, 2_000_000, contracts.LedgerAdminReward, "seed-wdr"); err != nil {
...
func TestKeyLabelMasking(t *testing.T) {
	raw := contracts.KeyPrefix + "abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
	label := contracts.KeyLabel(raw)
	if !strings.HasPrefix(label, contracts.KeyPrefix) {
