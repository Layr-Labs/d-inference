...
func TestGlobalPayoutQuotePruningPreservesConfirmedFunds(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if err = g.ApplyGlobalPayout(posted.ID, contracts.GlobalPayoutResult{Status: "posted", ExternalID: "obp_prune"}, time.Now()); err != nil {
				t.Fatal(err)
			}
			...
			for _, p := range []contracts.GlobalPayout{pending, posted, live} {
				if _, err = g.GetGlobalPayout(p.ID); err != nil {
					t.Fatalf("retained row %s: %v", p.ID, err)
				}
			}
			if _, err = g.GetGlobalPayout(expired.ID); !errors.Is(err, contracts.ErrNotFound) {
...
func TestGlobalPayoutRejectionFencesFutureDispatch(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			for range 2 {
				if err = g.ApplyGlobalPayout(p.ID, contracts.GlobalPayoutResult{Status: "failed", FailureCode: got.Rejection.Code}, time.Now()); err != nil {
					t.Fatal(err)
				}
			}
			...
			if err = g.RecordGlobalPayoutRejection(ambiguous.ID, 1, "forbidden"); !errors.Is(err, contracts.ErrPayoutConflict) {
...
func TestGlobalPayoutQuoteInvalidationSerializesWithDebit(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			now := time.Now()
			if _, err := g.ExpireGlobalPayoutQuote("other", p.ID, now); !errors.Is(err, contracts.ErrNotFound) {
				t.Fatal("wrong account invalidated quote")
			}
			if _, err := g.ExpireGlobalPayoutQuote(p.AccountID, p.ID, now); err != nil {
				t.Fatal(err)
			}
			if _, err := g.BeginGlobalPayout(p.AccountID, p.ID, now.Add(-time.Second)); !errors.Is(err, contracts.ErrPayoutQuoteExpired) {
