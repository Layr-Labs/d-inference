...
func (s *Store) RecordRequestProfiles(records []*contracts.RequestProfileRecord) error {
	live := make([]*contracts.RequestProfileRecord, 0, len(records))
...
func (s *Store) RequestProfilesSinceFiltered(since time.Time, filter contracts.RequestProfileFilter) []contracts.RequestProfileRecord {
	...
	args = append(args, routerecord.MaxTelemetryReadRows)
	...
	var records []contracts.RequestProfileRecord
	for rows.Next() {
		...
		r.GateRejections = recordutil.JsonbParam(gate)
		r.Candidates = recordutil.JsonbParam(candidates)
		r.ProviderProfile = recordutil.JsonbParam(providerProfile)
...
func (s *Store) FleetSnapshotsSince(since time.Time) []contracts.FleetSnapshotRow {
	...
	var out []contracts.FleetSnapshotRow
	for rows.Next() {
		...
		f.QueueDepthByModel = recordutil.JsonbParam(queueDepthByModel)
		out = append(out, f)
	}
	return out
}
...
func (s *Store) PruneTelemetry(ctx context.Context, profilesBefore, snapshotsBefore time.Time, batch int) (int, error) {
...
func (s *Store) pruneTelemetryTable(ctx context.Context, t telemetryTable, before time.Time, batch int) (int, int, error) {
	if before.IsZero() {
		return 0, 0, nil
	}
	if batch <= 0 {
...
func (s *Store) deleteTelemetryIDRange(ctx context.Context, t telemetryTable, lo, hi int64, before time.Time) (int, error) {
