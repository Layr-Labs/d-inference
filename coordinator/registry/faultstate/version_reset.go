...
type disconnectSource[C comparable] struct {
	p  *Session[C]
	at time.Time
}

func (r *Manager[C]) captureDisconnectSource(sessionID string) disconnectSource[C] {
	r.gatesMu.RLock()
	defer r.gatesMu.RUnlock()
	if p := r.sessions[sessionID]; p != nil {
		return disconnectSource[C]{p: p}
	}
	return disconnectSource[C]{at: r.disconnectedStableIDs[sessionID].at}
}

func (source disconnectSource[C]) supersededBy(g *gateState) bool {
	at := source.at
	if source.p != nil {
...
func (r *Manager[C]) IsSupersededDisconnectFlush(sessionID string, statusCode int, causes ...protocol.CoordinatorInferenceErrorCause) bool {
...
func noteIdentityVersionLocked[C comparable](g *gateState, r *Manager[C], version string) {
	if version == "" {
		return
	}
