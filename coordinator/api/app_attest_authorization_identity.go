...
func (x *appAttestShadowSession) updateServingAuthorization(status *protocol.AppAttestStatus, evidence appattest.AuthorizationEvidence, verdict appattest.AuthorizationVerdict) {
	a := x.s.appAttestAuthorizer
...
func (s *Server) appAttestIdentityCandidate(r *protocol.RegisterMessage) bool {
	return s.appAttestShadow.ServingEnabled && r.AppAttestProtocol == 3 &&
