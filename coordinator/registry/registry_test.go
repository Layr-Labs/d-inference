...
func TestProviderMissingPrivacyCapsExcludedFromTextRouting(t *testing.T) {
	...
	found := reg.SelectProvider("mlx-community/Qwen3.5-9B-Instruct-4bit")
	if found != nil {
...
func TestFindProvider(t *testing.T) {
	...
	// ReserveProvider is the production select-and-serve path; it marks the
	// chosen provider StatusServing (the read-only SelectProvider does not).
	p := reg.ReserveProvider("mlx-community/Qwen3.5-9B-Instruct-4bit",
		&PendingRequest{RequestID: "req-find", Model: "mlx-community/Qwen3.5-9B-Instruct-4bit"})
	if p == nil {
...
func TestFindProviderNoMatch(t *testing.T) {
	...
	p := reg.SelectProvider("nonexistent-model")
	if p != nil {
...
func TestFindProviderSkipsAtMaxConcurrency(t *testing.T) {
	...
	p := reg.SelectProvider("mlx-community/Qwen3.5-9B-Instruct-4bit")
	if p != nil {
...
func TestSetProviderIdle(t *testing.T) {
	...
	// Mark as serving via the production select-and-serve path (reserves capacity).
	reg.ReserveProvider("mlx-community/Qwen3.5-9B-Instruct-4bit",
		&PendingRequest{RequestID: "req-idle", Model: "mlx-community/Qwen3.5-9B-Instruct-4bit"})
	p := reg.GetProvider("p1")
	if p.Status != StatusServing {
		t.Errorf("status = %q, want %q", p.Status, StatusServing)
	}

	// Simulate request completion (SetProviderIdle only transitions to online
	// once the provider has no pending requests).
	p.RemovePending("req-idle")
...
func TestScoringHigherDecodeTPS(t *testing.T) {
	...
	selected := reg.SelectProvider("mlx-community/Qwen3.5-9B-Instruct-4bit")
	if selected == nil {
...
func TestSetProviderIdleDrainsQueue(t *testing.T) {
	...
	reg.SelectProvider("mlx-community/Qwen3.5-9B-Instruct-4bit")
...
func TestChallengeSuccessEnablesRouting(t *testing.T) {
	...
	if reg.SelectProvider("mlx-community/Qwen3.5-9B-Instruct-4bit") != nil {
...
func TestThermalCriticalBlocksRouting(t *testing.T) {
	...
	selected := reg.SelectProvider(model)
	if selected == nil {
...
func TestModelCatalogGatesRoutingWithoutDroppingInventory(t *testing.T) {
	...
	testMakeTextRoutable(p)
	if found := reg.SelectProvider("mlx-community/random-model-not-in-catalog"); found != nil {
		t.Fatal("expected non-catalog model to stay unroutable")
	}
	if found := reg.SelectProvider("mlx-community/Qwen3.5-9B-Instruct-4bit"); found == nil {
...
func TestRegisterWithEmptyConfiguredCatalogPreservesInventoryButRoutesNothingUntilCatalogUpdates(t *testing.T) {
	...
	if found := reg.SelectProvider(modelID); found != nil {
...
// TestCrashedOnlyProviderNotRouted verifies the live cost-based selection
// excludes a provider whose only slot for the model is crashed (its routing
// cost is infinite), so a crashed-only fleet yields no routable provider.
func TestCrashedOnlyProviderNotRouted(t *testing.T) {
	reg := New(testLogger())
	model := "mlx-community/Qwen3.5-9B-Instruct-4bit"
	msg := testRegisterMessage()
	...
	if found := reg.SelectProvider(model); found != nil {
		t.Errorf("crashed-only provider should not be routable, got %q", found.ID)
	}
}
