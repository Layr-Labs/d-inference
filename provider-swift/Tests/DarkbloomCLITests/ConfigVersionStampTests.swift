...
/// Durable provider config migrations.
///
/// Decode-time migration changes generated values in memory, while the
/// `config_version` bump and text rewrite spend the old-schema evidence once.
/// Without the durable half, a later operator edit could be reinterpreted on
/// every boot. These tests pin both the v0.8.1 concurrency migration and the
/// tri-state MTP normalization without reserializing away operator comments.
...
struct ConfigVersionStampTests {
    ...
    @Test("a generated 8 becomes 4 and the file is re-dated to 3")
    func migratesGeneratedEight() throws {
        try withTempConfig(generated) { path in
            ...
            #expect(text.hasPrefix("config_version = 3\n"))
    ...
    func redatesFileWithNothingToMigrate() throws {
        try withTempConfig(
            ...
            #expect(migrateConfigSchema(in: path).isEmpty)
            #expect(try read(path).hasPrefix("config_version = 3\n"))
    ...
    @Test("version-2 generated MTP booleans normalize to auto and on exactly once")
    func migratesLegacyMTPBooleans() throws {
        for (legacy, expected) in [(false, MTPMode.auto), (true, MTPMode.on)] {
            try withTempConfig(
                """
                config_version = 2
                [provider]
                ...
                [backend]
                mtp = \(legacy) # generated by the boolean schema
                """
            ) { path in
                #expect(migrateConfigSchema(in: path).isEmpty)
                let afterMigration = try read(path)
                #expect(afterMigration.hasPrefix("config_version = 3\n"))
                #expect(
                    afterMigration.contains(
                        "mtp_mode = \"\(expected.rawValue)\" # generated by the boolean schema"))
                #expect(!afterMigration.contains("\nmtp = "))

                // The schema migration is durable and one-shot.
                #expect(migrateConfigSchema(in: path).isEmpty)
                #expect(try read(path) == afterMigration)

                let migrated = try ConfigManager.load(from: path)
                #expect(migrated.backend.mtpMode == expected)

                // A normal config save also remains stable after normalization.
                try ConfigManager.save(migrated, to: path)
                let firstSave = try read(path)
                try ConfigManager.save(try ConfigManager.load(from: path), to: path)
                #expect(try read(path) == firstSave)
            }
        }
    }
    ...
    func preservesExplicitMTPOff() throws {
        try withTempConfig(
            ...
            #expect(migrateConfigSchema(in: path).isEmpty)
            let migrated = try ConfigManager.load(from: path)
            #expect(migrated.backend.mtpMode == .off)

            try ConfigManager.save(migrated, to: path)
            let normalized = try read(path)
            #expect(normalized.contains("mtp_mode = 'off'"))
    ...
    func spacingVariantsCountAsDated() throws {
        for (stamp, want) in [
            ("  config_version = 1", "  config_version = 3"),
            ("config_version=1", "config_version=3"),
            ("config_version\t=\t1", "config_version\t=\t3"),
            ("config_version = 1 # dated by the upgrade", "config_version = 3 # dated by the upgrade"),
        ] {
            try withTempConfig(
                """
                \(stamp)
