...
extension ProviderLoop {
    ...
    internal func handleInferenceRequest(
        ...
        // A local opaque id is generated by the tracker; the coordinator's
        // request id is deliberately never persisted. Requests that reach the
        // terminal observer below owns completion across every early and late
        // coordinator path.
        let earlyAnalytics = InferenceAnalyticsTracker(
            servingMode: "coordinator", model: nil, streaming: false)
        var analyticsAccepted = false
        defer {
            // Covers only paths that return without sending a terminal. Normal
            // terminal delivery below wins because the tracker is one-shot.
            earlyAnalytics.finish(
                outcome: analyticsAccepted ? "failed" : "rejected",
                errorClass: analyticsAccepted ? "pre_generation" : "admission")
        }
        ...
        let lookupReceiptFinalizer = PrefixCacheLookupReceiptFinalizer(
            ...
            terminalObserver: earlyAnalytics.finish(outbound:))
        ...
        guard requestToModel[requestId] == modelId else {
            ...
            earlyAnalytics.finish(outcome: "cancelled", errorClass: "cancelled")
            return
        }
            ...
            do {
                for try await frame in frames {
                    ...
                    if let parsed = Self.parseStreamChunk(frame) {
                        ...
                        if frameHadContent {
                            contentFrameCount += 1
                            earlyAnalytics.noteFirstContent()
                        }
