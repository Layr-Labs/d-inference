...
enum WorkerInferenceSupport {
    ...
    internal static func parseStreamChunk(_ frame: String) -> StreamChunkExtract? {
        guard let joined = joinedDataPayload(frame) else { return nil }
    ...
    internal static func inferReasoningParser(
        for modelType: String?
    ) -> ReasoningParserFormat {
        switch modelType?.lowercased() {
        case let value? where value.contains("gemma"):
            return .gemma4
        case let value? where value.contains("gpt_oss"):
            return .harmony
