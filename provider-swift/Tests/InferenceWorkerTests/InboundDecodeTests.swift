...
struct InboundDecodeTests {
    ...
    private func decode(_ json: String) throws -> OpenAIChatCompletionRequest {
        try WorkerInferenceSupport.decodeOpenAIRequest(Data(json.utf8))
    }
    ...
    private func effort(_ json: String) -> String? {
        WorkerInferenceSupport.extractChatTemplateControls(from: Data(json.utf8)).reasoningEffort
    }
    ...
    func qwenTemplateControlsExtracted() {
        ...
        let controls = WorkerInferenceSupport.extractChatTemplateControls(from: Data(json.utf8))
    ...
    func qwenThinkingAliasPrecedence() {
        let kwargsOnly = WorkerInferenceSupport.extractChatTemplateControls(
            from: Data(
                #"{"model":"m","messages":[],"chat_template_kwargs":{"enable_thinking":true}}"#
                    .utf8))
        #expect(kwargsOnly.enableThinking == true)

        let both = WorkerInferenceSupport.extractChatTemplateControls(
            from: Data(
    ...
    func malformedTopLevelThinkingFallsThroughToKwargs() {
        let controls = WorkerInferenceSupport.extractChatTemplateControls(
            from: Data(
    ...
    private func logprobsSpec(_ json: String) -> (topLogprobs: Int?, requested: Bool)? {
        WorkerInferenceSupport.extractLogprobsSpec(from: Data(json.utf8))
    }
    ...
    private func sampling(_ json: String) -> EngineV2SamplingOverrides? {
        WorkerInferenceSupport.extractSamplingOverrides(from: Data(json.utf8))
    }
