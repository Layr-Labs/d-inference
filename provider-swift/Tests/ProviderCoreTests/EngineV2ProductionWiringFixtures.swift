...
func productionWiringStubTokenizer(decodeOverride: String? = nil) -> StubTokenizer {
    StubTokenizer(
        tokenTable: ["</s>": 2],
        decodeOverride: decodeOverride,
        idTokenConversion: .unicodeScalar
    )
}
...
func productionRecordServerStream(
    ...
    func productionBuildAndInstallSlotA(
        ...
        engines: @escaping @Sendable (String, Int) -> ScriptedCBv2Engine
    ) async throws -> (bridge: EngineV2Bridge, engine: ScriptedCBv2Engine, sizing: SlotSizingSnapshot) {
    ...
    final class ProductionEngineBox: @unchecked Sendable {
        private let lock = NSLock()
        private var _engines: [ScriptedCBv2Engine] = []
        var all: [ScriptedCBv2Engine] { lock.withLock { _engines } }
        func append(_ engine: ScriptedCBv2Engine) { lock.withLock { _engines.append(engine) } }
    }
