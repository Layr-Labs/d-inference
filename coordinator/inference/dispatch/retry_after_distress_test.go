...
func TestNoteAttempt0RouteLatency_IgnoresMediaFetchTime(t *testing.T) {
	...
	srv.deps.Registry().SetModelCatalog([]registry.CatalogEntry{{ID: model, SizeGB: 1, MinRAMGB: 24}})
	...
	srv.dispatchOneProvider(
		...
		registry.RequestTraits{}, nil, false, RoutePolicy{}, timing,
...
func TestNoteAttempt0RouteLatency_RecordsFailedSelections(t *testing.T) {
	srv := newTestController(t) // zero providers: every reservation fails
	now := time.Now()
	...
	_, _, _, _, lastErr, _ := srv.dispatchOneProvider(
		...
		false, registry.RequestTraits{}, nil, false, RoutePolicy{}, timing,
		false, registry.CachePlan{}, map[string]struct{}{}, 0, nil, "", nil, nil)
