...
// Store is a read-through cache decorator over Store. It overrides only
...
type Store struct {
	contracts.Store
	users  *domainCache[contracts.User]
	models *domainCache[contracts.ModelRegistryRecord]
}
...
// New wraps inner (memory or Postgres) with the read-through cache.
func New(inner contracts.Store, cfg CacheConfig) *Store {
	cfg = cfg.withDefaults()
	return &Store{
		Store:  inner,
		users:  newDomainCache[contracts.User](cfg.UserTTL, cfg.NegativeTTL, cfg.MaxUsers, cfg.Now),
		models: newDomainCache[contracts.ModelRegistryRecord](cfg.ModelTTL, cfg.NegativeTTL, cfg.MaxModels, cfg.Now),
	}
}
...
func (c *Store) Unwrap() contracts.Store { return c.Store }

// Compile-time checks: the decorator still satisfies the full Store, and it
// is transparent to store.As.
var (
	_ contracts.Store     = (*Store)(nil)
	_ contracts.Unwrapper = (*Store)(nil)
)
...
func (c *Store) Stats() CacheStats {
	return CacheStats{Users: c.users.counters(), Models: c.models.counters()}
}

func (c *Store) GetUserByAccountID(accountID string) (*contracts.User, error) {
	return c.users.get("account\x00"+accountID, func() (*contracts.User, error) {
		return c.Store.GetUserByAccountID(accountID)
	}, cloneUser)
}

func (c *Store) GetUserByPrivyID(privyUserID string) (*contracts.User, error) {
	return c.users.get("privy\x00"+privyUserID, func() (*contracts.User, error) {
		return c.Store.GetUserByPrivyID(privyUserID)
	}, cloneUser)
}

func (c *Store) CreateUser(user *contracts.User) error {
	err := c.Store.CreateUser(user)
	c.users.invalidate()
	return err
}

func (c *Store) SetUserStripeAccount(accountID, stripeAccountID, status, stripeAccountCountry, destinationType, destinationLast4 string, instantEligible bool) error {
...
func (c *Store) SetUserRole(accountID, role string) error {
	err := c.Store.SetUserRole(accountID, role)
	c.users.invalidate()
	return err
}

func (c *Store) SetUserPlatformFeePercent(accountID string, feePercent *int64) error {
	err := c.Store.SetUserPlatformFeePercent(accountID, feePercent)
	c.users.invalidate()
	return err
}

func (c *Store) GetModelRegistryRecord(modelID string) (*contracts.ModelRegistryRecord, error) {
	return c.models.get(modelID, func() (*contracts.ModelRegistryRecord, error) {
...
func (c *Store) GetModelManifest(modelID string) (*contracts.ModelManifest, error) {
	rec, err := c.models.get(modelID, func() (*contracts.ModelRegistryRecord, error) {
		return c.Store.GetModelRegistryRecord(modelID)
	}, func(r *contracts.ModelRegistryRecord) *contracts.ModelRegistryRecord { return r })
	if err != nil {
		return nil, err
	}
...
func (c *Store) UpsertModelRegistryEntry(entry *contracts.ModelRegistryEntry) error {
	err := c.Store.UpsertModelRegistryEntry(entry)
	c.models.invalidate()
	return err
}

func (c *Store) SetModelVersion(entry *contracts.ModelRegistryEntry, version *contracts.ModelVersion, files []contracts.ModelVersionFile) error {
	err := c.Store.SetModelVersion(entry, version, files)
	c.models.invalidate()
	return err
}

func (c *Store) PromoteModelVersion(modelID, version string) error {
	err := c.Store.PromoteModelVersion(modelID, version)
	c.models.invalidate()
	return err
}

func (c *Store) SetModelStatus(modelID, status string) error {
