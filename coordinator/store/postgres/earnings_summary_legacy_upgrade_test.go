...
func legacyFloorUpgradeStore(t *testing.T) *Store {
	t.Helper()
	s, err := New(context.Background(), contracts.Config{DatabaseURL: newWithdrawableTestDatabase(t)})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(s.Close)
...
func executeLegacyFloorSettlement(t *testing.T, s *Store) {
	t.Helper()
	...
	for attempt := 0; attempt < 2; attempt++ {
		var credited bool
		err := s.pool.QueryRow(context.Background(), string(sql), "p", "a", "floor-epoch", int64(800), int64(800), int64(0), float64(1), float64(64), string(contracts.LedgerFloorDraw), "floor:floor-epoch:p").Scan(&credited)
...
func executeLegacyBootBackfill(t *testing.T, s *Store) {
	t.Helper()
...
func assertLegacyUpgradeSummaries(t *testing.T, s *Store, want contracts.ProviderEarningsSummary) {
	t.Helper()
...
func TestLegacyFloorSettlementBootAndUpgradeKeepBaseRewardsOutOfWork(t *testing.T) {
	for _, withInference := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			want := contracts.ProviderEarningsSummary{TotalMicroUSD: 800}
			if withInference {
				...
				want = contracts.ProviderEarningsSummary{Count: 1, TotalMicroUSD: 850, PromptTokens: 5, CompletionTokens: 7}
			}
...
func TestLegacyFloorUpgradePreservesLifetimeWorkDespiteRetainedHistory(t *testing.T) {
	...
	for _, job := range []struct {
		...
		var balance int64
		err := s.pool.QueryRow(context.Background(), string(sql), "a", job.amount, string(contracts.LedgerPayout), job.id, nil, "live", "p", "m", job.prompt, job.completion).Scan(&balance)
		if err != nil {
			t.Fatal(err)
		}
	}
	want := contracts.ProviderEarningsSummary{Count: 2, TotalMicroUSD: 930, PromptTokens: 8, CompletionTokens: 11}
	assertLegacyUpgradeSummaries(t, s, want)
	...
	var retained contracts.ProviderEarningsSummary
