...
func TestRejectionStoreContract(t *testing.T) {
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			testRejectionStore(t, st)
		})
	}
}
...
func TestCancelledRouteCompletionTokensStoreContract(t *testing.T) {
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			assertCancelledRowPersistsZeroCompletionTokens(t, st)
		})
	}
}
