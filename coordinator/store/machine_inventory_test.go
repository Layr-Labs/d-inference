...
func machineInventoryContract(t *testing.T, s MachineInventoryStore) {
	ctx := context.Background()
	now := time.Now().UTC()
	observe := func(session, account, se, serial string) MachineIdentity {
		t.Helper()
		id, err := s.ObserveMachine(ctx, MachineObservation{SessionID: session, AccountID: account, SEKey: se, VerifiedSerial: serial, At: now, OSMajor: 27, Source: "live_registration"})
		if err != nil {
			t.Fatal(err)
		}
		return id
	...
	for i := 0; i < 20; i++ {
		...
		go func() {
			defer wg.Done()
			id, err := s.ObserveMachine(ctx, MachineObservation{SessionID: time.Now().String(), AccountID: "owner", SEKey: "se", At: now})
	...
	if _, err := s.ObserveMachine(ctx, MachineObservation{SessionID: "a", AccountID: "different-account", SEKey: "se", At: now}); err == nil {
...
func TestMachineInventoryMemory(t *testing.T) { machineInventoryContract(t, NewMemory(Config{})) }
...
func TestMachineBackfillCannotOverwriteLiveSessionPostgres(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now()
	live := MachineObservation{SessionID: "live", AccountID: "real-owner", SEKey: "se", At: now, Source: "live_registration", OSMajor: 27}
	id, err := s.ObserveMachine(ctx, live)
	if err != nil {
		t.Fatal(err)
	}
	old := MachineObservation{SessionID: "live", AccountID: "old-owner", SEKey: "other", At: now.Add(-time.Hour), Source: "historical_registration", Disconnected: true}
