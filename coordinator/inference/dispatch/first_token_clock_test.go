...
func TestFirstTokenExpiredAndPreambleCap(t *testing.T) {
	...
	relative := &execution{deadline: 9 * time.Second}
...
func firstTokenWaitState(t *testing.T, receivedAgo, deadline time.Duration) (*execution, *registry.PendingRequest) {
	t.Helper()
	s := newTestController(t)
	st, ok := testControllerStore(s).(*store.MemoryStore)
	if !ok {
		t.Fatalf("store = %T", testControllerStore(s))
	}
	const model = "first-token-deadline-model"
	provider := s.deps.Registry().Register("first-token-provider", nil, &protocol.RegisterMessage{
...
func TestFirstTokenSpeculativeWaitUsesAbsoluteClock(t *testing.T) {
	...
	relative := &execution{deadline: 9 * time.Second, speculativeAt: 4500 * time.Millisecond}
...
func TestAbandonInflightDefersToPublishedIngress(t *testing.T) {
	...
	d.s.deps.Attempts().Cancel(provider, pr, attempt.CancelCauseFirstChunkTimeout)
}
...
func TestGenericDispatchQueueWaitUsesAbsoluteDeadline(t *testing.T) {
	...
	if depth := s.deps.Registry().Queue().QueueSize(d.model); depth != 0 {
...
func TestBufferedContentBeatsReadyErrorAndPreservesNativeMessagesTerminal(t *testing.T) {
	...
	pr.ConsumerEndpoint = response.MessagesEndpoint
