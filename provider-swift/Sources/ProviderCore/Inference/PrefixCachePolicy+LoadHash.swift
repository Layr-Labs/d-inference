...
extension PrefixCachePolicy {
    ...
    /// Explicitly enabled other families and unreadable configurations retain
    /// the conservative full bracket. Default-disabled model IDs skip it.
    ///
    /// Used only by the standalone server's SSD-specific hashes. Connected
    /// ProviderLoop loading uses the same model activation gate and preserves
    /// its content-hash publication lifecycle for attestation.
    ///
