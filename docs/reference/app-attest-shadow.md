...
| `start_reason` | `launchd`, `watchdog`, `update`, `stall_restart`, `manual`, `unknown` | `ready` | First match wins; "recent" is a timestamp from 120 s before process start up to classification time (so evidence written after start, e.g. across the in-place exec of a startup self-update, counts): `stall_restart` (recent App Attest stall-restart marker or recorded stall relaunch), `watchdog` (recent watchdog `last_restart_at`), `update` (previous run's version differs, or recent update relaunch), `manual` (recent CLI lifecycle stop, or not started by launchd), `launchd` (`XPC_SERVICE_NAME` is the provider LaunchAgent label: login/boot/kickstart) | Why this process started. |
...
The failure diagnostics page at `/app-attest/diagnostics` (linked from `/app-attest`, same window) groups macOS 27+ connections into the open failure cohorts: `is_supported_false`, fresh-key `apple_invalid_key`, dead-key assertion `apple_error`, stalled `busy` (only with an integer `operation_stalled_seconds` in 1–86400) and `identity_required`. It breaks each cohort down by the [provider diagnostics](#provider-diagnostics), the derived reboot/restart flags, OS build, provider version and chip family. It also classifies dead keys per day as OS change, reboot, process restart, no restart or unknown, reports rotation outcomes from `app_attest_key_rotations`, and summarises `push_history` for all OS versions. Rows from providers that predate these fields show as no data rather than zero. Coordinator-side push results are metrics only; see [APNs code identity](../architecture/security/attestation.md#flag--apns-code-identity). Code: `admin-ui/src/lib/queries/app-attest-diagnostics.ts`, `admin-ui/src/lib/app-attest-diagnostics.ts` and `admin-ui/src/app/app-attest/diagnostics/page.tsx`.

Rotation outcomes resolve each request's original machine scope through the bounded canonical-machine chain before matching replacement proofs. Sessions moved by a later merge still count toward that request's recovery; scope totals use current canonical identities. Account-fallback requests retain account matching. These diagnostic queries never rewrite rotation or inventory rows. Code: `admin-ui/src/lib/queries/app-attest-diagnostics.ts` (`appAttestRotationOutcomes`).
