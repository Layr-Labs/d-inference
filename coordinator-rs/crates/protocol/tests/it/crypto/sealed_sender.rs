...
fn vectors() -> Vectors {
    serde_json::from_slice(&support::read_vector_file("sealed_sender/vectors.json"))
        .expect("sealed_sender vectors parse")
}

fn nonce(hex: &str) -> [u8; NONCE_LEN] {
    support::from_hex(hex).try_into().expect("24-byte nonce")
}
...
fn coordinator_opens_go_sealed_request_and_reseals_identically() {
    let v = vectors();
    let coord_secret = SecretKey::from(support::hex32(&v.coordinator_private_key_hex));
    let coord_pub = parse_public_key(&v.coordinator_public_key_b64).unwrap();
    let client_secret = SecretKey::from(support::hex32(&v.client_private_key_hex));

    let (plaintext, sender_pub) =
        open_request(&v.request.envelope, &coord_secret, &v.kid).expect("open request");
    assert_eq!(plaintext, support::from_hex(&v.request.plaintext_hex));
    assert_eq!(
        sender_pub.as_bytes(),
...
fn client_opens_go_sealed_response_and_coordinator_reseals_identically() {
    ...
    assert_eq!(plaintext, support::from_hex(&v.response.plaintext_hex));
...
fn sse_line_round_trips_byte_identically() {
    ...
    assert_eq!(event, support::from_hex(&v.sse.event_hex));
