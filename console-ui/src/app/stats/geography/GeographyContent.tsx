...
import { formatGeoCount, type GeographyData } from "./geography-data";

export function GeographyContent({ data, exploring, onExploringChange }: {
  data: GeographyData;
  exploring: boolean;
  ...
  return (
    <>
      <div className="grid min-w-0 items-center lg:grid-cols-[minmax(0,1fr)_244px] xl:grid-cols-[minmax(0,1fr)_260px]">
          ...
          >
            {(context) => (
              <>
                ...
                <GeographyMarkers markers={data.markers} mode={mode} context={context} onExplore={() => onExploringChange(true)} />
          ...
          <div className="mt-1 flex min-h-9 items-center justify-between gap-3 px-3 sm:px-4">
            ...
            {data.markers.length > 0 && (
              <button
                ...
                onClick={() => onExploringChange(!exploring)}
        ...
        <GeographyRegions data={data} />
      </div>

      <GeographyPrivacyDetails data={data} />
    </>
  );
}

function GeographyRegions({ data }: { data: GeographyData }) {
  const { mode } = data;
...
function GeographyPrivacyDetails({ data }: { data: GeographyData }) {
  const { mode } = data;
  return (
