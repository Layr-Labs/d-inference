...
// mergeFleet builds a deduplicated list of providerView structs for an account
...
func (s *Controller) mergeFleet(ctx context.Context, accountID string) ([]providerView, error) {
	...
	out := make([]providerView, 0, len(deduped))
	for i := range deduped {
		...
		mp := buildProvider(&deduped[i], live)
		out = append(out, mp)
	...
	for id, p := range liveByID {
		...
		out = append(out, buildProvider(nil, p))
	}
	return out, nil
}
