...
For one release the gate is narrower than the registry. The slot lifecycle
still loads a checkpoint into an `MLXLMCommon.ModelContainer` rather than
calling `Runner.load`, so a `model_type` whose module the container path
cannot adapt is withheld
(`EngineV2ModelAdaptation.containerServableModelTypes`): `qwen3_5_text`,
`qwen4_exp` and `qwen4_exp_text` are claimed by runners and are not
advertised yet. Withholding keeps the 404 the advertise gate has always
returned instead of a load-then-503. The path is HTTP/WebSocket → `ProviderLoop` /
...
| Runner boundary (provider side) | Resolves a checkpoint through `RunnerRegistry`, loads it with `Runner.load`, injects load-time resources, selects the decoder from `loadedDecoders`, and hands the slot's policy to `runner.makeEngine` on `EngineBuild` | `provider-swift/Sources/ProviderCore/Inference/EngineV2RunnerBuild.swift` |
...
| Container-path model adaptation | The one typed family seam left in the provider: layer kinds, capabilities and `newCacheV2` for the families that still load through `ModelContainerLoading`. Deleted when the slot lifecycle loads through `Runner.load` | `provider-swift/Sources/ProviderCore/Inference/EngineV2ModelAdaptation.swift` |
...
| `qwen3_5_text` | Qwen 3.5 text target | Claimed by the Qwen 3.5 runner; not advertised yet (see the context above) |
| `qwen4_exp`, `qwen4_exp_text` | Qwen 3.8 Flash-Next 125B-A6B | Not advertised yet (see the context above). | Hybrid trunk, QSA keep mask, recurrent state. Contiguous KV only; MTP is the one engine capability the manifest sets. The n-gram PLE table is never model parameters: it is read from the checkpoint's shard directory through a load-time resource (`Qwen4ExpRunner.ngramRowSourceResource`) |
...
1. Nothing is advertised or loaded that no runner claims. The gate is
   `RunnerRegistry.contains(modelType:)` behind
   `EngineV2SupportedModels.isSupported`, intersected with
   `EngineV2ModelAdaptation.containerServableModelTypes` while the slot
   lifecycle still loads a `ModelContainer`; an unsupported load request
   fails the advertised-set guard (404) — `EngineV2SupportedModels.swift`.
