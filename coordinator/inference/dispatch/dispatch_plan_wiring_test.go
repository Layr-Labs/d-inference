...
func TestPlanFirstRetryConsumesPlanBeforeRescanAndRefreshesOnce(t *testing.T) {
	...
	for i := range 6 {
		planWiringProvider(t, s.deps.Registry(), fmt.Sprintf("pw%d", i), model, int64(i)*400)
	}
	plan := planWiringPlan(t, s.deps.Registry(), model)
...
func TestGovernorSuppressionFallsThroughToNoBackup(t *testing.T) {
	...
	busy := planWiringProvider(t, d.s.deps.Registry(), "busy-alt", d.model, 0)
	busy.Mu().Lock()
...
func TestGovernorAllowKeepsLegacyBackupPath(t *testing.T) {
	...
	planWiringProvider(t, d.s.deps.Registry(), "idle-backup", d.model, 0)
	...
	st, ok := testControllerStore(d.s).(*store.MemoryStore)
	if !ok {
		t.Fatalf("store = %T", testControllerStore(d.s))
	}
...
func TestSetLastInferenceErrorPrefersLiveBudgetAndKeepsFeasibleAfter(t *testing.T) {
	d := &execution{s: newTestController(t), model: "m", modelMaxContext: 8192}
...
func TestSetLastInferenceErrorExplicitZeroBudgetStaysTransient(t *testing.T) {
	...
	d2 := &execution{s: newTestController(t), model: "m", modelMaxContext: 8192}
...
func TestCollectCapacityQuotesRefinesOnlyOnHighConfidence(t *testing.T) {
	...
	for i := range 4 {
		planWiringProvider(t, s.deps.Registry(), fmt.Sprintf("cq%d", i), model, int64(i)*400)
	}
	receivedAt := time.Now()
