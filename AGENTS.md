...
- **DeepSeek-V4 serving is special-cased in four ways**: (1) routed experts stream from SSD (`DeepseekV4ExpertStreaming` + `backend.stream_experts`) — memory budgets MUST derive from `UnifiedMemoryCap`, never physical RAM (`ExpertStreamingAdmission`); (2) it serves sequentially (one request at a time, raw-text runner, heartbeat concurrency = 1) because the batched engine cannot represent its pooling caches — the batched cache factory silently substitutes `BatchKVCache` for unknown cache classes, producing garbage, not crashes; (3) prompts are encoded by the native `DeepseekV4Encoding` (no Jinja template exists upstream) and tool calls use the DSML parser (`ToolCallFormat.dsml`) — golden fixtures in `Tests/ProviderCoreFoundationTests` pin the official format; (4) `DeepseekV4Model` must implement `newCache(parameters:)` (not just a `makeCache` helper) or every generate path silently gets `KVCacheSimple`s.
- **M5 NAX kernels are nondeterministic** for some bf16 GEMM shapes. Releases build with `-Xcc -DMLX_METAL_NO_NAX` (`release-swift.yml`); debugging of "impossible" numeric flakiness on M5 hardware should start with `DSV4Smoke --op-stress`. See docs/spikes/nax-nondeterminism-m5.md.
- The expert cache is process-wide state in mlx-swift-lm — model unload must purge it (`BatchScheduler.stopCurrentEngine` does; keep it that way or idle-unload leaks tens of GB).
