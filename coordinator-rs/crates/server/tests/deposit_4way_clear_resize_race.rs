//! Concurrent deposit with 4-way clear after resize: money conserved.
...
#[test]
fn concurrent_deposit_with_4way_clear_after_resize_conserves() {
    ...
    let deposit = thread::spawn(move || {
        ...
        apply_stripe_deposit(&mut ib, &mut g, "stripe", "evt_4way_rz", "a", 160_000, 55_000)
            .map(|a| a)
            .unwrap_or(false)
    });
    ...
    let force = thread::spawn(move || {
        force_settle_held(&led_f, "j", "a", 85_000, "force-dep-4way-rz")
