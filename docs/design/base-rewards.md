...
**Status:** Implemented (v0.6.21, PR #282; status re-verified 2026-09-03 at `5d400cf75`) · Shipped as `coordinator/payments/baserewards/` (`floor.go`, `alloc.go`, `epoch.go`, `engine.go`) with settlement in `coordinator/store/postgres_base_rewards.go`, but the engine runs only when `EIGENINFERENCE_BASE_REWARDS=true` (default `false`, `coordinator/api/server_config.go`), so no floor draw is paid in a default deployment; both "required fixes" in §8 are in the schema — `provider_earnings(job_id)` has the unique partial index `idx_provider_earnings_job`, and `provider_sessions` carries a `provider_key` column indexed by `idx_provider_sessions_key` (`coordinator/store/postgres.go`) — and `provider_floor_draws` enforces `UNIQUE (provider_key, epoch_id)`. The as-built description is [`architecture/billing.md` → Base rewards](../architecture/billing.md#base-rewards-implemented-disabled-by-default); constants and the tier table are in [`reference/pricing-model.md` → Base rewards](../reference/pricing-model.md#base-rewards). The UI surfacing described in this document is NOT built: `console-ui/src/components/earn/BaseRewardsPanel.tsx` is not mounted by any page, the `/api/admin/base-rewards` relay (`console-ui/src/app/api/admin/base-rewards/route.ts`) has no caller, and the `/earn` calculator (`console-ui/src/app/earn/calc.ts`) and the landing calculator exclude base rewards from their estimates. · **Original date:** 2026-06-06 · **Owner:** Coordinator / Payments
