...
> Last updated: 2026-09-25 · commit `13c06040c`
...
the admin UI suite too, with two exceptions. The benchmark-wrapper tests run
only locally, through `make benchmark-wrapper-test`. The landing tests run in CI
only when `landing/` changes (see the CI workflow map). The e2e suite needs an
Apple Silicon Mac with the test checkpoints cached.
