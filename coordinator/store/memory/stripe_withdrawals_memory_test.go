...
func TestMemoryStripeWithdrawalFeeRefundedRoundTrip(t *testing.T) {
	...
	if _, err := s.GetStripeWithdrawalByPayoutID("po_rt"); !errors.Is(err, contracts.ErrNotFound) {
...
func TestMemoryStripeWithdrawalLookupNotFound(t *testing.T) {
	s := New(contracts.Config{})
	if _, err := s.GetStripeWithdrawalByPayoutID("po_missing"); !errors.Is(err, contracts.ErrNotFound) {
		t.Errorf("payout lookup err = %v, want ErrNotFound", err)
	}
	if _, err := s.GetStripeWithdrawalByTransferID("tr_missing"); !errors.Is(err, contracts.ErrNotFound) {
...
func TestMemoryCreditWithdrawableOnce(t *testing.T) {
	...
	applied, err := s.CreditWithdrawableOnce("acct-once", 500_000, contracts.LedgerRefund, "stripe_withdraw_fee:wd-1")
	if err != nil || !applied {
	...
	if applied, _ = s.CreditWithdrawableOnce("acct-once", 4_500_000, contracts.LedgerRefund, "stripe_withdraw:wd-1"); !applied {
	...
	if applied, _ = s.CreditWithdrawableOnce("acct-once", 100, contracts.LedgerDeposit, "stripe_withdraw:wd-1"); !applied {
...
func TestMemoryCreateStripeWithdrawalWithDebit(t *testing.T) {
	s := New(contracts.Config{})
	if err := s.CreditWithdrawable("acct-wdb", 10_000_000, contracts.LedgerPayout, "earnings"); err != nil {
		t.Fatal(err)
	}
	...
	if err := s.CreateStripeWithdrawalWithDebit(wd, contracts.LedgerStripePayout, "stripe_withdraw:wd-atomic-1"); err != nil {
	...
	err = s.CreateStripeWithdrawalWithDebit(wd2, contracts.LedgerStripePayout, "stripe_withdraw:wd-atomic-2")
	...
	if err := s.CreateStripeWithdrawalWithDebit(dup, contracts.LedgerStripePayout, "stripe_withdraw:dup"); err == nil {
...
func TestMemoryMarkStripeWithdrawalPaidGuards(t *testing.T) {
	...
	mk := func(id, status string, refunded bool) {
		if err := s.CreateStripeWithdrawal(&contracts.StripeWithdrawal{
	...
	if _, err := s.MarkStripeWithdrawalPaid("wd-mp-missing", "", ""); !errors.Is(err, contracts.ErrNotFound) {
...
func TestMemoryReopenStripeWithdrawalAfterPayoutFailureGuards(t *testing.T) {
	...
	if _, err := s.GetStripeWithdrawalByPayoutID("po_ro1"); !errors.Is(err, contracts.ErrNotFound) {
		t.Errorf("detached payout lookup err = %v, want ErrNotFound", err)
	}
