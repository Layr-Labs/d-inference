...
| `PROFILE_SIGNING_P12_B64` / `PROFILE_SIGNING_P12_PASSWORD` | `eigeninference-profile-signing-p12-b64` / `…-password` | Developer ID Application identity (base64 PKCS#12 + password) used to CMS-sign the `/v1/enroll` profile. Hardware-trust startup requires it unless the dev-only `EIGENINFERENCE_ALLOW_UNSIGNED_PROFILES=true` escape hatch is set. |
