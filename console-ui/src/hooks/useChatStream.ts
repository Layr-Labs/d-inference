...
- Providers are authorized through complete legacy verification or, when enabled and qualified, an independent App Attest path on eligible macOS 27+ machines. App Attest does not require MDM and does not imply legacy MDA/APNs verification
- Legacy hardware trust uses Secure Enclave attestation and MDM security checks. Apple Managed Device Attestation (MDA) is a separate certificate proof, reported by mda_verified when verified
...
- Signed provider builds use macOS Hardened Runtime and debugger restrictions. Legacy security-posture evidence and qualified App Attest authorization are separate checks
