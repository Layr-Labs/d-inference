...
func insertFlowProvider(t *testing.T, s *Store, id string, location any) {
	t.Helper()
...
func assertUsageFlowsMatchOriginal(t *testing.T, s *Store, since time.Time) []contracts.UsageFlowBucket {
	t.Helper()
	...
	defer rows.Close()
	key := func(b contracts.UsageFlowBucket) string {
	...
	want := make(map[string]contracts.UsageFlowBucket)
	for rows.Next() {
