...
func TestPostgresLedgerOnceLookupUsesAllIdentityColumns(t *testing.T) {
	...
	defer pool.Close()
	writer := &PostgresStore{pool: pool}
	if applied, err := writer.CreditOnce(account, 100, LedgerStripeDeposit, "stripe:point-lookup"); err != nil || !applied {
...
func TestPostgresLedgerOnceIndexPreservesLegacyReferences(t *testing.T) {
	...
	if err := s.Credit(account, 10, LedgerAdminCredit, reference); err != nil {
		t.Fatal(err)
	}
	for range 2 {
		if err := s.Credit(account, 100, LedgerStripeDeposit, "stripe:legacy-duplicate"); err != nil {
			t.Fatal(err)
		}
	}
	var indexOID uint32
	...
	for _, identity := range []struct {
		...
		{LedgerAdminCredit, reference},
		{LedgerStripeDeposit, "stripe:legacy-duplicate"},
	} {
	...
	if applied, err := s.CreditOnce(account, 20, LedgerAdminCredit, reference+":different"); err != nil || !applied {
