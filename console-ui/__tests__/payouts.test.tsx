...
describe("useStripePayouts", () => {
  ...
  it("withdraw() submits, reloads, toasts, and fires analytics", async () => {
    ...
    const { result } = renderHook(() =>
      useStripePayouts({ addToast, enabled: true, onAfterWithdraw, onWithdrawStart, onWithdrawSuccess }),
    );
  ...
  it("withdraw() surfaces errors and fires the error analytics", async () => {
    ...
    const { result } = renderHook(() =>
      useStripePayouts({ addToast, enabled: true, onWithdrawError }),
    );
    await waitFor(() => expect(result.current.status?.status).toBe("ready"));
  ...
  it("withdraw() on stripe_account_gone shows friendly copy, closes the modal, and refreshes status", async () => {
    ...
    (fetchStripeStatus as ReturnType<typeof vi.fn>).mockResolvedValueOnce(readyStatus).mockResolvedValue({
      ...readyStatus,
      has_account: false,
      status: "",
    });
    ...
    const addToast = vi.fn();
    const { result } = renderHook(() => useStripePayouts({ addToast, enabled: true }));
    await waitFor(() => expect(result.current.status?.status).toBe("ready"));
