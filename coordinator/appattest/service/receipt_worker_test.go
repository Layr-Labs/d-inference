...
func TestAppAttestReceiptRenewalBoundsAndAuthentication(t *testing.T) {
	...
	r := renewAppAttestReceipt(context.Background(), old, Config{ReceiptKeyPath: path, ReceiptKeyID: "TESTKEY"}, client)
...
func TestAppAttestReceiptWorkerRenewsWhenShadowDisabled(t *testing.T) {
	...
	for _, cfg := range []Config{{ReceiptKeyPath: path}, {ReceiptKeyID: "TESTKEY"}} {
		s.config = cfg
