...
public enum MemoryLimit {
    ...
    /// a hand-edited TOML below it is clamped UP to this floor rather than
    /// treated as unset — the operator asked for a cap, so dropping it would
    /// invert their intent, while honoring e.g. 2 GB leaves no room for the
    /// ~6.5 GiB load headroom and silently blackholes the node (advertises
    /// models, rejects every load). 8 GB is the smallest cap that can hold
    ...
    /// and `StandaloneServer`, so no serving mode can hold back less than
    /// another — the review-flagged failure was exactly local mode honoring
    /// only the limit while a larger `memory_reserve_gb` was configured.
