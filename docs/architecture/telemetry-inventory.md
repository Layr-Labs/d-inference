...
| Sidecar plan latency | `registry/cache_route_keys.go:167-174` (mode=on); `api/prompt_artifacts.go` `planCacheRoute` (mode=off: `outcome=off` recorded before the artifact-status, preload-gate and `Registry.mu` reads it skips) | DD-C `exact_cache.plan{outcome}` / `exact_cache_plan_total{outcome}`, DD-H `exact_cache.plan_latency_ms` (`api/exact_cache_telemetry.go:13-34`, no model/request tag) | every planned request: `outcome=off` once per request while the gate is dark (`CACHE_ROUTING_MODE=off`, prod since 2026-07-17) — since T6-02 that bucket also absorbs the requests previously unrecorded at mode=off (no ready artifact, media requests, preloader not ready), so its rate stepped up at that deploy; `plan_latency_ms` only when the sidecar was called |
...
| Routing decision (`RoutingDecision` `registry/scheduler.go:288-352`) | `recordRoutingDecisionFor` `dispatch.go:355-480` | row: `outcome, cost_ms, state_ms, queue_ms, pending_ms, backlog_ms, this_req_ms, health_ms, ttft_ms, best_ttft_ms, effective_queue, candidate_count, {capacity,model_too_large,vision,ttft}_rejections, effective_tps, static_tps` + winner snapshot under `provider.Mu()` `:425-456` (`provider_status, trust, version, hardware_chip/family/tier, memory_gb, gpu/cpu_cores, system_*, gpu_memory_*, slot_state, backend_running/waiting, active_token_budget_used/max, queued_token_budget`) + request shape; DD-C `routing.decisions`, `routing.provider_selected{provider_id,model}` `dispatch.go:3088-3089`; log `inference request dispatched{trace_id,request_id,model,provider_id,stream,attempt}` `:3091-3098`. **`health_ms` is not comparable across the T13-01 deploy:** the GPU active-memory term (`gpuActive/total × 5000`, typically 1–4 s on any box holding a model) is priced at 0 from that build on, so `health_ms` and `cost_ms` drop by seconds at the boundary with no provider change — split cost decompositions on the deploy timestamp | per attempt |
