...
func (s *Store) CreateReferrer(accountID, code string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) GetReferrerByCode(code string) (*contracts.Referrer, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) GetReferrerByAccount(accountID string) (*contracts.Referrer, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) RecordReferral(referrerCode, referredAccountID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) GetReferrerForAccount(accountID string) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) GetReferralStats(code string) (*contracts.ReferralStats, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	for _, entry := range s.ledgerEntries {
		if entry.AccountID == ref.AccountID && entry.Type == contracts.LedgerReferralReward {
			totalRewards += entry.AmountMicroUSD
		}
	}

	return &contracts.ReferralStats{
		Code:                 code,
