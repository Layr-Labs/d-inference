...
func (s *Store) SumProviderEarningsByKey(ctx context.Context, providerKey string, since, until time.Time) (int64, error) {
	var total int64
...
func (s *Store) SettleProviderFloorDraw(ctx context.Context, draw *contracts.ProviderFloorDraw) (bool, error) {
	if draw == nil {
	...
	err := s.pool.QueryRow(ctx, `
		...
		draw.UptimeFrac,                   // $7
		draw.MemoryGB,                     // $8
		string(contracts.LedgerFloorDraw), // $9
		earningJobID,                      // $10
	).Scan(&credited)
	if err != nil {
...
func (s *Store) SumFloorDrawsForEpoch(ctx context.Context, epochID string) (int64, error) {
	var total int64
	err := s.pool.QueryRow(ctx,
...
func (s *Store) ListFloorDrawsForEpoch(ctx context.Context, epochID string) ([]contracts.ProviderFloorDraw, error) {
	...
	for rows.Next() {
		var d contracts.ProviderFloorDraw
...
func (s *Store) ListProviderSessionsOverlapping(ctx context.Context, start, end time.Time, openSessionGrace time.Duration) ([]contracts.ProviderSession, error) {
	...
	for rows.Next() {
		var ps contracts.ProviderSession
...
func (s *Store) WithEpochSettlementLock(ctx context.Context, epochID string, fn func() error) error {
