...
| `EIGENINFERENCE_APP_ATTEST_KEY_ROTATION_PERCENT` | `100`; account cohort for coordinator-requested dead-key rotation. After `keyRotationFailureThreshold = 2` DeviceCheck code 0/2 or uncoded `apple_error` assertion failures since the key's last verified assertion, a protocol ≥ 2 `ready` for that key gets `attest`, at most one per machine per hour and four per 24 hours. The next attempt waits 15–45 seconds. Invalid values disable rotation with a `rotation/configuration_error` observation | `coordinator/appattest/service/key_rotation.go` (`maybeRequestKeyRotation`); [rotation flow](app-attest-shadow.md#dead-key-rotation) |
| Fresh-key enrollment failures | An `attestation` failing with `apple_invalid_key` when the canonical machine (else account) has at least three in the trailing 24 hours waits `enrollmentInvalidKeyBackoff = 6 * time.Hour` before the next attempt, including in a new session after a reconnect or coordinator restart | `coordinator/appattest/service/enrollment_backoff.go` (`enrollmentBackoffDue`, `enrollmentBackoffRemaining`); [backoff](app-attest-shadow.md#invalid-key-enrollment-backoff) |
