...
// ChunkCh and ErrorCh.
func (d *execution) commitReadyFirstContent(
	pr *registry.PendingRequest,
	held *[]string,
...
func (d *execution) firstTokenRemaining() (remaining time.Duration, ok bool) {
...
func (d *execution) firstTokenWait(relativeFallback time.Duration) time.Duration {
...
func (d *execution) firstTokenExpired() bool {
	remaining, ok := d.firstTokenRemaining()
	return ok && remaining == 0
}

func (d *execution) canExtendPreambleLiveness() bool {
...
func (d *execution) firstTokenSpeculativeWait() time.Duration {
	remaining, ok := d.firstTokenRemaining()
	if !ok {
...
func (d *execution) abandonInflightForFirstTokenTimeout() bool {
	if d == nil {
		return true
	}
	...
	if !d.s.deps.Attempts().CancelForFirstContentTimeout(provider, pr) {
		return false
	}
	d.setLastError("timeout waiting for first response", http.StatusGatewayTimeout)
	d.excludeProviders[provider.ID] = struct{}{}
	d.s.deps.Registry().RecordWarmPoolTTFTMiss(d.model, d.deadline)
	...
	if d.s.deps.Metrics() != nil {
		d.s.deps.Metrics().IncCounter("inference_dispatches_total", metrics.Label{Name: "result", Value: "timeout"})
	}
	d.s.deps.Counters.Incr("inference.dispatches", []string{"status:timeout"})
	d.provider = nil
	d.pr = nil
	return true
}
