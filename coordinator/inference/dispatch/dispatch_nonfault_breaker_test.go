...
func newBreakerExemptionHarness(t *testing.T, name string) (*Controller, *registry.Registry, *registry.Provider, *registry.PendingRequest) {
	t.Helper()
	logger := slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{Level: slog.LevelError}))
	st := store.NewMemory(store.Config{AdminKey: "test-key"})
	reg := registry.New(logger)
	srv := newConfiguredController(reg, st, logger)
...
func TestNoteDispatchRetry_JinjaSkipsProviderFaultBreakers(t *testing.T) {
	...
	d := &execution{s: srv, model: pr.Model}
