...
func testPostgresStoreWithTracer(t *testing.T, configure ...func(*pgxpool.Config)) (*Store, *statementTracer) {
	t.Helper()
	...
	s, err := newPostgresWithPoolConfig(ctx, contracts.Config{DatabaseURL: dbURL}, func(cfg *pgxpool.Config) {
...
func TestUsageAnalyticsRunInWorkMemTransaction(t *testing.T) {
	...
	if err := s.UpsertProvider(ctx, contracts.ProviderRecord{
		ID:       providerID,
