...
# load-bearing:
#
#   paged      — `auto` is a SELECTION, not an outcome: it degrades to
#                contiguous on kill switch, kernel preflight, or pool
#                capacity, so an `auto` run can report green while measuring
#                the rollback backend. An explicit `paged` refuses instead of
#                degrading, which is the only way this wrapper can promise it
#                measured what it says it measured.
