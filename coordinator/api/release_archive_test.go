...
func TestReleaseArchiveCountsHeadersAndTrailerInExpandedLimit(t *testing.T) {
	archive := buildRawReleaseArchiveForTest(
		rawReleaseTarEntry{name: "file", typeflag: '0'},
	)
	archive = append(archive, make([]byte, releaseTarBlockSize)...)
	policy := defaultReleaseArchivePolicy
	policy.maxExpandedBytes = int64(len(archive) - releaseTarBlockSize)
