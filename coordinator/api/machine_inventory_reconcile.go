...
func (s *Server) startMachineInventoryReconciler(ctx context.Context) {
...
func (s *Server) reconcileMachineInventory(ctx context.Context, st store.MachineInventoryReconcileStore) {
	operation, cancel := context.WithTimeout(ctx, 2*time.Second)
	defer cancel()
	if s.machineInventorySlots != nil {
		select {
		case s.machineInventorySlots <- struct{}{}:
			defer func() { <-s.machineInventorySlots }()
		case <-operation.Done():
