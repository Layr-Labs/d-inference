...
func TestCreditProviderAccount_DuplicateJobNoop(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			acct := uniqueID("acct")
			earning := &contracts.ProviderEarning{
				AccountID:        acct,
				ProviderID:       "prov-1",
...
func TestSettleProviderFloorDraw_Idempotent(t *testing.T) {
	...
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			acct := uniqueID("acct")
			epoch := "2026-05"
			draw := &contracts.ProviderFloorDraw{
				ProviderKey:    pk,
				AccountID:      acct,
				EpochID:        epoch,
			...
			floorEntries := 0
			for _, e := range h {
				if e.Type == contracts.LedgerFloorDraw {
					floorEntries++
				}
			}
			if floorEntries != 1 {
...
func TestSumProviderEarningsByKey_Filters(t *testing.T) {
	...
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			mk := func(model string, amt int64, at time.Time) *contracts.ProviderEarning {
				return &contracts.ProviderEarning{
					AccountID:      acct,
					ProviderID:     "prov",
					ProviderKey:    pk,
					JobID:          uniqueID("job"),
...
func TestSettleProviderFloorDraw_RecordsVisibleEarning(t *testing.T) {
	...
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			acct := uniqueID("acct")
			epoch := "2026-07"
			if _, err := s.SettleProviderFloorDraw(ctx, &contracts.ProviderFloorDraw{
...
func unionCoveredSeconds(sessions []contracts.ProviderSession, serial string, start, end time.Time) float64 {
...
func countSessions(sessions []contracts.ProviderSession, serial string) int {
	n := 0
	for _, ps := range sessions {
		if ps.SerialNumber == serial {
			n++
