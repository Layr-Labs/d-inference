...
| 15 | Settle: charge the account from provider-reported usage, record usage, credit the provider | `handleCompleteAt` (`coordinator/api/provider.go`): `claimSettlement`, `ledger.Charge`, `store.RecordUsage`, `store.CreditProviderAccount`; rules in [`../billing.md`](../billing.md) | — |
