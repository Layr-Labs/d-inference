...
func TestMDMSchedulerRetiredWorkerCannotReleaseReplacementClaim(t *testing.T) {
	srv, st, sch := newSchedulerTestServer(t, MDMSchedulerConfig{Workers: 2, QueueCapacity: 8}, mdmSchedulerDeps{
	...
	claim := func() mdmSchedulerWork {
		...
		case <-time.After(time.Second):
			...
			return mdmSchedulerWork{}
		}
	}
	old := schedulerTestProvider(t, srv, "retired-worker-old", seKey)
	...
	replacement := schedulerTestProvider(t, srv, "retired-worker-new", seKey)
	...
	newWork := claim()
	sch.finishAttempt(oldWork, mdmSchedulerAttemptResult{outcome: store.VerificationOutcomeCancelled})
	...
	sch.finishAttempt(newWork, mdmSchedulerAttemptResult{outcome: store.VerificationOutcomeCancelled})
}
...
func TestMDMSchedulerSettlementKeepsReboundVerification(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			key := verificationSchedulerKey(seKey, store.VerificationTaskSecurityInfo)
			old := schedulerTestProvider(t, srv, "settling-old", seKey)
			sch.Submit(context.Background(), old.ID, old, store.VerificationPriorityFirstOrExpired)
			sch.ChallengeSettled(old, false)
			sch.claimAndDispatch(key, time.Now().UTC())
			...
			go func() {
				defer close(done)
				sch.finishAttempt(work, mdmSchedulerAttemptResult{outcome: store.VerificationOutcomeTransient, terminal: tc.complete})
			}()
			...
			replacement := schedulerTestProvider(t, srv, "settling-replacement", seKey)
			generation := sch.Submit(context.Background(), replacement.ID, replacement, store.VerificationPriorityFirstOrExpired)
			sch.ChallengeSettled(replacement, false)
			unblock()
			<-done
...
func TestMDMSchedulerReboundReadCannotOverwriteNewerSettlement(t *testing.T) {
	...
	unblock := func() { releaseOnce.Do(func() { close(st.release) }) }
	t.Cleanup(unblock)
	srv, sch := newSchedulerTestServerWithStore(t, st, MDMSchedulerConfig{Workers: 2, QueueCapacity: 8}, mdmSchedulerDeps{
		jitter: func(minimum, _ time.Duration) time.Duration { return minimum },
	})
	withoutLiveDispatcher(sch)
	...
	old := schedulerTestProvider(t, srv, "rebound-read-old", seKey)
	sch.Submit(context.Background(), old.ID, old, store.VerificationPriorityFirstOrExpired)
	sch.ChallengeSettled(old, false)
	oldWork := claim()
	replacement := schedulerTestProvider(t, srv, "rebound-read-new", seKey)
	generation := sch.Submit(context.Background(), replacement.ID, replacement, store.VerificationPriorityFirstOrExpired)
	sch.ChallengeSettled(replacement, false)
	done := make(chan struct{})
	go func() {
	...
	newWork := claim()
	sch.finishAttempt(newWork, mdmSchedulerAttemptResult{outcome: store.VerificationOutcomeTransient})
	unblock()
	<-done
