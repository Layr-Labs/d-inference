      ...
      {expanded && (
        <div id="provider-filters" className="mt-3 grid gap-3 rounded-xl bg-bg-secondary p-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="grid gap-2 text-xs text-text-tertiary">Trust<select value={filters.trust} onChange={(event) => onChange({ trust: event.target.value as ProviderTrustFilter })} aria-label="Filter by trust" className={SELECT_STYLE}>
            <option value="all">All verification states</option><option value="verified">Currently verified</option><option value="app_attest">App Attest</option><option value="legacy">Legacy authorized</option><option value="dual">Both methods</option><option value="hardware">Legacy hardware evidence</option><option value="basic">Not currently verified</option>
          </select></label>
