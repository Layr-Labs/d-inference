...
func TestGlobalPayoutManualReviewDoesNotCrowdReconciliation(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if err := g.ApplyGlobalPayout(old.ID, contracts.GlobalPayoutResult{FailureCode: contracts.GlobalPayoutManualReview}, now); err != nil {
				t.Fatal(err)
			}
			...
			if err := g.ApplyGlobalPayout(old.ID, contracts.GlobalPayoutResult{ExternalID: "obp_found", Status: "processing"}, now); err != nil {
				t.Fatal(err)
			}
