...
func (r *Manager[C]) Bind(p *Session[C], stableID string, version string) {
	if p == nil || p.id == "" {
		return
	}
	now := r.now()
	...
	if cur != nil && cur.key == targetKey {
		if stableID != "" {
			cur.mu.Lock()
			noteIdentityVersionLocked(cur, r, version)
	...
	if stableID != "" {
		target.mu.Lock()
		noteIdentityVersionLocked(target, r, version)
...
func (r *Manager[C]) migrateGateLocked(p *Session[C], src, dst *gateState, orphan bool) {
	if src == dst {
		p.gate.Store(dst)
		return
	}
