	...
	for _, banned := range []string{
		...
		if bytes.Contains(src, []byte(banned)) {
			t.Fatalf("DAR-349 regression: boot-time provider_earnings dedupe reintroduced "+
				"(found %q in startup schema sources); move destructive cleanup to an offline job", banned)
		}
	}
}
...
func jobIndexValid(t *testing.T, s *Store) bool {
	t.Helper()
	var valid bool
...
func countEarningsByJob(t *testing.T, s *Store, job string) int {
	t.Helper()
	var n int
	if err := s.pool.QueryRow(context.Background(),
