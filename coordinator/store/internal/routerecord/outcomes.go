...
func SplitInferenceRouteBatches(records []*contracts.InferenceRouteRecord, maxRows int) [][]*contracts.InferenceRouteRecord {
	var out [][]*contracts.InferenceRouteRecord
	var cur []*contracts.InferenceRouteRecord
...
func MergeInferenceRouteOutcome(dst *contracts.InferenceRouteOutcome, src *contracts.InferenceRouteOutcome) {
	if dst == nil || src == nil {
		return
	}
	if src.FinalStatus != "" {
...
func ApplyInferenceRouteOutcomeToRecord(rec *contracts.InferenceRouteRecord, outcome contracts.InferenceRouteOutcome) {
	if rec == nil {
		return
	}
