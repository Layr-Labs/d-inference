...
func (s *Controller) recordToolConstraintMetric(mode toolpolicy.Mode, outcome string) {
	if mode == "" {
		mode = "invalid"
	}
	s.deps.Metrics.Incr("inference.tool_constraint", []string{
		"mode:" + string(mode),
		"outcome:" + outcome,
	})
}
...
func writeToolConstraintValidationError(
	...
	if typed, ok := err.(*toolpolicy.ValidationError); ok {
		options := []httpresponse.ErrorDetail{}
		if typed.Param != "" {
			options = append(options, httpresponse.WithParam(typed.Param))
		}
		status := http.StatusBadRequest
		if typed.Kind == toolpolicy.UnsupportedSchema {
			status = http.StatusUnprocessableEntity
		}
		httpresponse.WriteJSON(w, status, httpresponse.ErrorBody(
