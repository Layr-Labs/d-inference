...
func TestCacheLookupRejectionReasonsPreserveFences(t *testing.T) {
	for _, tc := range []struct {
		...
		mismatch bool
		change   func(*testDirectory, *protocol.PrefixCacheLookupV2Message)
	}{
		{"missing attempt", CacheReceiptAttemptUnavailable, false, func(t *testDirectory, m *protocol.PrefixCacheLookupV2Message) { m.CacheReceiptNonce = "unknown" }},
		{"wrong request", CacheReceiptAttemptBinding, false, func(t *testDirectory, m *protocol.PrefixCacheLookupV2Message) { m.RequestID = "other" }},
		{"identity", CacheReceiptIdentityMismatch, true, func(t *testDirectory, m *protocol.PrefixCacheLookupV2Message) {
			m.ModelAggregateHash = strings.Repeat("f", 64)
		}},
		{"prompt", CacheReceiptPromptMismatch, true, func(t *testDirectory, m *protocol.PrefixCacheLookupV2Message) {
			m.PromptAnchor.ChainHash = strings.Repeat("f", 64)
		}},
		{"sequence", CacheReceiptSequence, false, func(t *testDirectory, m *protocol.PrefixCacheLookupV2Message) { m.CacheSeq = 0 }},
		{"invalid shape", CacheReceiptInvalid, false, func(t *testDirectory, m *protocol.PrefixCacheLookupV2Message) { m.StageMs = -1 }},
		{"duplicate lookup", CacheReceiptDuplicateLookup, false, func(t *testDirectory, m *protocol.PrefixCacheLookupV2Message) {
			a := t.attempts[m.CacheReceiptNonce]
			a.LookupSeen = true
			t.attempts[m.CacheReceiptNonce] = a
		}},
		{"capability change", CacheReceiptCapabilityChanged, false, func(t *testDirectory, m *protocol.PrefixCacheLookupV2Message) {
			a := t.attempts[m.CacheReceiptNonce]
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			d := tracker.ApplyLookup("provider", nil, cap, msg, []byte("route"), time.Now())
...
func TestCacheReadyReportsMissingLookupWithoutPoisoningValidReceipt(t *testing.T) {
	...
	prompt := protocol.PrefixCacheAnchor{ChainHash: strings.Repeat("c", 64), TokenCount: int(cap.BlockSize)}
	testV2Attempt(tracker, "nonce", cap, prompt)
	d := tracker.ApplyReady("provider", nil, cap, testV2Ready("nonce", cap, prompt, 2), []byte("route"), time.Now())
...
func TestCachePromptMismatchDiagnosticsPreserveExactRejection(t *testing.T) {
	for _, tc := range []struct {
		...
		got := tracker.ApplyLookup("provider", nil, cap, msg, []byte("route"), time.Now())
