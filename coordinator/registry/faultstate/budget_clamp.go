...
// (ejection.go) both require ZERO interleaved accepts, and each accept
...
//
// Lives on the STABLE fault identity's gate (state.go: serial → SE-key →
...
func (r *Manager[C]) BudgetClamped(providerID, modelID string, heartbeatAt time.Time, rawBudgetRemaining int64, budgetReported bool, now time.Time) bool {
...
func (r *Manager[C]) ReleaseBudgetClampsOnHeartbeat(providerID string, heartbeatAt time.Time, capacity *protocol.BackendCapacity) {
