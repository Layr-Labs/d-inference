...
func globalPayoutAPIFixture(t *testing.T, failFirst bool) (*controllerFixture, *store.MemoryStore, *store.User, *fakeGlobalStripe) {
	t.Helper()
	srv, st := stripePayoutsTestServer(t, true, nil)
	srv.SetBilling(billingservice.NewService(st, srv.billing().Ledger(), srv.logger, billingservice.Config{MockMode: true, StripeConnectReturnURL: "https://app.test/billing", StripeGlobalPayoutsEnabled: true, StripeGlobalPayoutsFinancialAccount: "fa_gp", StripeGlobalPayoutsSecretKey: "rk_test_gp", StripeGlobalPayoutsWebhookSecret: "whsec_test"}))
...
func globalAPIRequest(t *testing.T, s *controllerFixture, u *store.User, path, body string, handler http.HandlerFunc) *httptest.ResponseRecorder {
	t.Helper()
...
func TestGlobalPayoutOnboardQuoteConfirmAndReturn(t *testing.T) {
	...
	w := globalAPIRequest(t, s, u, "/onboard", `{"country":"IN"}`, s.StripeOnboard)
	if w.Code != 200 {
	...
	w = globalAPIRequest(t, s, u, "/quote", `{"amount_usd":"10.00"}`, s.GlobalPayoutQuote)
	if w.Code != 200 {
	...
	w = globalAPIRequest(t, s, u, "/withdraw", body, s.StripeWithdraw)
	if w.Code != 202 {
...
func TestGlobalPayoutRejectsInvalidQuotesAndCountries(t *testing.T) {
	...
	for _, country := range []string{"CN", "KH", "GI"} {
		w := globalAPIRequest(t, s, u, "/onboard", `{"country":"`+country+`"}`, s.StripeOnboard)
		if w.Code != 400 {
...
func TestGlobalPayoutAmbiguousSendThenPermissionLossDoesNotRefund(t *testing.T) {
	s, st, u, f := globalPayoutAPIFixture(t, true)
	globalAPIRequest(t, s, u, "/onboard", `{"country":"IN"}`, s.StripeOnboard)
	w := globalAPIRequest(t, s, u, "/quote", `{"amount_usd":"10"}`, s.GlobalPayoutQuote)
	var q struct {
		ID string `json:"id"`
	}
	_ = json.Unmarshal(w.Body.Bytes(), &q)
	body := `{"amount_usd":"10","quote_id":"` + q.ID + `"}`
	globalAPIRequest(t, s, u, "/withdraw", body, s.StripeWithdraw)
	f.mu.Lock()
...
func TestGlobalPayoutPausePreservesReconciliation(t *testing.T) {
	...
	s.SetBilling(billingservice.NewService(st, s.billing().Ledger(), s.logger, billingservice.Config{MockMode: true, StripeGlobalPayoutsFinancialAccount: "fa_gp", StripeGlobalPayoutsSecretKey: "rk_test_gp", StripeGlobalPayoutsWebhookSecret: "whsec_test"}))
...
func TestGlobalPayoutWebhookUsesCurrentStateAndSignature(t *testing.T) {
	...
	_ = json.Unmarshal(w.Body.Bytes(), &q)
	globalAPIRequest(t, s, u, "/withdraw", `{"amount_usd":"10","quote_id":"`+q.ID+`"}`, s.StripeWithdraw)
	payload := []byte(`{"type":"v2.money_management.outbound_payment.returned","related_object":{"id":"obp_gp"}}`)
	unsigned := httptest.NewRecorder()
	s.GlobalPayoutWebhook(unsigned, httptest.NewRequest("POST", "/", strings.NewReader(string(payload))))
	if unsigned.Code != 400 {
		t.Fatalf("unsigned event accepted: %d", unsigned.Code)
	}
	signed := httptest.NewRecorder()
	s.GlobalPayoutWebhook(signed, signedConnectRequest(t, payload, "whsec_test"))
	if signed.Code != 200 {
