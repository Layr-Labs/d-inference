...
// RaceTerminalFactory connects these private dispatch assertions to the real
// provider-frame owner from the external test package. That owner depends on
// dispatch, so importing it here would introduce a test import cycle.
type RaceTerminalFactory func(*registry.Registry, attempt.Service, *slog.Logger) func(*registry.Provider, *protocol.InferenceErrorMessage)
...
func CheckRaceClosedErrorKeepsSurvivingAttempt(t *testing.T, newTerminal RaceTerminalFactory) {
	for _, failedPrimary := range []bool{true, false} {
		...
		t.Run(name, func(t *testing.T) {
			...
			newTerminal(d.s.deps.Registry(), d.s.deps.Attempts(), d.s.deps.Logger())(failed, &protocol.InferenceErrorMessage{
...
func CheckRunRaceQueuedErrorKeepsSurvivingAttempt(t *testing.T, newTerminal RaceTerminalFactory) {
	for _, failedPrimary := range []bool{true, false} {
		name := "backup_failed"
		if failedPrimary {
			name = "primary_failed"
...
func CheckRaceChunkWinnerStillCancelsLoser(t *testing.T, newTerminal RaceTerminalFactory) {
		...
		for _, terminal := range []string{"content", "empty_completion", "content_then_error"} {
			t.Run(winnerName+"/"+terminal, func(t *testing.T) {
				...
				if terminal == "content_then_error" {
					...
					newTerminal(d.s.deps.Registry(), d.s.deps.Attempts(), d.s.deps.Logger())(winner, &protocol.InferenceErrorMessage{
