...
> Last updated: 2026-09-19 · commit `42876a117`
...
| `challenge_max_age_seconds` | integer | `challengeFreshnessMaxAge` in seconds; the `stale_challenge` window for legacy-attested machines. A machine with a current App Attest authorization is never `stale_challenge` (`providerChallengeFreshAtLocked`) |
...
excluded from the network-wide aggregates (`warmPoolOnlyModels`). The same two
figures reach Datadog on the `warm_pool_tick` event, with the tally flattened to
scalar `cold_disq_<reason>` attributes
([`telemetry-inventory.md`](telemetry-inventory.md)).
