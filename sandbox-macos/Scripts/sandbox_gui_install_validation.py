...
from sandbox_gui_host_plan import UNVERIFIED, installed_job_path, launch_agent
...
def validate_installed_job(path, expected):
    ...
    try:
        ...
        if (not stat.S_ISREG(before.st_mode) or before.st_uid != 0 or before.st_nlink != 1
                or stat.S_IMODE(before.st_mode) != 0o644 or not 0 < before.st_size <= 131072):
            raise ValueError("GUI job definition must be root-owned, single-link and mode 0644")
        require_no_extended_acl(path)
        data = bytearray()
        ...
        if (identity(before) != identity(after) or identity(before) != identity(named)
                or len(data) != before.st_size or not _exact_value(plistlib.loads(data), expected)):
