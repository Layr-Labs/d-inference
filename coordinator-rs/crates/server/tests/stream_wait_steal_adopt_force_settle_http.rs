//! Stream wait-steal → adopt-jobs → force-settle e2e (DECISIONS #65/#72/#74).
...
#[tokio::test]
async fn stream_wait_steal_then_adopt_force_settle() {
    ...
    state
        ...
        .upsert_lifecycle(ProviderSnapshot {
            provider_id: "p-swait-orphan".into(),
            session_epoch: 1,
            trusted: true,
    ...
    hub.attach("p-swait-orphan".into(), 1, tx).await;
    let hub2 = hub.clone();
                ...
                Request::builder()
                    ...
                    .body(Body::from(
                        json!({
                            ...
                            "messages": [{"role":"user","content":"stream wait orphan"}],
                            "stream": true
                        })
                        .to_string(),
                    ))
    ...
    let q = app
        ...
        .await
        .unwrap();
    let orphan_id = body_json(q).await["active_job_ids"][0]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let res = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    json!({ "job_ids": [orphan_id] }).to_string(),
                ))
                .unwrap(),
        )
        .await
    ...
    let av = body_json(res).await;
    assert_eq!(av["adopted_count"], 1);
    assert_eq!(av["failed_count"], 0);
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        "job_id": orphan_id,
                        "actual_micro_usd": 20_000,
                        "terminal_digest": "swait-orphan-d"
                    })
                    .to_string(),
                ))
                .unwrap(),
    ...
    assert_eq!(v["balance_micro_usd"], bal_before - 20_000);
}
