...
| `first_flush_us`, `last_flush_us`, `done_flushed_us`, `chunks_out`, `bytes_out`, `max_chunk_gap_us`, `client_write_err` | `relayStamps` (`api/profiler_dispatch.go`) from the chat, Responses and generic SSE relays; non-streaming 200 bodies stamp the same four fields (one chunk, body bytes) in `writeNonStreamBody` | SSE relay / JSON body to the client |
