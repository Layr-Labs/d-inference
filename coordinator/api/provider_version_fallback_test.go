...
//
// The two drifted apart silently through several releases before this guard
// existed. The failure is quiet by construction: production reads the latest
// version from the releases table, so a stale fallback only shows up on dev
// and in-memory coordinators, where it makes them advertise a floor the Swift
// binary does not match. scripts/check-release-version.sh re-checks this at
// release time, but a per-test-run guard catches the drift when it happens: a
// cross-language constant has no compiler between the two sides, so the test
// IS the linkage.
