...
// needs to rebuild it.
func runChatRewrites(t *testing.T, srv *Controller, body string, service bool) (
	...
	buildModel, _, rewrote, ok := srv.resolveRequestedBuild(
		parsed, prelude.model, nil, dispatch.RoutePolicy{}, registry.RequestTraits{HasTools: shape.hasTools})
	if !ok {
...
func TestProviderBodyMemoSeedMatchesFreshBuild(t *testing.T) {
	srv, _, _ := newBenchController(t)
	registerBuildsProvider(srv, "memo-provider", inferencefixture.DesiredBuild, inferencefixture.PreviousBuild)
	...
	tests := []struct {
		...
		{"alias + runtime defaults + injected max_tokens", `{"model":"` + inferencefixture.Alias + `","messages":[{"role":"user","content":"hi"}]}`, false},
		{"raw build, explicit max_completion_tokens alias field", `{"model":"` + inferencefixture.DesiredBuild + `","messages":[{"role":"user","content":"hi"}],"max_completion_tokens":7}`, false},
		{"stop string normalized", `{"model":"` + inferencefixture.DesiredBuild + `","messages":[{"role":"user","content":"hi"}],"stop":"END","max_tokens":3}`, false},
		...
		{"caller-provided parser default kept", `{"model":"` + inferencefixture.Alias + `","messages":[{"role":"user","content":"hi"}],"tool_call_parser":"mine","max_tokens":3}`, false},
		{"private routing field stripped", `{"model":"` + inferencefixture.Alias + `","messages":[{"role":"user","content":"hi"}],"provider_serial":"C02XYZ","max_tokens":3}`, false},
		{"responses lowered", `{"model":"` + inferencefixture.Alias + `","input":"hello","max_output_tokens":5}`, false},
		{"responses date only", `{"model":"` + serviceReasoningOptInModel + `","input":"hello","max_output_tokens":5}`, false},
		{"tools normalized", `{"model":"` + inferencefixture.Alias + `","messages":[{"role":"user","content":"hi"}],"max_tokens":3,"tools":[{"type":"function","function":{"name":"f","parameters":{"type":"object","properties":{"q":{"description":"x"}}}}}]}`, false},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !isResponsesAPI && model == inferencefixture.DesiredBuild {
				before, err := srv.candidateProviderBody(parsed, defaults, inferencefixture.PreviousBuild, test.service, reasoningProvided, false)
				if err != nil {
					t.Fatal(err)
				}
				...
				parsed["model"] = inferencefixture.PreviousBuild
				if rec, err := srv.deps.Store().GetModelRegistryRecord(inferencefixture.PreviousBuild); err == nil {
					defaults.apply(parsed, rec.RuntimeParameters)
				} else {
					defaults.apply(parsed, nil)
				}
				applyResolvedModelReasoningPolicy(parsed, inferencefixture.PreviousBuild, test.service, reasoningProvided)
				after, err := httpresponse.MarshalBody(parsed)
				if err != nil {
					t.Fatal(err)
				}
