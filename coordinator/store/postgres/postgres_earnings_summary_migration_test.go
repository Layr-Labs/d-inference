...
func earningsMigrationFixture(t *testing.T) *Store {
	t.Helper()
	...
	for _, q := range []string{
		...
		startupschema.EarningsSummaryBackfillPendingDDL,
...
func TestRecordProviderEarningMaintainsSummaryWithoutRestart(t *testing.T) {
	...
	e := &contracts.ProviderEarning{AccountID: uniqueID("account"), ProviderKey: uniqueID("key"), ProviderID: "p", JobID: uniqueID("job"), Model: "m", AmountMicroUSD: 100, PromptTokens: 20, CompletionTokens: 30}
	for i := 0; i < 2; i++ {
