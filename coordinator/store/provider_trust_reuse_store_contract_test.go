...
// providerTrustReuseRoundTrip exercises the trust-reuse persistence contract
// after the mature trust architecture (#778): upserts are keyed by SE pubkey
// and return generation-checked write results, revocation installs a durable
// monotonic tombstone that ordinary writes cannot clear, retrying a committed
// revocation event is idempotent, and only reviewed full-device recovery at
// the observed generation clears the tombstone.
func providerTrustReuseRoundTrip(t *testing.T, st Store) {
	...
	// Re-upsert and revoke overwrite in place: still exactly se-A + se-B.
	if len(rows) != 2 {
		t.Fatalf("re-upsert/revoke must not duplicate: len = %d, want 2", len(rows))
	}
	byKey = map[string]ProviderTrustReuse{}
	for _, row := range rows {
		byKey[row.SEPubKey] = row
...
func TestProviderTrustReuseStoreContract(t *testing.T) {
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			providerTrustReuseRoundTrip(t, st)
		})
	}
}

func TestProviderTrustReuseCoverageStoreContract(t *testing.T) {
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			providerTrustReuseCoverageAdvance(t, st)
		})
	}
}

// TestPostgresProviderTrustReuseLegacyMigrationIsIdempotent is inherently
// PostgreSQL-only: it manipulates the live schema to model a pre-#778
// deployment (legacy columns, no revocation state) and asserts the migration
// backfills conservatively and can replay without corrupting rows.
