...
func TestApplyMetadataDetailsRequestStripsBodyFlag(t *testing.T) {
	...
	forward, err := httpresponse.MarshalBody(parsed)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSnapshotAndAttachChatCompletionMetadata(t *testing.T) {
	...
	SnapshotChatCompletionMetadata(pr, ProviderInfo{
		ProviderID: "prov-9",
		Attested:   true,
	...
	SnapshotChatCompletionMetadata(skipped, ProviderInfo{ProviderID: "prov-9"})
...
func TestWriteCommittedProviderHeaders(t *testing.T) {
	...
	WriteCommittedProviderHeaders(rec, ProviderInfo{
		ProviderID:    "prov-h",
		Attested:      false,
