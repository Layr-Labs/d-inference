...
func seededLegacyEarningsStore(t *testing.T) *Store {
	t.Helper()
...
func executeOldServingCredit(t *testing.T, tx pgx.Tx) {
	...
	var balance int64
	err = tx.QueryRow(context.Background(), string(sql), "a", int64(50), string(contracts.LedgerPayout), "live-work", nil, "live", "p", "m", 5, 7).Scan(&balance)
	if err != nil {
		t.Fatal(err)
	}
}

func assertBackfilledWork(t *testing.T, s *Store) {
	t.Helper()
	...
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			for _, credit := range []bool{false, true} {
				...
				e := &contracts.ProviderEarning{AccountID: acct, ProviderKey: key, ProviderID: "p", JobID: uniqueID("base-job"), Model: "base_reward", AmountMicroUSD: 800, PromptTokens: 999, CompletionTokens: 999}
				for i := 0; i < 2; i++ {
					var err error
					if credit {
				...
				for _, summary := range []contracts.ProviderEarningsSummary{accountSummary, providerSummary} {
