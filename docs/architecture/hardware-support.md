...
The provider is Apple-Silicon-only: `provider-swift/Package.swift` declares
`platforms: [.macOS(.v14)]` (the pinned `libs/mlx-swift-lm/Package.swift` also
declares `.macOS(.v14)`; `libs/mlx-swift/Package.swift` declares no
`platforms:` floor at all),
...
`requireMetal`) and at least 8 GB of RAM
(`provider-swift/Sources/darkbloom/StartCommand+Preflight.swift`,
`hardware.memoryGb < 8`). There is no hard runtime macOS-version check:
`BootSecuritySnapshot.recommendedMacOSMajorVersion = 26` only turns
`darkbloom doctor` yellow below macOS 26
(`provider-swift/Sources/ProviderCore/Security/BootSecurity.swift`). Chip
...
| `memory_reserve_gb` | operator config reserve, `memoryReserveGB` default `4` | `provider-swift/Sources/ProviderCore/Config/ProviderConfig.swift` |
...
defaultLoadHeadroomGb = loadHeadroomBytes() / 2^30
    -- 6.5 GiB by default (5.5 + 1); 4.5 GiB for a gpt-oss-20b-only serving set (3.5 + 1)
...
`weightsGb` is the scanner's padded estimate (`sizeBytes / 2^30 × 1.2`), so a
load needs `weights × 1.2 + activationReserve + 1 GiB` of free-for-load memory.
Worked example from the source comment: gpt-oss-20b drops from `13.5 + 5.5 + 1
= 20.0` to `13.5 + 3.5 + 1 = 18.0` GB with its measured floor
(`provider-swift/Sources/ProviderCore/Inference/ModelLoadAdmission.swift`).
There is no `× 3.0` headroom rule anywhere in the load path.
...
- `KVHeadroomProbe.measuredLiveKVHeadroomBytes` (after trimming the cold-load
  buffer cache) must satisfy `loadIsServeable` (≥ 1 GiB) or the model is
...
2. The cap never leaves the OS less than `minimumReserveBytes` — `hardCapBytes`
   takes `min(fraction × physical, physical − 2 GiB)`.
...
5. A load passes only if `weights(×1.2) + activations + 1 GiB ≤ freeForLoadGb`
   and, after loading, measured live KV headroom is ≥ 1 GiB — `canLoad`,
   `loadIsServeable`, `KVHeadroomProbe.hasServeableKVHeadroom`.
...
7. `freeForLoadGb` applies no multiplicative discount; the only padding is the
   scanner's `1.2` on weights — `ModelLoadAdmission.swift`,
...
| Symptom | Cause | Where |
|---|---|---|
| `darkbloom start` exits: "At least 8 GB is needed" | `hardware.memoryGb < 8` | `StartCommand+Preflight.swift` |
| Start refused: Metal unavailable | `GPUEnforcement.requireMetal()` threw | `GPUEnforcement.swift` |
| Load refused: "… need N GB to serve — unloaded" | Post-load `hasServeableKVHeadroom` false (< 1 GiB live KV headroom) | `ProviderLoop+ModelLoading.swift`, `KVHeadroomProbe.swift` |
...
| `darkbloom doctor` warns about macOS | Below `recommendedMacOSMajorVersion = 26`; warning only | `BootSecurity.swift` |
...
- [`components/mlx-swift.md`](components/mlx-swift.md) — the pinned MLX stack and its platform floor
