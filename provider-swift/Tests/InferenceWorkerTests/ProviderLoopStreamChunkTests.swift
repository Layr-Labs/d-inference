...
func parseStreamChunkExtractsRoleOnlyOpeningFrame() throws {
    ...
    let parsed = try #require(WorkerInferenceSupport.parseStreamChunk(frame))
...
func visibleOutputSignalIgnoresRoleOnlyPreamble() throws {
    ...
    #expect(!WorkerInferenceSupport.hasVisibleStreamOutput(
...
func parseStreamChunkReturnsNilOnDoneSentinel() {
    #expect(WorkerInferenceSupport.parseStreamChunk(ServerSentEventEncoder.done) == nil)
}
...
func parseStreamChunkIgnoresSSEComment() {
    let frame = ":keepalive\n\n"
    #expect(WorkerInferenceSupport.parseStreamChunk(frame) == nil)
}
...
func parseStreamChunkHandlesMultiLineDataEvent() throws {
    ...
    let parsed = try #require(
        WorkerInferenceSupport.parseStreamChunk(frame),
...
func parseStreamChunkPreservesEmptyDataLines() {
    let frame = "data: foo\ndata: \ndata: bar\n\n"
    let joined = WorkerInferenceSupport.joinedDataPayload(frame)
    #expect(joined == "foo\n\nbar",
...
func encodeToolCallsForHashProducesStableEncoding() {
    ...
    let encoded = WorkerInferenceSupport.encodeToolCallsForHash([call])
    ...
    let encoded2 = WorkerInferenceSupport.encodeToolCallsForHash([call])
...
func encodeToolCallsForHashEmptyInputReturnsEmptyString() {
    #expect(WorkerInferenceSupport.encodeToolCallsForHash([]) == "")
}
...
func injectReasoningTokensAddsDetails() throws {
    ...
    let rewritten = WorkerInferenceSupport.injectReasoningTokens(into: frame, reasoningTokens: 12)

    // The rewritten frame must still parse and preserve the original counts.
    let parsed = try #require(WorkerInferenceSupport.parseStreamChunk(rewritten))
    let usage = try #require(parsed.usage)
    #expect(usage.promptTokens == 10)
    #expect(usage.completionTokens == 30)

    // And carry the OpenAI-standard reasoning detail on the wire.
    let payload = try #require(WorkerInferenceSupport.joinedDataPayload(rewritten))
    let obj = try #require(
...
func injectReasoningTokensZeroIsNoop() throws {
    ...
    #expect(WorkerInferenceSupport.injectReasoningTokens(into: frame, reasoningTokens: 0) == frame)
}

@Test("injectReasoningTokens leaves frames without a usage block untouched")
func injectReasoningTokensNoUsageIsNoop() throws {
    let frame = try encodeChunk(content: "hello")
    #expect(WorkerInferenceSupport.injectReasoningTokens(into: frame, reasoningTokens: 9) == frame)
}
...
func injectReasoningTokensMergesExistingDetails() throws {
    ...
    let rewritten = WorkerInferenceSupport.injectReasoningTokens(into: raw, reasoningTokens: 4)
    let payload = try #require(WorkerInferenceSupport.joinedDataPayload(rewritten))
    let obj = try #require(
        try JSONSerialization.jsonObject(with: Data(payload.utf8)) as? [String: Any]
    )
...
func injectLogprobsContentFrame() throws {
    ...
    let rewritten = try #require(
        WorkerInferenceSupport.injectLogprobs(into: frame, entries: sampleLogprobEntries())
    )
...
func cappedPendingWindowSplicesIntoContentFrame() throws {
    ...
    #expect(WorkerInferenceSupport.injectLogprobs(
    ...
    let frame = try encodeChunk(content: "Hello")
    let rewritten = try #require(WorkerInferenceSupport.injectLogprobs(into: frame, entries: pending))
    let payload = try #require(WorkerInferenceSupport.joinedDataPayload(rewritten))
    let obj = try #require(
        try JSONSerialization.jsonObject(with: Data(payload.utf8)) as? [String: Any]
    )
...
func injectLogprobsSkipsNonContentFrames() throws {
    ...
    #expect(WorkerInferenceSupport.injectLogprobs(into: "data: [DONE]\n\n", entries: entries) == nil)
    #expect(WorkerInferenceSupport.injectLogprobs(into: "data: {not json}\n\n", entries: entries) == nil)
