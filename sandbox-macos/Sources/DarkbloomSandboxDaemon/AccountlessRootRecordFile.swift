...
final class AccountlessRootRecordFile {
    ...
    static func read(_ path: URL) throws -> Data { try HostUserIdentityFile.read(path) }
    ...
    func publish(_ bytes: Data) throws {
        guard !bytes.isEmpty, bytes.count <= HostUserIdentityFile.maximumBytes else { throw AccountlessInstallationError.invalidBinding }
