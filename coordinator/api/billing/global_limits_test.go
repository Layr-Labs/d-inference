...
func TestGlobalPayoutRecipientLimits(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			w := globalAPIRequest(t, s, u, "/onboard", `{"country":"`+tc.country+`"}`, s.StripeOnboard)
			if w.Code != 200 {
				t.Fatal(w.Body.String())
			}
			w = globalAPIRequest(t, s, u, "/quote", `{"amount_usd":"`+tc.amount+`"}`, s.GlobalPayoutQuote)
			...
			w = globalAPIRequest(t, s, u, "/status", ``, s.StripeStatus)
