...
// transaction only.
func (s *Store) withAnalyticsTx(ctx context.Context, fn func(pgx.Tx) error) error {
...
func (s *Store) UsageLocationBuckets(since time.Time) ([]contracts.UsageLocationBucket, error) {
	...
	var buckets []contracts.UsageLocationBucket
	err := s.withAnalyticsTx(ctx, func(tx pgx.Tx) error {
		...
		defer rows.Close()
		for rows.Next() {
			var b contracts.UsageLocationBucket
			if err := rows.Scan(
				&b.City,
				&b.Region,
				&b.RegionCode,
...
func (s *Store) UsageFlowBuckets(since time.Time, _ map[string]*contracts.ProviderLocation) ([]contracts.UsageFlowBucket, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	var buckets []contracts.UsageFlowBucket
	err := s.withAnalyticsTx(ctx, func(tx pgx.Tx) error {
		...
		defer rows.Close()
		for rows.Next() {
			var b contracts.UsageFlowBucket
			if err := rows.Scan(
...
func (s *Store) NetworkTotals(since time.Time) (contracts.NetworkTotalsRow, error) {
	...
	if err != nil {
		return contracts.NetworkTotalsRow{}, fmt.Errorf("store: network totals: %w", err)
	}
	return t, nil
}
