...
8. No request body, prompt, or completion text reaches structured logs or the store; the only content-derived artifacts are keyed digests for cache routing — `coordinator/inference/dispatch/run.go`, `coordinator/store/interface.go`, `coordinator/registry/cache_route_keys.go`.
