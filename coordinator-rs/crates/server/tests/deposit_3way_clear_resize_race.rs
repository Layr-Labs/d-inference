//! Concurrent deposit with 3-way clear after resize: money conserved.
...
#[test]
fn concurrent_deposit_with_3way_clear_after_resize_conserves() {
    ...
    let deposit = thread::spawn(move || {
        ...
        apply_stripe_deposit(&mut ib, &mut g, "stripe", "evt_3way_rz", "a", 200_000, 80_000)
            .map(|a| a)
            .unwrap_or(false)
    });
    ...
    let force = thread::spawn(move || {
        force_settle_held(&led_f, "j", "a", 90_000, "force-dep-3way-rz")
    ...
    // Seed 1.2M + deposit 200k = 1.4M. Charged 90k → free = 1.31M
    assert_eq!(g.active_job_count(), 0);
    assert_eq!(g.balance("a").0, 1_310_000);
}
