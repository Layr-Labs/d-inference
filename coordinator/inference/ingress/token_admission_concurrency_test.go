	...
	for _, mode := range []string{"account", "key", "both", "separate keys"} {
		t.Run(mode, func(t *testing.T) {
			for attempt := 0; attempt < 100; attempt++ {
				...
				if mode != "key" {
					services.consumerTokenLimiter = ratelimit.NewTokenLimiter(0.000001, 1, 0.000001, 1)
				}
				if mode != "account" {
					services.keyTokenLimiter = ratelimit.NewKeyTokenLimiter()
				}
				s := newTestController(services)
