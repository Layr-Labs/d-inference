...
// NewChatSink constructs the chat formatter and its per-stream write accounting.
func NewChatSink(pr *registry.PendingRequest, w http.ResponseWriter, flusher http.Flusher, profile *registry.RequestProfile, observer WriteObserver) *ChatSink {
	return &ChatSink{pr: pr, w: w, flusher: flusher, stamps: newRelayStamps(profile), observer: observer}
}
...
func (rl *ChatSink) Chunk(chunk string) {
	...
	rl.Frame(rewriteChunkModel(chunk, rl.pr))
}
...
func (rl *ChatSink) Frame(frame string) {
...
func (rl *ChatSink) Flush() {
	...
	observeContentWrite(rl.observer, rl.w, content, n, size, err)
	observeTerminalWrite(rl.observer, rl.w, ResponseStreamTerminals(rl.buf.Bytes()), n, size, err)
	if n != size {
		rl.stamps.writeErr()
	}
	if rl.buf.Cap() > MaxBatchBytes {
		rl.buf = bytes.Buffer{}
	} else {
		rl.buf.Reset()
	}
...
// Done records a successfully flushed terminal batch in the request profile.
// An earlier short or failed write still prevents a completed-egress stamp.
func (rl *ChatSink) Done() { rl.stamps.done() }
