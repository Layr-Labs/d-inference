...
function EndpointRow({
  ...
  const t = useTranslations("ApiConsolePage");
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          {auth && (
            <p className="text-xs text-text-tertiary">
              {t.rich("requiresAuth", {
                code: (chunks) => <code className="text-accent-brand">{chunks}</code>,
              })}
            </p>
          )}
          {request && (
            <div>
              <p className="text-xs font-mono text-text-tertiary mb-1.5">{t("request")}</p>
          ...
          {response && (
            <div>
              <p className="text-xs font-mono text-text-tertiary mb-1.5">{t("response")}</p>
        ...
        <div className="max-w-4xl mx-auto p-6 space-y-8">
          <div className="rounded-xl bg-accent-amber/5 border border-accent-amber/15 px-5 py-4">
            <p className="text-sm text-text-secondary leading-relaxed">
              {t.rich("intro", {
                strong: (chunks) => <span className="font-semibold text-text-primary">{chunks}</span>,
              })}
            </p>
          </div>
          ...
          <section>
            <h2 className="text-lg font-semibold text-text-primary mb-4">{t("endpointReference")}</h2>
          ...
          <section>
            <div className="rounded-xl bg-bg-secondary shadow-sm p-5">
              <h3 className="text-sm font-semibold text-text-primary mb-2">{t("baseUrl")}</h3>
          ...
          {/* API Key Management */}
          <section>
            <h2 className="text-lg font-semibold text-text-primary mb-4">{t("apiKey")}</h2>
            <div className="rounded-xl bg-bg-secondary shadow-sm p-5">
              <div className="flex items-center gap-3">
                <Key size={18} className="text-accent-brand shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-mono text-text-primary truncate">
                    {showKey ? apiKey || t("noKey") : maskedKey}
                  </p>
                </div>
                <button
          ...
          {/* SDK Setup */}
          <section>
            <h2 className="text-lg font-semibold text-text-primary mb-2">{t("quickStart")}</h2>
            <p className="text-sm text-text-secondary mb-4">
          ...
          {/* Available Models */}
          <section>
            <h2 className="text-lg font-semibold text-text-primary mb-2">{t("availableModels")}</h2>
            <div className="rounded-xl bg-bg-secondary shadow-sm overflow-hidden">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border-dim">
                    <th className="text-left text-xs text-text-tertiary font-medium px-4 py-3">{t("modelId")}</th>
                    <th className="text-left text-xs text-text-tertiary font-medium px-4 py-3">{t("type")}</th>
                    <th className="text-left text-xs text-text-tertiary font-medium px-4 py-3">{t("architecture")}</th>
                  </tr>
                </thead>
                <tbody>
                  {[
          ...
          {/* Chat Completions */}
          <section>
            <h2 className="text-lg font-semibold text-text-primary mb-2">{t("chatCompletions")}</h2>
            <p className="text-sm text-text-secondary mb-4">
              {t("chatDescription")}
            </p>
          ...
          {/* List Models */}
          <section>
            <h2 className="text-lg font-semibold text-text-primary mb-2">{t("listModels")}</h2>
            <p className="text-sm text-text-secondary mb-4">
              {t("listModelsDescription")}
            </p>
