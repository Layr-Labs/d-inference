...
Provider owner
    |
    | POST /v1/enroll {serial_number}
    v
Coordinator generates + CMS-signs .mobileconfig
    |
    | User installs profile in System Settings
    v
macOS enrolls in MicroMDM + requests ACME cert from step-ca
    |
    v
Coordinator receives MDM check-in and can query SecurityInfo / MDA
```
...
```
Provider registers with SE-signed attestation
    -> trust level = self_signed
MDM SecurityInfo passes
    -> trust level = hardware
MDA certificate chain verifies and serial matches
    -> MDA proof stored, user-verifiable attestation available
APNs code-identity round-trip passes
    -> CodeAttested = true, private traffic may route
```
