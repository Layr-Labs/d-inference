...
fn vectors() -> Vec<BoxVector> {
    serde_json::from_slice(&support::read_vector_file("nacl_box/vectors.json"))
        .expect("nacl_box vectors parse")
}

fn secret_from_hex(s: &str) -> SecretKey {
    SecretKey::from(support::hex32(s))
}
...
fn rust_opens_every_go_sealed_payload() {
    for v in vectors() {
        ...
        assert_eq!(plaintext, support::from_hex(&v.plaintext_hex), "{}", v.name);
    }
}
...
fn precomputed_shared_key_matches_go_precompute() {
    for v in vectors() {
        ...
        let expected = support::hex32(&v.shared_key_hex);
        ...
        assert_eq!(
            ...
            support::from_hex(&v.plaintext_hex),
            "{}",
            v.name
        );
    }
...
fn rust_seal_is_byte_identical_to_go_seal() {
    for v in vectors() {
        let sender = secret_from_hex(&v.sender_private_key_hex);
        let recipient_pub = parse_public_key(&v.recipient_public_key_b64).unwrap();
        let nonce: [u8; NONCE_LEN] = support::from_hex(&v.nonce_hex)
            .try_into()
            .expect("24-byte nonce");
        let plaintext = support::from_hex(&v.plaintext_hex);
