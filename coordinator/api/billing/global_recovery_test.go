...
func TestGlobalPayoutDisabledPreservesLegacyConnect(t *testing.T) {
	for _, country := range []string{"AU", "IN", "JP", "BR", "GH", "NG"} {
		t.Run(country, func(t *testing.T) {
			...
			s.SetBilling(billingservice.NewService(st, s.billing().Ledger(), s.logger, billingservice.Config{MockMode: true, StripeConnectReturnURL: "https://app.test/billing", StripeGlobalPayoutsFinancialAccount: "fa_gp", StripeGlobalPayoutsSecretKey: "rk_test_gp"}))
...
func TestGlobalPayoutBankLookupFailurePreservesReadyDestination(t *testing.T) {
	...
	for _, code := range []int{429, 500} {
		...
		f.bankStatus = code
		f.mu.Unlock()
		for _, handler := range []http.HandlerFunc{s.StripeStatus, s.GlobalPayoutQuote} {
...
func TestGlobalPayoutDefinitiveRejectionSurvivesRefundFailure(t *testing.T) {
	...
	cfg := billingservice.Config{MockMode: true, StripeGlobalPayoutsEnabled: true, StripeGlobalPayoutsFinancialAccount: "fa_gp", StripeGlobalPayoutsSecretKey: "rk_test_gp"}
	baseURL := s.billing().GlobalPayouts().BaseURL
	s.SetBilling(billingservice.NewService(wrapper, s.billing().Ledger(), s.logger, cfg))
	s.billing().GlobalPayouts().BaseURL = baseURL
	f.mu.Lock()
	f.rejectRequests = true
	f.mu.Unlock()
	w = globalAPIRequest(t, s, u, "/withdraw", `{"amount_usd":"10","quote_id":"`+q.ID+`"}`, s.StripeWithdraw)
	if w.Code != 202 {
		t.Fatal(w.Body.String())
	}
