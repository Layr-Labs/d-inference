...
func providerTrustReuseRoundTrip(t *testing.T, st contracts.Store) {
	t.Helper()
	ctx := context.Background()
	t0 := time.Now().UTC().Truncate(time.Second)

	if _, err := st.UpsertProviderTrustReuse(ctx, contracts.ProviderTrustReuse{
	...
	recA := contracts.ProviderTrustReuse{
	...
	byKey := map[string]contracts.ProviderTrustReuse{}
...
func TestMemoryProviderTrustReuseRoundTripRevocationEvents(t *testing.T) {
	providerTrustReuseRoundTrip(t, memory.New(contracts.Config{}))
}
...
func providerTrustReuseCoverageAdvance(t *testing.T, st contracts.Store) {
	t.Helper()
	ctx := context.Background()
	base := time.Now().UTC().Truncate(time.Second)

	hw := contracts.ProviderTrustReuse{
...
func TestMemoryProviderTrustReuseCoverageAdvance(t *testing.T) {
	providerTrustReuseCoverageAdvance(t, memory.New(contracts.Config{}))
}
