//! Concurrent MemoryLedger.credit zero total: InvalidAmount (via deposit path).
...
#[test]
fn concurrent_deposit_zero_total_all_invalid() {
    ...
    assert_eq!(state.lock().unwrap().1.balance("a").0, 0);
}
