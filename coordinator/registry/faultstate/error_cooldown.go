...
//
// Modeled on the dispatch-load cooldown (dispatch_load_cooldown.go): per-identity gate
// state (state.go), window-rebuild-on-write. Only sickness-shaped
...
//
// State lives on the identity's gate (state.go), keyed inside it by
...
func (r *Manager[C]) RecordInferenceError(providerID, modelID string, statusCode int, shape string, causes ...protocol.CoordinatorInferenceErrorCause) (enteredCooldown bool) {
	switch statusCode {
	case 500, 502, 504:
...
func (r *Manager[C]) RecordInferenceSuccess(providerID, modelID, shape string) {
...
func (r *Manager[C]) InferenceErrorCooldownActive(providerID, modelID, shape string) bool {
	return r.InferenceErrorCooled(providerID, modelID, shape, r.now())
}
...
func (r *Manager[C]) InferenceErrorCooled(providerID, modelID, shape string, now time.Time) bool {
