...
export function DemandIntervalTable({ model, bucketSeconds }: { model: ModelDemand; bucketSeconds: number }) {
  return <details className="mt-4 text-xs text-text-secondary">
    <summary className="min-h-8 cursor-pointer font-medium">View interval data</summary>
    <div className="max-h-80 overflow-auto rounded-lg border border-border-dim">
      <table className="w-full min-w-[900px] text-left tabular-nums">
        <caption className="sr-only">{model.model} request outcomes by interval</caption>
        <thead className="sticky top-0 bg-bg-secondary"><tr>{["Interval (local time)", "Received", "Completed", "Capacity", "Latency limited", "Timeouts", "Service errors", "Departed", "Unknown", "HTTP 429"].map(label => <th key={label} className="whitespace-nowrap px-3 py-3 font-medium">{label}</th>)}</tr></thead>
