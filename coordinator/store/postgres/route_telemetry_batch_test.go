...
func TestSplitInferenceRouteBatches(t *testing.T) {
	rec := func(id string, attempt int) *contracts.InferenceRouteRecord {
		return &contracts.InferenceRouteRecord{RequestID: id, Attempt: attempt}
	}

	t.Run("empty and nil-only input yields no batches", func(t *testing.T) {
		if got := routerecord.SplitInferenceRouteBatches(nil, 10); len(got) != 0 {
			t.Fatalf("nil input: got %d batches", len(got))
		}
		if got := routerecord.SplitInferenceRouteBatches([]*contracts.InferenceRouteRecord{nil, nil}, 10); len(got) != 0 {
			t.Fatalf("nil-only input: got %d batches", len(got))
		}
	})

	t.Run("duplicate key starts a new batch, order preserved", func(t *testing.T) {
		in := []*contracts.InferenceRouteRecord{rec("a", 1), rec("b", 1), nil, rec("a", 1), rec("c", 1), rec("a", 2)}
		got := routerecord.SplitInferenceRouteBatches(in, 0)
		if len(got) != 2 {
	...
	t.Run("maxRows chunks without reordering", func(t *testing.T) {
		var in []*contracts.InferenceRouteRecord
		for i := 0; i < 7; i++ {
			in = append(in, rec(fmt.Sprintf("r%d", i), 1))
		}
		got := routerecord.SplitInferenceRouteBatches(in, 3)
...
func TestInferenceRouteInsertSQLShape(t *testing.T) {
	...
	if got := len(inferenceRouteInsertArgs(nil, &contracts.InferenceRouteRecord{}, time.Now())); got != inferenceRouteInsertParamCount {
		t.Fatalf("inferenceRouteInsertArgs produced %d args, want %d", got, inferenceRouteInsertParamCount)
	}
	if got := len(inferenceRouteOutcomeUpdateArgs("r", 1, &contracts.InferenceRouteOutcome{})); got != 24 {
...
func testInferenceRouteBatch(t *testing.T, s contracts.Store) {
	t.Helper()

	if err := s.RecordInferenceRoutes(nil); err != nil {
		t.Fatalf("RecordInferenceRoutes(nil): %v", err)
	}
	if err := s.RecordInferenceRoutes([]*contracts.InferenceRouteRecord{nil}); err != nil {
		t.Fatalf("RecordInferenceRoutes([nil]): %v", err)
	}
	...
	records := []*contracts.InferenceRouteRecord{
	...
	updates := []contracts.InferenceRouteOutcomeUpdate{
		{RequestID: id(1), Attempt: 1, Outcome: &contracts.InferenceRouteOutcome{ActualTTFTMs: 42, UsedBackup: true}},
		{RequestID: id(1), Attempt: 1, Outcome: &contracts.InferenceRouteOutcome{FinalStatus: "error", ErrorCode: 502, ErrorClass: "provider_error", CompletionTokensSet: true}},
		{RequestID: id(1), Attempt: 1, Outcome: &contracts.InferenceRouteOutcome{FinalStatus: "partial_success"}},
		{RequestID: id(2), Attempt: 1, Outcome: &contracts.InferenceRouteOutcome{FinalStatus: "success", PromptTokens: 10, CompletionTokens: 20, CostMicroUSD: 7, CompletionTokensSet: true}},
		{RequestID: id(3), Attempt: 1, Outcome: nil},
		{RequestID: id(3) + "-missing", Attempt: 9, Outcome: &contracts.InferenceRouteOutcome{FinalStatus: "success"}},
	}
	if err := s.UpdateInferenceRouteOutcomes(updates); err != nil {
		t.Fatalf("UpdateInferenceRouteOutcomes: %v", err)
	}

	byKey := map[string]contracts.InferenceRouteRecord{}
	for _, r := range s.InferenceRouteRecordsSince(time.Time{}) {
		if strings.HasPrefix(r.RequestID, prefix) {
			byKey[routerecord.InferenceRouteKey(r.RequestID, r.Attempt)] = r
		}
	}
	if len(byKey) != 4 {
	...
	r1 := byKey[routerecord.InferenceRouteKey(id(1), 1)]
	...
	r2 := byKey[routerecord.InferenceRouteKey(id(2), 1)]
	...
	r3 := byKey[routerecord.InferenceRouteKey(id(3), 1)]
	if r3.FinalStatus != "" || r3.ProviderRegion != "us-east" {
		t.Fatalf("row 3 must be untouched by the nil update: %+v", r3)
	}
	if r12 := byKey[routerecord.InferenceRouteKey(id(1), 2)]; r12.ProviderID != "p1b" {
		t.Fatalf("attempt 2 must be its own row: %+v", r12)
	}

	// A single-row write after a batch behaves identically (same code path).
	if err := s.RecordInferenceRoute(&contracts.InferenceRouteRecord{RequestID: id(3), Attempt: 1, ProviderID: "p3-refresh", Outcome: "selected", Model: "m"}); err != nil {
		t.Fatalf("RecordInferenceRoute: %v", err)
	}
	if err := s.UpdateInferenceRouteOutcome(id(3), 1, &contracts.InferenceRouteOutcome{FinalStatus: "cancelled", CompletionTokensSet: true}); err != nil {
...
func TestInferenceRouteBatchChunking(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			records := make([]*contracts.InferenceRouteRecord, 0, n+1)
			for i := 0; i < n; i++ {
				records = append(records, &contracts.InferenceRouteRecord{RequestID: fmt.Sprintf("%s-%d", prefix, i), Attempt: 1, Outcome: "selected", ProviderID: "p"})
			}
			// Re-record row 0 at the very end: lands in the last chunk and must
			// win over the first chunk's write.
			records = append(records, &contracts.InferenceRouteRecord{RequestID: fmt.Sprintf("%s-%d", prefix, 0), Attempt: 1, Outcome: "selected", ProviderID: "p-last"})
			if err := s.RecordInferenceRoutes(records); err != nil {
				t.Fatalf("RecordInferenceRoutes(%d): %v", len(records), err)
			}
			updates := make([]contracts.InferenceRouteOutcomeUpdate, 0, n)
			for i := 0; i < n; i++ {
				updates = append(updates, contracts.InferenceRouteOutcomeUpdate{RequestID: fmt.Sprintf("%s-%d", prefix, i), Attempt: 1, Outcome: &contracts.InferenceRouteOutcome{FinalStatus: "success", CompletionTokens: i + 1}})
			}
			if err := s.UpdateInferenceRouteOutcomes(updates); err != nil {
...
func tracedPostgresStore(t *testing.T, counter *statementCounter) *Store {
	t.Helper()
	testPostgresStore(t)
	...
	return &Store{pool: pool, priceCache: make(map[string]cachedPrice)}
}
...
func TestPostgresInferenceRouteBatchStatementCount(t *testing.T) {
	...
	rec := func(tag string, i int) *contracts.InferenceRouteRecord {
		return &contracts.InferenceRouteRecord{RequestID: fmt.Sprintf("%s-%s-%d", prefix, tag, i), Attempt: 1, ProviderID: "p", Outcome: "selected", Model: "m"}
	}
	upd := func(tag string, i int) contracts.InferenceRouteOutcomeUpdate {
		return contracts.InferenceRouteOutcomeUpdate{RequestID: fmt.Sprintf("%s-%s-%d", prefix, tag, i), Attempt: 1, Outcome: &contracts.InferenceRouteOutcome{FinalStatus: "success", CompletionTokens: 3, CompletionTokensSet: true}}
	}
	...
	records := make([]*contracts.InferenceRouteRecord, 0, n)
	updates := make([]contracts.InferenceRouteOutcomeUpdate, 0, n)
	for i := 0; i < n; i++ {
...
func TestPostgresInferenceRouteBatchDuplicateKeysSplit(t *testing.T) {
	...
	id := uniqueID("dup")
	counter.reset()
	err := s.RecordInferenceRoutes([]*contracts.InferenceRouteRecord{
