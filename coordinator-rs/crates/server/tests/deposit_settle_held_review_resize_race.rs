//! Concurrent deposit while settle vs held-review after resize.
...
#[test]
fn concurrent_deposit_settle_vs_held_review_after_resize() {
    let inbox = Arc::new(Mutex::new(ExternalEventInbox::new()));
    let led = Arc::new(Mutex::new(MemoryLedger::default()));
    {
        let mut ib = inbox.lock().unwrap();
        let mut g = led.lock().unwrap();
        apply_stripe_deposit(&mut ib, &mut g, "stripe", "seed", "a", 780_000, 0).unwrap();
        g.reserve(OperationKey("r".into()), "j", "a", 100_000)
            .unwrap();
        g.resize_and_authorize(OperationKey("ra".into()), "j", "a", 320_000)
            .unwrap();
    }
    ...
    let deposit = thread::spawn(move || {
        ...
        apply_stripe_deposit(&mut ib, &mut g, "stripe", "evt_sh_rz", "a", 95_000, 28_000)
            .map(|a| a)
            .unwrap_or(false)
    });
    ...
    // Seed 780k + deposit 95k = 875k. Charged 55k → free = 820k
