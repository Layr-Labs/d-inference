...
<div className="mt-7 grid gap-4 border-t border-border-dim pt-5 sm:grid-cols-[minmax(0,1fr)_minmax(240px,1fr)] sm:gap-10">
          <div>
            ...
            <p className="mt-1.5 max-w-md text-xs leading-relaxed text-text-secondary">Legacy hardware evidence and current serving authorization are distinct. Request routing depends on additional checks. Reported chip and RAM are not Apple-certified by an App Attest grant.</p>
          </div>
          <div>
            <p className="mb-2.5 flex justify-between gap-3 text-xs text-text-secondary"><span>{data.hardwareAttestedProviders.toLocaleString()} of {data.providers.toLocaleString()} with legacy hardware evidence</span><span className="font-medium tabular-nums text-text-primary">{hardwareShare(data.hardwareAttestedProviders, data.providers)}</span></p>
            ...
            <p className="mt-2 text-xs text-text-secondary">{data.verification.authorized} currently authorized · {data.verification.appAttest} App Attest · {data.verification.legacy} legacy · {data.verification.overlap} both, counted once.</p>
          </div>
        </div>
  );
}
