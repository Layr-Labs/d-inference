	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			heartbeatKV := func(p *registry.Provider, backend *string) {
				d.s.deps.Registry().Heartbeat(p.ID, &protocol.HeartbeatMessage{
...
func heartbeatSlotKV(d *execution, p *registry.Provider, backend string) {
	d.s.deps.Registry().Heartbeat(p.ID, &protocol.HeartbeatMessage{
		Type:   protocol.TypeHeartbeat,
		Status: "serving",
		BackendCapacity: &protocol.BackendCapacity{
...
func speculativeFailureTestState(
	...
	deadline time.Duration,
	speculativeAt time.Duration,
) (*execution, *store.MemoryStore, *registry.Provider, *registry.PendingRequest, *registry.Provider, *registry.PendingRequest) {
	t.Helper()
	s := newTestController(t)
	st, ok := testControllerStore(s).(*store.MemoryStore)
	if !ok {
		t.Fatalf("test server store = %T, want *store.MemoryStore", testControllerStore(s))
	}
	model := "speculative-route-model"
	register := func(id string) *registry.Provider {
		return s.deps.Registry().Register(id, nil, &protocol.RegisterMessage{
...
func assertClearedRoutingAttemptIsNoop(t *testing.T, d *execution, captured dispatchRoutingAttempt) {
	t.Helper()
	target := d.currentOrCapturedRoutingAttempt(captured)
