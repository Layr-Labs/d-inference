...
| `key_history.consecutive_assertion_failures` | int 0–1000 | `ready` | `generateAssertion` calls on the current key that failed inside the Apple call (error, callback without `NSError`, or deadline) since its last success; reset on success and for a new key; capped at 1000. `0` for keys generated by this build; omitted for keys from older providers until their first counted failure or success | Whether the key is persistently dead. |
...
| `push_history.last_reply_sent_age_seconds` | int 0–315360000 | `ready` | now − last `code_attestation_response` sent (recorded right after the send); omitted as above | A push received after the last reply: it arrived but was not answered; compare with `code_attest.push_reply`. |
