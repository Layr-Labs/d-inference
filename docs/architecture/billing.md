...
|---|---|---|
| 1. Reserve | `coordinator/api/inference_admission.go` `reserveInferenceBalance` | `reserved = reservationCost(model, max(billingPromptTokens, estimatedPromptTokens), requestedMaxTokens)` at the platform price. The output bound is the request's explicit `max_tokens` / `max_completion_tokens` / `max_output_tokens`, else the registry's `max_output_length`, else `defaultMaxOutputTokens = 8192` (`coordinator/api/consumer.go` `ensureMaxTokensBound`). The per-key spend cap is checked first (`checkKeySpendCap`), then `reserveInitialBalance` debits the ledger (`LedgerCharge`, reference `reserve:<account>`) or, for a service account with holds enabled, adds to an in-memory hold (`coordinator/api/reservations.go` `serviceReservationManager`). Self-route and a nil billing backend skip the step entirely. |
...
| 5. Record usage | `handleCompleteAt` | In-memory `payments.Ledger.RecordUsage` always; a persistent `usage` row (`RecordUsageFullWithPublicModel`) unless the request was free self-route. |
