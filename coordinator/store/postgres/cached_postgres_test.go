...
func TestCachedStoreOverPostgres(t *testing.T) {
	...
	cfg.Now = clock.Now
	cached := storecache.New(counting, cfg)

	t.Run("users", func(t *testing.T) {
		acct := uniqueID("acct")
		u := &contracts.User{AccountID: acct, PrivyUserID: "did:privy:" + acct, Email: acct + "@example.test"}
		...
		if err := cached.SetUserRole(acct, contracts.RoleService); err != nil {
			t.Fatalf("SetUserRole: %v", err)
		}
		got, err := cached.GetUserByAccountID(acct)
		if err != nil || got.Role != contracts.RoleService {
		...
		if got, _ := cached.GetUserByAccountID(acct); got.Role != contracts.RoleService {
		...
		for i := 0; i < 3; i++ {
			if _, err := cached.GetUserByAccountID("acct-missing"); !errors.Is(err, contracts.ErrNotFound) {
