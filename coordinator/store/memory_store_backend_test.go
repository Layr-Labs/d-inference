...
func TestNewMemoryWithAdminKey(t *testing.T) {
	st := NewMemory(Config{AdminKey: "test-admin-key"})
	if !st.ValidateKey("test-admin-key") || st.KeyCount() != 1 {
		t.Fatalf("configured admin key was not initialized")
	}
}

func TestNewMemoryWithoutAdminKey(t *testing.T) {
	if count := NewMemory(Config{}).KeyCount(); count != 0 {
		t.Fatalf("empty memory store key count = %d, want 0", count)
	}
}
