//! Ephemeral PostgreSQL harness for the `ledger` and `e2e` suites.
//!
...
//! exercises the real pool. Cluster directories are unique per boot
//! (`pid` + process-wide sequence), so concurrently running tests never
//! share a cluster or socket dir. The [`TestDb`] guard stops the cluster
...
pub struct TestDb {
    pub pool: PgPool,
    /// sqlx-parsable URL over the unix socket (for `OwnershipGuard` and
    /// the full-stack bootstrap config).
    pub url: String,
