...
struct StartupPreloadGateTests {
    ...
    func gateTimesOutAndContinuesInBackground() async throws {
        ...
        let raced = await withTaskGroup(
            of: WorkerInferenceSupport.StartupPreloadGateOutcome?.self
        ) { group in
...
struct StartupPreloadNoEvictTests {
    ...
    func slotCapNoEvictRefuses() async throws {
        ...
        do {
            ...
            #expect(WorkerInferenceSupport.loadErrorStatusCode(for: InferenceError.invalidModelDirectory(message)) == 503)
        }
