...
| Fresh-key enrollment failures | An `attestation` failing with `apple_invalid_key` when the canonical machine (else account) has at least three in the trailing 24 hours waits `enrollmentInvalidKeyBackoff = 6 * time.Hour` before the next attempt | `coordinator/appattest/service/enrollment_backoff.go` (`enrollmentBackoffDue`); [backoff](app-attest-shadow.md#invalid-key-enrollment-backoff) |
