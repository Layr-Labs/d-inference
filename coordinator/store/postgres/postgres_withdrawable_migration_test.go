...
func TestWithdrawableBalanceMigrationFirstRunAndRestart(t *testing.T) {
	...
	seedMigrationLedger(t, s, "legacy-withdrawal", contracts.LedgerStripeDeposit, 100, 100, "deposit")
	seedMigrationLedger(t, s, "legacy-withdrawal", contracts.LedgerPayout, 100, 200, "payout")
	seedMigrationLedger(t, s, "legacy-withdrawal", contracts.LedgerCharge, -50, 150, "stripe_withdraw:legacy")
	seedMigrationLedger(t, s, "legacy-withdrawal", contracts.LedgerReferralReward, 25, 175, "referral")

	seedMigrationBalance(t, s, "legacy-reversal", 100, nil)
	seedMigrationLedger(t, s, "legacy-reversal", contracts.LedgerPayout, 100, 100, "payout")
	seedMigrationLedger(t, s, "legacy-reversal", contracts.LedgerCharge, -80, 20, "stripe_withdraw:reversed")
	seedMigrationLedger(t, s, "legacy-reversal", contracts.LedgerRefund, 80, 100, "stripe_withdraw:reversed")

	// Focused coverage for the remaining direct withdrawable entry types.
	seedMigrationBalance(t, s, "typed-deltas", 25, nil)
	seedMigrationLedger(t, s, "typed-deltas", contracts.LedgerAdminReward, 10, 10, "admin")
	seedMigrationLedger(t, s, "typed-deltas", contracts.LedgerFloorDraw, 20, 30, "floor")
	seedMigrationLedger(t, s, "typed-deltas", contracts.LedgerWithdrawal, -5, 25, "legacy-onchain")

	seedMigrationBalance(t, s, "capped-by-total", 25, nil)
	seedMigrationLedger(t, s, "capped-by-total", contracts.LedgerPayout, 100, 100, "payout")
	seedMigrationLedger(t, s, "capped-by-total", contracts.LedgerCharge, -75, 25, "inference")
	...
	seedMigrationLedger(t, s, "spent-then-deposited", contracts.LedgerPayout, 100, 100, "payout")
	seedMigrationLedger(t, s, "spent-then-deposited", contracts.LedgerCharge, -100, 0, "inference")
	seedMigrationLedger(t, s, "spent-then-deposited", contracts.LedgerStripeDeposit, 50, 50, "later-deposit")

	seedMigrationBalance(t, s, "no-eligible-earnings", 100, nil)
	seedMigrationLedger(t, s, "no-eligible-earnings", contracts.LedgerStripeDeposit, 100, 100, "deposit")

	seedMigrationBalance(t, s, "negative-net-earnings", 10, nil)
	seedMigrationLedger(t, s, "negative-net-earnings", contracts.LedgerPayout, 50, 50, "payout")
	seedMigrationLedger(t, s, "negative-net-earnings", contracts.LedgerStripePayout, -50, 0, "withdrawal")
	seedMigrationLedger(t, s, "negative-net-earnings", contracts.LedgerStripeDeposit, 10, 10, "deposit")
	...
	seedMigrationLedger(t, s, "capped-by-total", contracts.LedgerPayout, 500, 525, "post-migration-payout")
	...
	tests := []struct {
	...
	}{
		{
			...
			seed: func(t *testing.T, s *Store) {
				...
				seedMigrationLedger(t, s, "ambiguous-refund", contracts.LedgerPayout, 100, 100, "payout")
				seedMigrationLedger(t, s, "ambiguous-refund", contracts.LedgerCharge, -100, 0, "reserve")
				seedMigrationLedger(t, s, "ambiguous-refund", contracts.LedgerRefund, 100, 100, "reservation_refund")
			},
		},
		{
			...
			seed: func(t *testing.T, s *Store) {
				...
				seedMigrationLedger(t, s, "ambiguous-zero-net", contracts.LedgerPayout, 100, 100, "payout")
				seedMigrationLedger(t, s, "ambiguous-zero-net", contracts.LedgerStripePayout, -100, 0, "withdrawal")
				seedMigrationLedger(t, s, "ambiguous-zero-net", contracts.LedgerRefund, 100, 100, "reservation_refund")
			},
		},
		{
			...
			seed: func(t *testing.T, s *Store) {
				...
				seedMigrationLedger(t, s, "ambiguous-migration", contracts.LedgerMigration, 100, 100, "migrate:in")
			},
		},
	}
...
func TestWithdrawableBalanceMigrationPreservesLiveAccounting(t *testing.T) {
	...
	seedMigrationLedger(t, s, "withdrawn-to-zero", contracts.LedgerPayout, 100, 100, "earning")
	seedMigrationLedger(t, s, "withdrawn-to-zero", contracts.LedgerStripePayout, -100, 0, "withdrawal")
	seedMigrationLedger(t, s, "withdrawn-to-zero", contracts.LedgerStripeDeposit, 50, 50, "later-deposit")

	seedMigrationBalance(t, s, "reversed-withdrawal", 80, &refunded)
	seedMigrationLedger(t, s, "reversed-withdrawal", contracts.LedgerPayout, 100, 100, "earning")
	seedMigrationLedger(t, s, "reversed-withdrawal", contracts.LedgerStripePayout, -100, 0, "withdrawal")
	seedMigrationLedger(t, s, "reversed-withdrawal", contracts.LedgerRefund, 80, 80, "stripe_withdraw:refund")

	seedMigrationBalance(t, s, "migration-source", 0, &zero)
	seedMigrationLedger(t, s, "migration-source", contracts.LedgerPayout, 100, 100, "earning")
	seedMigrationLedger(t, s, "migration-source", contracts.LedgerMigration, -100, 0, "migrate:out")
	seedMigrationBalance(t, s, "migration-destination", 100, &migrated)
	seedMigrationLedger(t, s, "migration-destination", contracts.LedgerMigration, 100, 100, "migrate:in")

	seedMigrationBalance(t, s, "floor-draw", 30, &floorDraw)
	seedMigrationLedger(t, s, "floor-draw", contracts.LedgerFloorDraw, 30, 30, "floor")
...
func TestPostgresStartupRunsWithdrawableMigrationOnce(t *testing.T) {
	...
	first, err := New(ctx, contracts.Config{DatabaseURL: databaseURL})
	if err != nil {
	...
	if err := first.CreditWithdrawable("live-zero", 100, contracts.LedgerPayout, "earning"); err != nil {
		first.Close()
		t.Fatalf("credit live balance: %v", err)
	}
	if err := first.DebitWithdrawable("live-zero", 100, contracts.LedgerStripePayout, "withdrawal"); err != nil {
		first.Close()
		t.Fatalf("withdraw live balance: %v", err)
	}
	first.Close()

	restart, err := New(ctx, contracts.Config{DatabaseURL: databaseURL})
	if err != nil {
...
func TestWithdrawableBalanceMigrationConcurrentReplicas(t *testing.T) {
	for attempt := 0; attempt < 5; attempt++ {
		t.Run(fmt.Sprintf("attempt-%d", attempt), func(t *testing.T) {
			...
			seedMigrationLedger(t, first, "concurrent", contracts.LedgerPayout, 50, 50, "earning")
...
func TestWithdrawableBalanceMigrationBlockedReplicaTakesOverAfterRollback(t *testing.T) {
	...
	seedMigrationLedger(t, first, "rollback-takeover", contracts.LedgerPayout, 60, 60, "earning")
...
func TestWithdrawableBalanceMigrationFailureRollsBackAndRetries(t *testing.T) {
	...
	seedMigrationLedger(t, s, "retry", contracts.LedgerPayout, 75, 75, "earning")
