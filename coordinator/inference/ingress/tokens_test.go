...
func TestApplyTokenRateLimitConsumerOTPM(t *testing.T) {
	s := newTestController(testServices{
...
func TestApplyTokenRateLimitAdminBypass(t *testing.T) {
	s := newTestController(testServices{consumerTokenLimiter: ratelimit.NewTokenLimiter(0.01, 1, 0.01, 1)})
	for i := 0; i < 10; i++ {
		rec := httptest.NewRecorder()
...
func TestTokenExpectedOutputAdmissionDisabledUnchanged(t *testing.T) {
	s := newTestController(testServices{serviceTokenLimiter: ratelimit.NewTokenLimiter(1_000_000, 10_000_000, 0.000001, 512_000)})
...
func TestTokenExpectedOutputAdmissionDeltaConsumesFutureCapacity(t *testing.T) {
	...
	if ok, dim, _ := s.deps.ServiceTokens().Peek("openrouter", 0, 31); ok || dim != "output_tokens" {
