...
| `trust_or_runtime` | `providerTrustMeetsMinimumAtLocked` false (no current App Attest authorization AND trust rank below `MinTrustLevel`), `!RuntimeVerified`, or no private-text support | no |
| `stale_challenge` | `providerChallengeFreshAtLocked` false: no current App Attest authorization AND `LastChallengeVerified` unset or older than `challengeFreshnessMaxAge` | no |
