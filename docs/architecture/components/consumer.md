...
The consumer surface is the part of the coordinator that speaks OpenAI Chat Completions, OpenAI Responses, Anthropic Messages, and legacy Completions to clients, and turns each request into one provider job. Every one of those four endpoints runs the same pipeline in `handleChatCompletions` (`coordinator/api/consumer.go`); the endpoint-specific work is a lowering step before the pipeline and a re-shaping step after it. This page explains what "compatible" means concretely, walks the pipeline stage by stage, and lists the invariants and failure modes that follow. The exact routes, headers, and JSON shapes are in [`../../reference/api-contracts.md`](../../reference/api-contracts.md).
...
| 2 | Parse prelude: 16 MiB cap, tool-schema normalisation, JSON decode, `model` required, key allow-list | `parseInferencePrelude`, `keyModelAllowed` | 400, 403 `model_not_allowed` |
...
| 9 | Reserve balance for the worst-case cost | `reserveInferenceBalance` (`coordinator/api/inference_admission.go`) | 402 `insufficient_funds` / `insufficient_quota` |
...
5. **No keepalives.** Silence on a stream means no token has been produced; the first-content deadline bounds it before commit (a miss is a 429 with `Retry-After`) and `inferenceTimeout` (600 s between chunks) after (a terminal `error` event).
...
| 429 `rate_limit_exceeded` + `Retry-After: 3` | Coordinator draining; new inference is refused until the drain completes | `drainGate` (`coordinator/api/drain.go`), `coordinatorDrainRetryAfter` |
...
| 402 | Balance or key budget exhausted | `reserveInferenceBalance` |
