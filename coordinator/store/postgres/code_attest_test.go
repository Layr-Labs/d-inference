...
func codeAttestRoundTrip(t *testing.T, st contracts.Store) {
	t.Helper()
	...
	if err := st.UpsertCodeAttestation(ctx, contracts.CodeAttestation{Version: "0.6.0", AttestedAt: t0}); err != nil {
	...
	if err := st.UpsertCodeAttestation(ctx, contracts.CodeAttestation{SEPubKey: "se-A", Version: "0.6.0", AttestedAt: t0, APNsToken: "tok-A", BinaryHash: "hash-A"}); err != nil {
		t.Fatalf("upsert A: %v", err)
	}
	if err := st.UpsertCodeAttestation(ctx, contracts.CodeAttestation{SEPubKey: "se-B", Version: "0.6.1", AttestedAt: t0}); err != nil {
	...
	byKey := map[string]contracts.CodeAttestation{}
	for _, r := range rows {
		byKey[r.SEPubKey] = r
	}
	if got := byKey["se-A"]; got.Version != "0.6.0" || !got.AttestedAt.Equal(t0) || got.APNsToken != "tok-A" || got.BinaryHash != "hash-A" {
		t.Fatalf("se-A round-trip mismatch: %+v", got)
	}

	// Upsert the same key with a newer time+version overwrites (no duplicate).
	t1 := t0.Add(10 * time.Minute)
	if err := st.UpsertCodeAttestation(ctx, contracts.CodeAttestation{SEPubKey: "se-A", Version: "0.6.2", AttestedAt: t1, APNsToken: "tok-A2", BinaryHash: "hash-A2"}); err != nil {
	...
	for _, r := range rows {
		if r.SEPubKey == "se-A" && (r.Version != "0.6.2" || !r.AttestedAt.Equal(t1) || r.APNsToken != "tok-A2" || r.BinaryHash != "hash-A2") {
...
func TestMemoryCodeAttestationRoundTrip(t *testing.T) {
	codeAttestRoundTrip(t, memory.New(contracts.Config{}))
}
