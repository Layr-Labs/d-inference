...
> Last updated: 2026-09-24 · commit `b6f9574e`
...
| Status check | `POST /v1/releases/qualification` takes the scoped release key and the same body as `POST /v1/releases` (`registerReleaseRequest`, `platform` defaulting to `macos-arm64`), builds the identity the same way as publication, and reports the current durable status without approving, revoking, publishing, or writing anything: `pending` (no row), `approved` (with `approved_by`, `approved_at`, `evidence`), `mismatched` (with `mismatched_fields`, drawn from `version, platform, backend, bundle_hash, metallib_hash, url, code_directory_hash, source_commit, ci_run_id`, sorted), or `revoked` (with `revoked_by`, `revocation_reason`) — revocation wins even when other requested fields differ from what was approved. The admin key does not substitute for the release key (401); an identity that fails `Validate` returns 400; unreadable qualification storage returns 503 `qualification_unavailable` | `coordinator/api/release_qualification_status.go` (`handleReleaseQualificationStatus`) |
