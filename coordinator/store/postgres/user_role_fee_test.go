...
func TestSetUserRole(t *testing.T) {
	...
	if err := s.CreateUser(&contracts.User{AccountID: "acct-1", PrivyUserID: "did:privy:1"}); err != nil {
		t.Fatal(err)
	}

	if err := s.SetUserRole("acct-1", contracts.RoleService); err != nil {
	...
	if u.Role != contracts.RoleService {
		t.Errorf("role = %q, want %q", u.Role, contracts.RoleService)
	}
	...
	if err := s.SetUserRole("nope", contracts.RoleService); err == nil {
