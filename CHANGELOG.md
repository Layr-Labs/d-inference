...
- Report the first failed legacy serving prerequisite to hardware-trusted providers, so `doctor` distinguishes missing APNs code identity, signed-release evidence, runtime posture and stale challenges from App Attest qualification. This changes diagnostics only; routing gates and MDM-removal requirements remain unchanged.
