...
async fn stale_reserved_job_is_swept_released() {
    ...
    let processed = sweep_reserved(&ledger, &eager_config())
        ...
        .expect("sweep fresh");
    assert_eq!(processed, 0);
    assert_eq!(pg::job_state(&db.pool, fresh.get()).await, "reserved");
}
...
async fn expired_prepared_job_is_swept_released() {
    ...
    let db = pg::boot().await;
    pg::set_epoch(&db.pool, 1).await;
    pg::seed_consumer(&db.pool, CONSUMER, 10_000_000, 0).await;
    let ledger = pg::ledger_at_epoch(&db.pool, 1);
    ...
    assert_eq!(processed, 1);
    assert_eq!(pg::job_state(&db.pool, job.get()).await, "released");
    assert_eq!(pg::balance_of(&db.pool, CONSUMER).await, (10_000_000, 0));
}
...
async fn stale_start_authorized_job_moves_to_review() {
    ...
    ledger
        ...
        .await
        .expect("resize");
    let funded = pg::balance_of(&db.pool, CONSUMER).await;
    ...
    assert_eq!(pg::balance_of(&db.pool, CONSUMER).await, funded);
    assert_eq!(
        pg::count(
            &db.pool,
...
async fn stale_running_job_without_terminal_moves_to_review() {
    ...
    assert_eq!(pg::job_state(&db.pool, with_receipt.get()).await, "running");
}
...
async fn unprojected_fees_are_folded_into_balances() {
    ...
    // (plan §12.11).
    assert_eq!(pg::balance_of(&db.pool, "platform").await, (480_000, 0));
    assert_eq!(pg::balance_of(&db.pool, REFERRER).await, (120_000, 120_000));
    assert_eq!(
        pg::count(
            &db.pool,
            "SELECT COUNT(*) FROM rust_coord.fee_allocations WHERE NOT projected"
