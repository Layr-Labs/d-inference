...
struct SpecDecLoadGateTests {
    ...
    func loadGateAddsExtraGb() {
        let base = 21.5
        #expect(WorkerInferenceSupport.loadGateWeightsGb(estimatedWeightsGb: base, extraWeightBytes: 0) == base)
        let withDrafter = WorkerInferenceSupport.loadGateWeightsGb(
    ...
    func requiredToLoadComposition() {
        let required = ModelLoadAdmission.requiredToLoadGb(
            weightsGb: WorkerInferenceSupport.loadGateWeightsGb(
    ...
    func pendingReservationIncludesExtra() {
        let bytes = WorkerInferenceSupport.pendingLoadReservationBytes(
    ...
    func pendingReservationBaseline() {
        #expect(
            WorkerInferenceSupport.pendingLoadReservationBytes(estimatedWeightsGb: 2.0, extraWeightBytes: 0)
                == 2 * 1_073_741_824)
    }
    ...
    func pendingReservationGarbageEstimate() {
        for garbage in [0.0, -3.5, .nan, .infinity, -.infinity] as [Double] {
            #expect(
                WorkerInferenceSupport.pendingLoadReservationBytes(
    ...
    func assistantAdmissionBoundary() {
        ...
        #expect(WorkerInferenceSupport.assistantMemoryFits(
