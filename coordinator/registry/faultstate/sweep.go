...
func pruneGateLocked[C comparable](g *gateState, r *Manager[C], now time.Time) (idle bool) {
...
func (r *Manager[C]) Sweep(now time.Time) {
	r.gatesMu.Lock()
...
func (r *Manager[C]) sweepGatesLocked(now time.Time) {
	r.gateSweepAt = now
	for key, g := range r.gates {
		g.mu.Lock()
		idle := pruneGateLocked(g, r, now)
		g.publishLocked()
