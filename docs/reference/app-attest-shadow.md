...
> Last updated: 2026-09-26 · commit `d4bee4702`
...
The coordinator derives two booleans without new wire data when the `ready` reply names a stored key; each is present only when both instants are known. `rebooted_since_last_success` is `boot_time` > the key's last verified assertion (its row insert time if it never asserted); `process_restarted_since_last_success` compares `process_started_at` the same way. Both appear in later events and evidence contexts of the attempt. Code: `coordinator/appattest/service/ready_diagnostics.go` (`deriveKeyLifecycleDiagnostics`).
