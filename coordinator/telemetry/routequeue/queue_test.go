...
func newTestSink(t *testing.T, st store.TelemetryStore, capacity, maxBatch int, window time.Duration) *Sink {
	t.Helper()
	sink := NewBatching(quietLogger(), capacity, 1, maxBatch, window)
	sink.Bind(st)
	t.Cleanup(func() { sink.CloseAndWait(5 * time.Second) })
	return sink
}

func mustCloseAndWait(t *testing.T, sink *Sink) {
	t.Helper()
...
func flushAndWait(t *testing.T, sink *Sink) {
	t.Helper()
