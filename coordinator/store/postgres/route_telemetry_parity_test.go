...
func TestInferenceRouteErrorReasonSurvivesReasonlessWrites(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			id := uniqueID("reason")
			read := func() contracts.InferenceRouteRecord {
				t.Helper()
			...
			if err := s.RecordInferenceRoute(&contracts.InferenceRouteRecord{RequestID: id, Attempt: 1, Outcome: "selected", ProviderID: "p1", ErrorReason: "jinja_template"}); err != nil {
				t.Fatalf("RecordInferenceRoute: %v", err)
			}
			if err := s.RecordInferenceRoute(&contracts.InferenceRouteRecord{RequestID: id, Attempt: 1, Outcome: "selected", ProviderID: "p2"}); err != nil {
			...
			if err := s.UpdateInferenceRouteOutcome(id, 1, &contracts.InferenceRouteOutcome{FinalStatus: "error", ErrorClass: "client_error", ErrorCode: 500, CompletionTokensSet: true}); err != nil {
			...
			if err := s.UpdateInferenceRouteOutcomes([]contracts.InferenceRouteOutcomeUpdate{{RequestID: id, Attempt: 1, Outcome: &contracts.InferenceRouteOutcome{ErrorReason: "provider_error"}}}); err != nil {
			...
			if err := s.RecordInferenceRoutes([]*contracts.InferenceRouteRecord{{RequestID: id, Attempt: 1, Outcome: "selected", ProviderID: "p3"}}); err != nil {
...
func TestInferenceRouteBatchStampsEachRecord(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			got := map[string]contracts.InferenceRouteRecord{}
			for _, r := range s.InferenceRouteRecordsSince(time.Time{}) {
			...
			for _, r := range []contracts.InferenceRouteRecord{a, b} {
