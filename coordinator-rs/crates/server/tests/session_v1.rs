...
use support::{attempt_sinks, FakeProvider, Harness, MODEL};
...
async fn full_v1_flow_streams_encrypted_chunks() {
    ...
    let grant = harness.admit_until_grant(support::ALIAS).await;
...
async fn no_heartbeat_means_model_not_ready_retry() {
    ...
    match harness.admit_once(support::ALIAS).await {
        AdmitOutcome::RetryAfter { .. } => {}
        other => panic!(
            "expected RetryAfter before first heartbeat, got {}",
            support::outcome_kind(&other)
        ),
    }

    // The heartbeat is exactly what flips it to a grant.
    provider.send_heartbeat(MODEL, "running").await;
    let _ = harness.admit_until_grant(support::ALIAS).await;
...
async fn reconnect_supersedes_prior_epoch() {
    ...
    let old_grant = harness.admit_until_grant(support::ALIAS).await;
    ...
    let new_grant = harness.admit_until_grant(support::ALIAS).await;
    assert_eq!(
...
async fn silent_socket_is_reaped_by_liveness_timeout() {
    ...
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    while let AdmitOutcome::Grant(_) = harness.admit_once(support::ALIAS).await {
        assert!(
            tokio::time::Instant::now() < deadline,
