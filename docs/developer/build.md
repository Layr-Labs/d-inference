...
| `coordinator/protocol/` | Go | Wire structs in `messages.go`; provider-frame dispatch and JSON decoding in `provider_message.go`. Both compile with the coordinator. |
