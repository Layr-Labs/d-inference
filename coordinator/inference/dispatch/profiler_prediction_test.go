...
func TestPredictiveProfileKeepsPolicySeparateFromBypass(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"soft", false, RoutePolicy{}, false, "soft", "none"},
		{"hard", true, RoutePolicy{}, false, "hard", "none"},
		{"media", true, RoutePolicy{}, true, "hard", "media"},
		{"self", true, RoutePolicy{Enabled: true}, false, "hard", "self_route"},
		{"prefer", true, RoutePolicy{Prefer: true}, false, "hard", "prefer_owner"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			srv.deps.TTFTHardReject = func() bool { return tc.hard }
...
func TestPredictiveProfileCapturesActualWriterEnvelopePerAttempt(t *testing.T) {
	...
	for i, requestID := range []string{"primary", "backup", "retry"} {
		...
		srv.recordPredictivePolicy(ap, RoutePolicy{}, false)
	...
	for _, deadline := range []time.Time{t0, {}} {
		...
		_, _ = builder(t0)
		if rec := (profiling.Builder{}).Build(rp, ap); rec.DispatchBudgetMs != nil {
