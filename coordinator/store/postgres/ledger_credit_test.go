...
type ledgerShape struct {
	Type   contracts.LedgerEntryType
	Amount int64
	After  int64
	Ref    string
}
...
func ledgerShapes(s contracts.Store, accountID string) []ledgerShape {
...
func legacyCredit(t *testing.T, pool *pgxpool.Pool, withdrawable bool, accountID string, amount int64, entryType contracts.LedgerEntryType, reference string) {
	t.Helper()
...
func TestPostgresCreditIsOneRoundTripAndByteIdentical(t *testing.T) {
	...
	type step struct {
		amount int64
		typ    contracts.LedgerEntryType
		ref    string
	}
	steps := []step{
		{100, contracts.LedgerRefund, "r1"},     // unknown account: row created
		{50, contracts.LedgerPlatformFee, "r2"}, // existing account
		{0, contracts.LedgerRefund, "r3"},       // zero: still recorded
		{-30, contracts.LedgerRefund, "r4"},     // negative: applied, recorded
	}
	wantRows := []ledgerShape{
		{contracts.LedgerRefund, 100, 100, "r1"},
		{contracts.LedgerPlatformFee, 50, 150, "r2"},
		{contracts.LedgerRefund, 0, 150, "r3"},
		{contracts.LedgerRefund, -30, 120, "r4"},
	}
...
func TestPostgresTransactionalCreditCallersUseOneStatement(t *testing.T) {
	...
	counter.reset()
	if err := s.CreditProviderWallet(&contracts.ProviderPayout{ProviderAddress: wallet, AmountMicroUSD: 700, Model: "m", JobID: uniqueID("job"), Timestamp: fixedAt}); err != nil {
	...
	if rows := ledgerShapes(s, wallet); len(rows) != 1 || rows[0].Type != contracts.LedgerPayout || rows[0].Amount != 700 || rows[0].After != 700 {
		t.Fatalf("wallet ledger = %+v", rows)
	}

	acct, ref := uniqueID("once"), uniqueID("ref")
	counter.reset()
	applied, err := s.CreditWithdrawableOnce(acct, 300, contracts.LedgerRefund, ref)
	if err != nil || !applied {
...
func TestCreditSemanticsAcrossBackends(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if err := s.Credit(acct, 250, contracts.LedgerRefund, "a"); err != nil {
				t.Fatalf("credit unknown account: %v", err)
			}
			rows := []ledgerShape{{contracts.LedgerRefund, 250, 250, "a"}}
			check("unknown account", 250, 0, rows)

			if err := s.Credit(acct, 0, contracts.LedgerPlatformFee, "z"); err != nil {
				t.Fatalf("zero credit: %v", err)
			}
			rows = append(rows, ledgerShape{contracts.LedgerPlatformFee, 0, 250, "z"})
			check("zero amount", 250, 0, rows)

			if err := s.Credit(acct, -100, contracts.LedgerRefund, "n"); err != nil {
				t.Fatalf("negative credit: %v", err)
			}
			rows = append(rows, ledgerShape{contracts.LedgerRefund, -100, 150, "n"})
			check("negative amount", 150, 0, rows)

			if err := s.CreditWithdrawable(acct, 40, contracts.LedgerPayout, "w"); err != nil {
				t.Fatalf("credit withdrawable: %v", err)
			}
			rows = append(rows, ledgerShape{contracts.LedgerPayout, 40, 190, "w"})
			...
			if err := s.Credit(acct, 10, contracts.LedgerRefund, "k"); err != nil {
				t.Fatalf("credit with nonzero withdrawable: %v", err)
			}
			rows = append(rows, ledgerShape{contracts.LedgerRefund, 10, 200, "k"})
			...
			if err := s.Debit(acct, 1000, contracts.LedgerCharge, "d"); !errors.Is(err, contracts.ErrInsufficientBalance) {
...
func TestConcurrentCreditDebitLedgerConsistency(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if err := s.Credit(acct, seed, contracts.LedgerRefund, "seed"); err != nil {
				t.Fatalf("seed: %v", err)
			}
			...
			for w := 0; w < workers; w++ {
				...
				go func(w int) {
					...
					for i := 0; i < opsPerWorker; i++ {
						...
						if (w+i)%2 == 0 {
							if err := s.Credit(acct, n, contracts.LedgerRefund, fmt.Sprintf("c-%d-%d", w, i)); err != nil {
								failures.Add(1)
								continue
							}
							credited.Add(n)
						...
						err := s.Debit(acct, n, contracts.LedgerCharge, fmt.Sprintf("d-%d-%d", w, i))
						switch {
						case err == nil:
							debited.Add(n)
							debits.Add(1)
						case errors.Is(err, contracts.ErrInsufficientBalance):
							insufficient.Add(1)
						default:
							failures.Add(1)
						}
