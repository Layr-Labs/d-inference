//! Stream=true live wait_terminal also aborts on ownership steal (DECISIONS #65).
...
#[tokio::test]
async fn live_stream_wait_terminal_aborts_on_ownership_steal() {
    ...
    state
        ...
        .upsert_lifecycle(ProviderSnapshot {
            provider_id: "p-stream-wait".into(),
            session_epoch: 1,
            trusted: true,
    ...
    hub.attach("p-stream-wait".into(), 1, tx).await;
    let hub2 = hub.clone();
            ...
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        ...
                        "messages": [{"role":"user","content":"stream wait"}],
                        "stream": true
                    })
                    .to_string(),
                ))
