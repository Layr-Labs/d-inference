...
The four-layer attestation architecture (Section~\ref{sec:attestation}) and the ACME enrollment protocol (Section~\ref{sec:enrollment}) establish a spectrum of verifiable trust guarantees. We formalize this spectrum into \emph{trust tiers} that consumers can select at inference time, allowing each request to specify the minimum acceptable level of cryptographic assurance.
...
This design separates the permission requirements by tier:
...
Consumers specify their minimum trust tier as a parameter in the inference request. The coordinator's routing algorithm filters the candidate provider set by trust tier before applying the composite scoring function (Section~\ref{sec:scoring}). Tier~3 providers receive a $1.2\times$ scoring multiplier, reflecting the stronger cryptographic guarantees they provide.

Workloads with stringent privacy requirements (e.g., processing protected health information or privileged legal documents) can require \texttt{apple\_grade} trust, ensuring that every token in the response is signed by a key whose Secure Enclave residency is cryptographically verified by Apple. Workloads with lower sensitivity can accept \texttt{hardware} trust, which broadens the available provider set while still requiring Apple-verified security posture.
