...
func TestTrustReuseJournalPathPrecedence(t *testing.T) {
	...
	if got := JournalPathFromEnv(); got != filepath.Join("/mnt/disks/userdata", "coordinator", trustReuseJournalFilename) {
		t.Fatalf("default journal path = %q", got)
	}
	t.Setenv("USER_PERSISTENT_DATA_PATH", "/persistent")
	if got := JournalPathFromEnv(); got != filepath.Join("/persistent", "coordinator", trustReuseJournalFilename) {
		t.Fatalf("persistent-root journal path = %q", got)
	}
	t.Setenv(envTrustReuseRevocationJournal, "/override/revocations.jsonl")
	if got := JournalPathFromEnv(); got != "/override/revocations.jsonl" {
...
func TestHardUntrustJournalMalformedLineFailsClosed(t *testing.T) {
	...
	blocked, reason := srv.SafetyStatus()
	if !blocked || reason != JournalHealthReason {
...
func TestHardUntrustJournalCleanupFailureLeavesPendingDenial(t *testing.T) {
	...
	srv.journal = &failingJournal{
		hardUntrustJournal: srv.journal,
	...
	srv.Invalidate(rec.SEPubKey)
	entries, err := srv.journal.Load()
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
...
func TestHardUntrustJournalStoreOutageKeepsPendingAndBlocksReplay(t *testing.T) {
	...
	blocked, reason := srv.SafetyStatus()
	if !blocked || reason != ReplayHealthReason {
...
func TestHardUntrustJournalReviewerTraceThreeFailuresRestartRecovery(t *testing.T) {
	...
	if afterRestart.cache.hasFreshRecord(rec.SEPubKey, rec.Serial) {
		t.Fatal("stale pre-untrust row seeded after restart")
	}
	if _, ok := afterRestart.cache.reuse(rec.SEPubKey, rec.Serial, trHashA); ok {
	...
	remaining, err := afterRestart.journal.Load()
...
func TestHardUntrustJournalRuntimeReplayClearsFailClosedLatch(t *testing.T) {
	...
	srv.Invalidate(rec.SEPubKey)
	if blocked, reason := srv.SafetyStatus(); !blocked ||
		reason != ReplayHealthReason {
...
func TestHardUntrustJournalReplayCreatesMissingTombstone(t *testing.T) {
	...
	if !srv.cache.isRevoked("se-no-row") {
