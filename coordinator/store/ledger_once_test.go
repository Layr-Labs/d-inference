...
func TestLedgerCreditOnceAcrossConcurrentHandles(t *testing.T) {
	for name, backend := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			handles := []Store{backend}
			if pg, ok := backend.(*PostgresStore); ok {
				...
				handles = append(handles, &PostgresStore{pool: pool})
			}
			for _, withdrawable := range []bool{false, true} {
				label := "deposit"
				entryType := LedgerStripeDeposit
				if withdrawable {
					label, entryType = "refund", LedgerRefund
				}
				t.Run(label, func(t *testing.T) {
					accountID, reference := uniqueID("credit-once"), "event-reference"
					if err := backend.CreditWithdrawable(accountID, 50, LedgerPayout, "prior-earnings"); err != nil {
						t.Fatal(err)
					}
					type result struct {
						applied bool
...
func TestCreditOnceRecognizesPriorLedgerAndIndependentIdentities(t *testing.T) {
	for name, backend := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if err := backend.Credit(accountID, 100, LedgerStripeDeposit, reference); err != nil {
				t.Fatal(err)
			}
			if applied, err := backend.CreditOnce(accountID, 100, LedgerStripeDeposit, reference); err != nil || applied {
			...
			for _, identity := range []struct {
				...
				{uniqueID("other-account"), reference, LedgerStripeDeposit},
				{accountID, "stripe:next-checkout", LedgerStripeDeposit},
				{accountID, reference, LedgerDeposit},
			} {
