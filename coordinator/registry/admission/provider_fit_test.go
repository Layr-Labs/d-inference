...
func TestProviderBudgetFitsWarmSlotLiveBudget(t *testing.T) {
	...
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(snap), 8_000, 1_000); !known || fits {
		t.Fatalf("9000-token request vs 8768 live budget = (fits=%v, known=%v), want (false, true)", fits, known)
	}
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(snap), 8_000, 512); !known || !fits {
	...
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(snap), 8_512, 256); !known || !fits {
	...
	if fits, _ := testPolicy.ProviderBudgetFits(snapPtr(snap), 8_512, 0); !fits {
		t.Fatal("reqMax=0 must default to defaultRequestedMaxTokens (8512+256=8768 fits)")
	}
	if fits, _ := testPolicy.ProviderBudgetFits(snapPtr(snap), 8_513, 0); fits {
		t.Fatal("reqMax=0 must default to defaultRequestedMaxTokens (8513+256=8769 must not fit)")
	}
	// Negative prompt clamps to 0.
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(snap), -5, 256); !known || !fits {
...
func TestProviderBudgetFitsColdLoadPostLoadBudget(t *testing.T) {
	...
	budget := testPolicy.ColdTokenBudgetEstimate(snap.TotalMemoryGB, snap.ModelSizeGB, 0, snap.BinaryVersion, snap.Model)
	...
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(snap), 30_000, 256); !known || fits {
	...
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(snap), 10_000, 256); !known || !fits {
...
func TestProviderBudgetFitsFailsOpenOnUnknown(t *testing.T) {
	...
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(Snapshot{ModelLoaded: true}), 1_000_000, 256); known || !fits {
		t.Fatalf("legacy resident slot = (fits=%v, known=%v), want (true, false)", fits, known)
	}
	// Cold slot missing memory/size data.
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(Snapshot{ModelSizeGB: 28}), 1_000_000, 256); known || !fits {
		t.Fatalf("cold slot missing memory = (fits=%v, known=%v), want (true, false)", fits, known)
	}
	if fits, known := testPolicy.ProviderBudgetFits(snapPtr(Snapshot{TotalMemoryGB: 48}), 1_000_000, 256); known || !fits {
