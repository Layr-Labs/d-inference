...
func TestMessagesResponsesPreserveExactMatchedStopSequence(t *testing.T) {
	...
	emitter := NewEndpointSink(recorder, recorder, pr, nil)
	emitter.Start()
	emitter.Chunk(`data: {"choices":[{"index":0,"delta":{"content":"answer"},"finish_reason":"length"}]}`)
...
func TestGenericEndpointStreamEmittersUseNativeSchemas(t *testing.T) {
	t.Run("completions corrects max token finish", func(t *testing.T) {
		...
		emitter := NewEndpointSink(recorder, recorder, pr, nil)
		emitter.Start()
		emitter.Chunk(`data: {"choices":[{"index":0,"delta":{"content":"ok"},"finish_reason":null}]}`)
		emitter.Chunk(`data: {"choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}`)
	...
	t.Run("messages emits tool use lifecycle", func(t *testing.T) {
		...
		emitter := NewEndpointSink(recorder, recorder, pr, nil)
		emitter.Start()
		emitter.Chunk(`data: {"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call-1","function":{"name":"weather","arguments":"{\"city\":\"SF\"}"}}]},"finish_reason":"tool_calls"}]}`)
	...
	t.Run("messages preserves parallel all-index-zero calls", func(t *testing.T) {
		...
		emitter := NewEndpointSink(recorder, recorder, pr, nil)
		emitter.Start()
		emitter.Chunk(`data: {"choices":[{"index":0,"delta":{"tool_calls":[` +
	...
	t.Run("messages committed error uses native envelope", func(t *testing.T) {
		...
		emitter.Error("provider_error", "generation failed")
		...
		timeoutEmitter := NewEndpointSink(timeoutRecorder, timeoutRecorder, pr, nil)
		timeoutEmitter.Error("timeout", "request timed out")
