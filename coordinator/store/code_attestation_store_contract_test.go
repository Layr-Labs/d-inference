...
func codeAttestRoundTrip(t *testing.T, st Store) {
	...
	if err := st.UpsertCodeAttestation(ctx, CodeAttestation{SEPubKey: "se-A", Version: "0.6.0", AttestedAt: t0, APNsToken: "tok-A", BinaryHash: "hash-A"}); err != nil {
	...
	if err := st.UpsertCodeAttestation(ctx, CodeAttestation{SEPubKey: "se-A", Version: "0.6.2", AttestedAt: t1, APNsToken: "tok-A2", BinaryHash: "hash-A2"}); err != nil {
	...
	if err := st.DeleteCodeAttestation(ctx, "se-A"); err != nil {
	...
	if err := st.DeleteCodeAttestation(ctx, "missing"); err != nil {
		t.Fatalf("delete missing key: %v", err)
	}
}

func TestCodeAttestationStoreContract(t *testing.T) {
	for name, st := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			codeAttestRoundTrip(t, st)
		})
	}
}
