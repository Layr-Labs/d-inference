...
//! `http` and `net` suite modules. No real database, no real fleet, no
//! real sockets.
...
/// The recording ledger keeps every call's FULL payload by design: which
/// payloads a suite asserts varies (only `Resize`/`Settle` today), and the
/// rest document the seam and ride along in `{:?}` failure output.
#[allow(dead_code)]
#[derive(Debug, Clone)]
pub enum LedgerCall {
...
impl ProviderSim {
    ...
    pub async fn expect_attach(&mut self) -> AttachedAttempt {
        ...
        match command {
            SessionCommand::AttachAttempt { wire_id, sinks, .. } => AttachedAttempt {
                wire_id,
...
pub enum AdmitReply {
    ...
    Grant(usize),
    RetryAfter(Duration),
    /// Kept unconstructed on purpose: the script API mirrors all three
    /// `AdmitOutcome` arms even though no scenario scripts an explicit
    /// reject today (an exhausted script already rejects).
    #[allow(dead_code)]
    Reject,
}
