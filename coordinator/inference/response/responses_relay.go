...
func (s *Writer) handleResponsesStreamingResponseWithFirstChunk(
	...
	emitter := NewResponsesSink(w, deferred, pr, responseID, createdAt, s.deps.Observer)
	emitter.Start()

	for _, firstChunk := range firstChunks {
		if firstChunk != "" {
			emitter.Chunk(sanitizeStreamCacheDetails(firstChunk))
		}
	}
	if initialError != nil {
	...
	finishStream := func() {
		...
		if !completed && s.deps.Reservation.Refund(pr, "provider_incomplete:"+pr.RequestID) {
