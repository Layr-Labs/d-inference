...
func TestInviteRedemptionCreditsOneWinningAccount(t *testing.T) {
	for name, s := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			if err := s.CreateInviteCode(&contracts.InviteCode{Code: code, AmountMicroUSD: 2_000_000, MaxUses: 1, Active: true}); err != nil {
				t.Fatal(err)
			}
			accounts := []string{"invite-a", "invite-b"}
			for _, id := range accounts {
				if err := s.CreateUser(&contracts.User{AccountID: id, PrivyUserID: "did:" + id}); err != nil {
					t.Fatal(err)
				}
			}
...
func TestPostgresInviteCreditFailureRollsBackClaim(t *testing.T) {
	...
	if err := s.CreateUser(&contracts.User{AccountID: account, PrivyUserID: "did:" + account}); err != nil {
		t.Fatal(err)
	}
	if err := s.CreateInviteCode(&contracts.InviteCode{Code: code, AmountMicroUSD: 3_000_000, MaxUses: 1, Active: true}); err != nil {
		t.Fatal(err)
	}
