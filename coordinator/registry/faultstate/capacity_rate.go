...
// live on the identity's gate (state.go), so reconnects cannot reset
...
func (r *Manager[C]) CapacityRatePenalty(providerID, modelID string, now time.Time) (penaltyMs, rate float64) {
...
func (r *Manager[C]) CapacityRejectRate(providerID, modelID string) (rate float64, samples int) {
