...
func (s *PostgresStore) ListProviderNotificationTargets(ctx context.Context) ([]ProviderNotificationTarget, error) {
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("store: iterate provider notification targets: %w", err)
	}
	return targets, nil
}
...
func (s *PostgresStore) ProviderNotificationsDue(ctx context.Context, checks []ProviderNotificationCheck, cooldown time.Duration) (ProviderNotificationDueSet, error) {
	...
	for start := 0; start < len(checks); start += providerNotificationBatchSize {
		end := start + providerNotificationBatchSize
		if end > len(checks) {
			end = len(checks)
		}
		var query strings.Builder
		query.WriteString(`SELECT n.provider_id, n.reason_key, n.last_sent_at
			   FROM (VALUES `)
		args := make([]any, 0, (end-start)*2)
		arg := 1
		for i, check := range checks[start:end] {
			if i > 0 {
				query.WriteString(",")
			}
			fmt.Fprintf(&query, "($%d::text,$%d::text)", arg, arg+1)
			args = append(args, check.ProviderID, string(check.ReasonKey))
			arg += 2
		}
		query.WriteString(`) AS input(provider_id, reason_key)
		...
		for rows.Next() {
			...
			if err := rows.Scan(&providerID, &reasonKey, &lastSent); err != nil {
				rows.Close()
...
func (s *PostgresStore) RecordProviderNotificationsSent(ctx context.Context, checks []ProviderNotificationCheck, sentAt time.Time) error {
	...
	for start := 0; start < len(checks); start += providerNotificationBatchSize {
		...
		batch := &pgx.Batch{}
		for _, check := range checks[start:end] {
			batch.Queue(
				...
				check.ProviderID, check.AccountID, string(check.ReasonKey), sentAt,
			)
		}
		results := s.pool.SendBatch(ctx, batch)
		for range checks[start:end] {
