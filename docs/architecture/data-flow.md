...
3. **Funds are reserved before dispatch and settled from provider-reported usage** — `reserveInferenceBalance` (`coordinator/api/inference_admission.go`), `Service.CompleteAt` (`coordinator/inference/providerframe/complete.go`).
