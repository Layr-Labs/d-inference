...
| Declared metric: adding, renaming, retyping or retagging one (`coordinator/metrics/catalog_*.go`) | `architecture/telemetry.md` (the declared-catalog section), `reference/telemetry-inventory.md` — regenerate its declared table with `cd coordinator && go run ./metrics/cmd/metricdoc`; `coordinator/metrics` tests fail until you do |
