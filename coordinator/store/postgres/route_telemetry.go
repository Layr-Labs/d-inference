...
func inferenceRouteInsertArgs(dst []any, record *contracts.InferenceRouteRecord, now time.Time) []any {
...
func inferenceRouteOutcomeUpdateArgs(requestID string, attempt int, outcome *contracts.InferenceRouteOutcome) []any {
	return []any{
		requestID, attempt,
...
func (s *Store) RecordInferenceRoute(record *contracts.InferenceRouteRecord) error {
	...
	if err := s.execInferenceRouteInsert([]*contracts.InferenceRouteRecord{record}, inferenceRouteWriteTimeout); err != nil {
...
func (s *Store) RecordInferenceRoutes(records []*contracts.InferenceRouteRecord) error {
	for _, chunk := range routerecord.SplitInferenceRouteBatches(records, maxInferenceRouteInsertRows) {
...
func (s *Store) execInferenceRouteInsert(rows []*contracts.InferenceRouteRecord, timeout time.Duration) error {
