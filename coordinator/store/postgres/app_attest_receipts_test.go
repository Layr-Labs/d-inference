...
func TestAppAttestReceiptVersionsAndLeasePostgres(t *testing.T) {
	...
	now := time.Now().UTC()
	r := contracts.AppAttestReceipt{ID: "initial", KeyID: "key", EvidenceID: "evidence", ReceivedAt: now, Outcome: "verified", Body: []byte{0, 1, 2}, Context: json.RawMessage(`{}`), Details: json.RawMessage(`{}`), NextAt: now.Add(-time.Second), ExpiresAt: now.Add(time.Hour)}
