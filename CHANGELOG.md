...
- **Refuse to install below the supported macOS floor** — `install.sh` checks `sw_vers -productVersion` against `MIN_MACOS` before downloading, rather than failing after a multi-gigabyte download and a staged-app verification error. `LSMinimumSystemVersion` moves from 14.0 to the same floor. `scripts/check-macos-floor.sh` (new, wired into CI) pins the floor and both metallib deployment targets across the installers, the release workflow, `scripts/fetch-metallib.sh`, and `PackagedMetallib.swift`.
