...
Focused release tests cover full/sliding cache offsets, sinks, chunk boundaries, embeddings, rollback policy, quantization aliases, checkpoint loading, native expert boundaries and tails, mutation and parameter-tree invariants. The final focused run passed 58 Swift Testing cases and 8 XCTest cases; the optional real-checkpoint test was run separately. The Python benchmark/control suite passed 24 tests. The C++ doctest additions are source coverage; this run executed the equivalent Swift GPU tests rather than a separate native CMake test build.
