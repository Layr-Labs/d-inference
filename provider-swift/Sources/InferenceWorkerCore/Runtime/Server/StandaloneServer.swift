...
/// Configuration for the worker-owned inference runtime.
...
public actor StandaloneServer {
    ...
    /// Resolves model IDs to brokered snapshot capabilities. Production local
    /// serving uses the existing Hugging Face resolver; the sandboxed XPC
    /// worker injects its hash-bound, security-scoped snapshot map.
    private let modelPathResolver: @Sendable (String) -> URL?
    private let expectedModelHashResolver: @Sendable (String) -> String?
    private var loadedManifestHashes: [String: String] = [:]
    ...
    public init(
        ...
        models: [ModelInfo] = [],
        modelPathResolver: @escaping @Sendable (String) -> URL? = {
            ModelScanner.resolveLocalPath(modelID: $0)
        },
        expectedModelHashResolver: @escaping @Sendable (String) -> String? = { _ in nil }
    ) {
        self.config = config
        self.modelPathResolver = modelPathResolver
        self.expectedModelHashResolver = expectedModelHashResolver
        ...
        self.maximumCachedModels = max(1, config.maxCachedModels)
    ...
    public func startMaintenance() {
        guard kvSweepTask == nil else { return }
        kvSweepTask = Task { [weak self] in
            while !Task.isCancelled {
                try? await Task.sleep(for: .seconds(1))
                if Task.isCancelled { return }
                await self?.performMaintenanceTick()
            }
        }
    }
    ...
    private func performMaintenanceTick() async {
        ...
        let candidates = slots.map { ($0.key, $0.value) }
        for (modelID, slot) in candidates {
            guard await slot.bridge.confirmedWedgeForRecovery() else { continue }
            guard slots[modelID]?.bridge === slot.bridge else { continue }
            await slot.bridge.beginRecoveryReload()
            ...
            loadedManifestHashes.removeValue(forKey: modelID)
            MLX.Memory.clearCache()
            await resliceGrowSurvivors()
            standaloneLogger.error(
                "Recovered confirmed engine wedge by retiring model slot: \(modelID)")
        }
    }

    /// Drain every resident engine and release all model/KV state before a
    /// catalog generation is replaced.
    public func shutdown() async {
        kvSweepTask?.cancel()
        kvSweepTask = nil
        await specDecFunnel.shutdown()
        var bridges = slots.values.map(\.bridge)
        for bridge in bridges { await bridge.shutdown() }
        bridges.removeAll()
        slots.removeAll()
        slotReservations.removeAll()
        modelsLoading.removeAll()
        loadedManifestHashes.removeAll()
        MLX.Memory.clearCache()
    }
    ...
    private func engineV2MaxConcurrent(forModel modelId: String) -> Int {
        ...
        return WorkerInferenceSupport.clampEngineV2Concurrency(raw)
    }
    ...
    private func evictLRUIdleSlot() async -> Bool {
        ...
        if slots[evictKey]?.bridge === evicted.bridge {
            ...
            loadedManifestHashes.removeValue(forKey: evictKey)
        }
    ...
    private func evictIfNeededForLoad() async throws {
        guard slots.count >= maximumCachedModels else { return }

        guard await evictLRUIdleSlot() else {
            throw StandaloneServerError.capacityUnavailable(
                "All \(maximumCachedModels) cached model slot(s) are active; try again when a request finishes"
            )
        }
    }
    ...
    func ensureModelLoaded(_ modelId: String) async throws {
        ...
        do {
            ...
            if let artifact = mtpPreparation.artifact {
                if !WorkerInferenceSupport.assistantMemoryFits(
            ...
            let pendingLoadBytes = WorkerInferenceSupport.pendingLoadReservationBytes(
            ...
            let expectedManifestHash = expectedModelHashResolver(modelId)
            let mustHashArtifact = reusableSSDRequested || expectedManifestHash != nil
            let preLoadCacheHash = mustHashArtifact
                ? await computeStandaloneWeightHash(modelPath: modelPath, modelId: modelId)
                : nil
            if let expectedManifestHash, preLoadCacheHash != expectedManifestHash {
                throw StandaloneServerError.capacityUnavailable(
                    "Model artifact failed its approved pre-load manifest binding")
            }
            ...
            let slotIsVLM = WorkerInferenceSupport.modelIsVLM(at: modelPath)
            ...
            try Task.checkCancellation()
            let postLoadCacheHash = mustHashArtifact
                ? await computeStandaloneWeightHash(modelPath: modelPath, modelId: modelId)
                : nil
            if let expectedManifestHash, postLoadCacheHash != expectedManifestHash {
                newcomer.release()
                MLX.Memory.clearCache()
                throw StandaloneServerError.capacityUnavailable(
                    "Model artifact changed during load or no longer matches its approved manifest")
            }
            let cacheEligibleWeightHash: String?
            if reusableSSDRequested {
                switch WorkerInferenceSupport.reusableSSDWeightHashDecision(
            ...
            if let expectedManifestHash {
                let finalManifestHash = await computeStandaloneWeightHash(
                    modelPath: modelPath, modelId: modelId)
                guard finalManifestHash == expectedManifestHash else {
                    await unwindBuiltSlotAndRegrow(
                        bundle: bundle, newcomer: newcomer)
                    throw StandaloneServerError.capacityUnavailable(
                        "Model artifact changed during final configuration reads; refusing admission")
                }
            }
            ...
            slots[modelId] = CachedSlot(
                ...
                lastUsedAt: .now)
            if let expectedManifestHash {
                loadedManifestHashes[modelId] = expectedManifestHash
            }
    ...
    func workerEvictIdleSlots(olderThan timeout: Duration) async {
        guard timeout > .zero else { return }
        while let oldest = slots
            .filter({ (slotReservations[$0.key] ?? 0) == 0 })
            .min(by: { $0.value.lastUsedAt < $1.value.lastUsedAt }),
              ContinuousClock.now - oldest.value.lastUsedAt >= timeout {
            guard await evictLRUIdleSlot() else { return }
        }
    }

    /// Worker-only safe snapshot. It contains identifiers and integer capacity
    /// facts, never model containers, tokenizers, KV objects, or prompts.
    func workerCapacityFacts() async -> [(
        modelIdentifier: String, loaded: Bool, active: Int,
        capacity: BackendSlotCapacity?, manifestHash: String?,
        mtpModelIdentifier: String?,
        prefixCacheCapability: PrefixCacheV2Capability?,
        ...
        facts.reserveCapacity(models.count)
        for model in models {
            let slot = slots[model.id]
            let mtp = await slot?.bridge.mtpStatusSnapshot()
            let capacity = await slot?.bridge.backendSlotCapacity()
            let prefixCache = slot?.bridge.prefixCacheAdvertisement()
            facts.append((
                modelIdentifier: model.id,
                loaded: slot != nil,
                active: max(0, slotReservations[model.id] ?? 0),
                capacity: capacity,
                manifestHash: loadedManifestHashes[model.id],
                mtpModelIdentifier: mtp?.assistantModelID,
                prefixCacheCapability: prefixCache?.capability,
                prefixCacheStatus: prefixCache?.status
            ))
        }
        return facts
    }
    ...
    func workerPrefixCacheV2Capability(
        modelIdentifier: String?
    ) -> PrefixCacheV2Capability? {
        guard let modelIdentifier,
              let slot = slots[modelIdentifier]
        else { return nil }
        return slot.bridge.prefixCacheAdvertisement().capability
    }

    func workerTakeNextPrefixCacheV2Sequence(
        modelIdentifier: String?,
        ...
        return slot.bridge.takeNextPrefixCacheV2Sequence(
