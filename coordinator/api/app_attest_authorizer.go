...
type appAttestAuthorizer struct {
	mu          sync.Mutex
	s           *Server
...
func (s *Server) startAppAttestAuthorizer(ctx context.Context) {
	if !s.appAttestShadow.ServingEnabled || s.appAttestShadow.Environment != "production" {
		return
	}
	a := &appAttestAuthorizer{s: s, current: make(map[*registry.Provider]*appAttestAuthorizationRecord)}
	a.post, a.postPending = make(chan *registry.Provider, 256), make(map[*registry.Provider]bool)
	s.appAttestAuthorizer = a
...
func (a *appAttestAuthorizer) remember(p *registry.Provider, record *appAttestAuthorizationRecord) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.current[p] = record
}

func (a *appAttestAuthorizer) forget(p *registry.Provider) {
	if a == nil {
		return
	}
	a.mu.Lock()
...
func (a *appAttestAuthorizer) queuePostLocked(p *registry.Provider) {
...
func (a *appAttestAuthorizer) refresh(ctx context.Context) {
...
func (a *appAttestAuthorizer) apply(p *registry.Provider, record *appAttestAuthorizationRecord, state store.AppAttestReadiness, observedAt time.Time) bool {
	e := record.evidence
	applyAppAttestReadiness(&e, state)
	if record.dropped != nil && record.dropped() > 0 {
		e.ArchiveComplete = false
	}
	snapshot := a.s.releaseTrustPolicy.Load()
	e.CatalogKnown = snapshot != nil && len(snapshot.ByBinaryHash) > 0
