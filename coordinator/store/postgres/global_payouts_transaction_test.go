...
func TestPostgresGlobalPayoutRefundRollsBackWhenPayoutWriteFails(t *testing.T) {
	...
	for _, p := range []contracts.GlobalPayout{first, second} {
		if _, err := s.BeginGlobalPayout(p.AccountID, p.ID, now); err != nil {
			t.Fatal(err)
		}
	}
	if err := s.ApplyGlobalPayout(first.ID, contracts.GlobalPayoutResult{Status: "posted", ExternalID: "obp_first"}, now); err != nil {
		t.Fatal(err)
	}
	...
	if err := s.ApplyGlobalPayout(second.ID, contracts.GlobalPayoutResult{Status: "returned", ExternalID: "obp_first"}, now); err == nil {
	...
	for range 2 {
		if err := s.ApplyGlobalPayout(second.ID, contracts.GlobalPayoutResult{Status: "returned", ExternalID: "obp_second"}, now); err != nil {
			t.Fatal(err)
		}
	}
