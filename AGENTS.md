...
- The provider's memory model is `UnifiedMemoryCap` (`provider-swift/Sources/ProviderCore/Inference/UnifiedMemoryCap.swift`): hard cap = 0.90 × physical RAM (always leaving ≥ 2 GiB for the OS; `DARKBLOOM_MEM_CAP_FRACTION` override). The model-load gate requires resident weights + incoming weights + the concurrent fleet activation reserve + 1 GiB minimum KV ≤ the cap, and a post-load guard unloads a freshly-loaded model whose measured live KV headroom is below that minimum. `DARKBLOOM_ACTIVATION_RESERVE_GB` is **raise-only** relative to each selected model profile. `BackendCapacity.freeForLoadGb` remains a conservative compatibility scalar; while fully idle with no load transition, current providers also report an unclamped pre-activation basis so current coordinators can subtract the candidate's exact reserve.
- `ModelActivationPolicy` selects activation memory from measured execution profiles, not a formula or model name. The exact measured GPT-OSS 20B config/architecture uses 3 GiB; unknown, incomplete, modified, and vision profiles use the conservative 5.5 GiB reserve. The process holds the **sum** across resident and loading models: each EngineV2 slot owns an independent queue, and MLX's `asyncEval` lock serializes submission rather than GPU completion, so activation lifetimes may overlap. Providers report the resolved per-model byte count as `ModelInfo.activation_reserve_bytes`; `coordinator/registry/servability.go` consumes that exact value for cold-budget prediction and uses version-gated 3/5.5 GiB fallbacks only when old providers omit it. Never derive a second coordinator-side model estimate: profile selection, provider gates, wire reporting, and coordinator consumption must move together.
