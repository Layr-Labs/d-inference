...
- **MoE expert SSD streaming (DeepSeek-V4)**: Checkpoints whose routed experts exceed RAM (141GB DSV4-Flash on 128GB boxes) serve via `DeepseekV4ExpertStreaming` (mlx-swift-lm): the loader skips `switch_mlp` tensors (~125GB never read), and `StreamingQuantizedSwitchGLU` preads only the 6-of-256 routed experts per layer per token through a byte-budgeted LRU (`ExpertCache`). Budgets derive from `UnifiedMemoryCap` (never physical RAM): cache = cap − resident − activation reserve − KV target. Enabled per provider via `backend.stream_experts`; admission uses a streaming-aware estimate (`ExpertStreamingAdmission`). The cache is purged in `BatchScheduler.stopCurrentEngine()` on unload. Decode is cold-miss disk-bound (~2 tok/s at 90% hit rate).
- **Sequential serving**: Models whose cache layout the batched engine can't represent (`Scheduler.supportsBatchedServing == false`; DeepSeek-V4's pooling caches) serve one request at a time through a raw-text token loop (`BatchScheduler+SequentialRawRunner`) — never `container.generate`, whose built-in tool parser can consume text. Heartbeats clamp advertised concurrency to 1; concurrent submits get the retryable `token_budget_exhausted` rejection.
- **DeepSeek-V4 prompt encoding**: DSV4 ships NO chat template — the provider uses a native Swift port (`DeepseekV4Encoding`, ProviderCoreFoundation) of the official `encoding/encoding_dsv4.py`, validated byte-for-byte against the official golden fixtures. Tool calls use the bespoke DSML block format, parsed by `DSMLToolCallParser` (mlx-swift-lm, `ToolCallFormat.dsml`). `TemplateRenderCheck` exercises the native encoder for template-less DSV4 snapshots so `template_render_ok` stays honest.
- **NAX disabled in releases**: M5 NAX (Neural Accelerator) bf16 GEMM kernels are nondeterministic for some shapes (intermittent NaN; repro: `DSV4Smoke --op-stress`). Release builds pass `-Xcc -DMLX_METAL_NO_NAX` (zero measured cost for MoE serving — see docs/spikes/nax-nondeterminism-m5.md). Re-test on every mlx bump.
