...
type duePageCapability interface {
	ListDueVerificationJobsPage(ctx context.Context, now time.Time, limit, offset int) ([]contracts.VerificationJob, error)
}

// opaqueStore is a decorator that does NOT implement Unwrapper: As must stop
// at it rather than guess.
type opaqueStore struct{ contracts.Store }
...
func TestAsFindsOptionalCapabilitiesThroughCachedStore(t *testing.T) {
	for name, backend := range storeBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			cached := storecache.New(backend, storecache.CacheConfig{})
			...
			budgets, ok := contracts.As[pushBudgetCapability](cached)
			if !ok {
			...
			paged, ok := contracts.As[duePageCapability](cached)
			if !ok {
			...
			// nested decorators.
			var narrow contracts.ProviderStore = cached
			if _, ok := contracts.As[duePageCapability](narrow); !ok {
				t.Fatal("As failed on a narrow sub-interface value")
			}
			if _, ok := contracts.As[pushBudgetCapability](storecache.New(cached, storecache.CacheConfig{})); !ok {
			...
			if _, err := cached.UpsertVerificationJob(context.Background(), contracts.VerificationJob{
				SEPubKey: seKey, Serial: "serial", Kind: contracts.VerificationTaskSecurityInfo,
				State: contracts.VerificationStatePending, Priority: contracts.VerificationPriorityFirstOrExpired,
...
func TestAsStopsAtNonUnwrappingDecoratorsAndNil(t *testing.T) {
	mem := memory.New(contracts.Config{})
	if _, ok := contracts.As[pushBudgetCapability](mem); !ok {
		t.Fatal("As must succeed on the bare backend")
	}
	if _, ok := contracts.As[pushBudgetCapability](opaqueStore{mem}); ok {
		t.Fatal("As must not see through a decorator that does not implement Unwrapper")
	}
	if _, ok := contracts.As[pushBudgetCapability](storecache.New(opaqueStore{mem}, storecache.CacheConfig{})); ok {
		t.Fatal("As must stop at the first non-unwrapping layer")
	}
	if _, ok := contracts.As[pushBudgetCapability](nil); ok {
		t.Fatal("As(nil) must be false")
	}
	var nilStore contracts.Store
	if _, ok := contracts.As[pushBudgetCapability](nilStore); ok {
	...
	if _, ok := contracts.As[absent](storecache.New(mem, storecache.CacheConfig{})); ok {
